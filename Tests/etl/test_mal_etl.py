...
class TestMalETL(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.original_output_dir = (
            MalETL.output_dir if hasattr(MalETL, "output_dir") else None
        )
    ...
    def setUp(self):  # Original signature
        ...
        self.sample_anime_node_1 = {"id": 1, "title": "Test Anime 1"}
        self.sample_anime_node_2 = {"id": 2, "title": "Test Anime 2"}

        self.seasonal_response = {"data": [self.sample_anime_node_1], "paging": {}}
        self.popular_response = {"data": [self.sample_anime_node_2], "paging": {}}
        self.favorite_response = {"data": [], "paging": {}}
    ...
    def test_initialization_success(self):  # Removed mock_load_dotenv_arg
        ...
        assert self.etl is not None
        assert self.etl.client_id == self.test_client_id
        assert self.etl.output_dir == self.test_output_dir
    ...
    def test_initialization_no_client_id(self):  # Removed mock_load_dotenv_arg
        ...
        with pytest.raises(ValueError) as context:
            MalETL()
        assert "MAL_CLIENT_ID environment variable not set" in str(context.value)
        os.environ["MAL_CLIENT_ID"] = self.test_client_id
    ...
    def test_make_request_success(self, mock_requests_get):  # Removed mock_load_dotenv
        ...
        mock_requests_get.assert_called_once_with(
            expected_url,
            headers={
                "X-MAL-CLIENT-ID": self.test_client_id,
    ...
    def test_transform_data(self):  # Removed mock_load_dotenv
        ...
        assert "seasonal" in transformed_output
        assert "popular" in transformed_output
        assert "favorite" in transformed_output

        assert len(transformed_output["seasonal"]) == 1
        assert isinstance(transformed_output["seasonal"][0], AnimeItem)
        assert transformed_output["seasonal"][0].title == self.sample_anime_node_1["title"]
        assert transformed_output["seasonal"][0].id == self.sample_anime_node_1["id"]

        assert len(transformed_output["popular"]) == 1
        assert isinstance(transformed_output["popular"][0], AnimeItem)
        assert transformed_output["popular"][0].title == self.sample_anime_node_2["title"]

        assert len(transformed_output["favorite"]) == 0
    ...
    def test_transform_data_malformed_node(self):  # Removed mock_load_dotenv
        ...
        malformed_seasonal_response = {
            "data": [{"node": self.sample_anime_node_1}, {"node": None}, {}],
            "paging": {},
        }
    ...
    def test_extract_transform_load_flow(
        ...
        expected_calls = [
            call(
                f"/anime/season/{current_year}/{current_season}",
                {"limit": 100, "fields": FIELDS_TO_REQUEST},
            ),
            call(
                "/anime/ranking",
                {
            ...
            call(
                "/anime/ranking",
                {"ranking_type": "favorite", "limit": 100, "fields": FIELDS_TO_REQUEST},
            ),
        ]
        ...
        with open(seasonal_file) as f:
            data = json.load(f)
        assert len(data) == 1
        assert data[0]["title"] == self.sample_anime_node_1["title"]
        assert data[0]["id"] == self.sample_anime_node_1["id"]
        assert data[0]["mean"] == self.sample_anime_node_1["mean"]
        assert "main_picture" in data[0]
        assert data[0]["main_picture"]["large"] == self.sample_anime_node_1["main_picture"]["large"]

    @patch.object(MalETL, "_make_request")
    def test_run_with_one_api_failure(
        self, mock_make_request
    ):  # Removed mock_load_dotenv
