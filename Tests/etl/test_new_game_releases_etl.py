...
class TestNewGameReleasesETL(unittest.TestCase):
    ...
    def test_process_games_data_valid(self):
        ...
        self.assertEqual(processed[0]['rawg_link'], f"https://rawg.io/games/{self.sample_game_high_score['slug']}")
    ...
    def test_process_games_data_missing_optional_fields(self):
        ...
        self.assertEqual(processed[0]['name'], "Game With Missing Info")
        self.assertEqual(processed[0]['platforms'], [])
        self.assertEqual(processed[0]['genres'], [])
        # Check fallback for description_raw
        self.assertEqual(processed[0]['description_raw'], f"https://rawg.io/games/{game_missing_some_fields['slug']}")
        self.assertEqual(processed[0]['rawg_link'], f"https://rawg.io/games/{game_missing_some_fields['slug']}")

        # Test game_no_platforms_or_genres
        self.assertEqual(processed[1]['name'], "Game No Platforms/Genres")
        self.assertEqual(processed[1]['platforms'], []) # Should default to empty list
        self.assertEqual(processed[1]['genres'], [])    # Should default to empty list
        self.assertEqual(processed[1]['description_raw'], "A description.")
    ...
    def test_get_new_releases_flow_success(self, mock_get_root, mock_fetch, mock_to_csv, mock_to_json, mock_ensure_dirs, mock_getenv):
        # Mock configurations
        ...
        self.assertTrue('ordering' in kwargs_page1['params'])
        self.assertTrue('metacritic' in kwargs_page1['params'])
    ...
    def test_get_new_releases_api_key_warning(self, mock_get_root, mock_ensure_dirs, mock_to_csv, mock_to_json, mock_fetch, mock_logger_warning, mock_getenv):
        ...
        if mock_fetch.call_args_list: # If fetch_games was called
             ...
             self.assertEqual(kwargs_page1['params']['key'], "YOUR_RAWG_API_KEY")
    ...
    @patch('src.etl.games.games_get_new_releases.get_project_root')
    def test_get_new_releases_fetch_fails(self, mock_get_root, mock_logger, mock_ensure_dirs, mock_to_csv, mock_to_json, mock_fetch, mock_getenv):
    ...
    @patch('src.etl.games.games_get_new_releases.logger')
    @patch('src.etl.games.games_get_new_releases.get_project_root')
    def test_get_new_releases_no_games_processed(self, mock_get_root, mock_logger, mock_ensure_dirs, mock_to_csv, mock_to_json, mock_fetch, mock_getenv):
        mock_getenv.return_value = self.mock_api_key
