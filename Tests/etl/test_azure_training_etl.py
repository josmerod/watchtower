...
from src.etl.courses.azure_training_etl import (
    fetch_azure_training_feed,
    save_azure_training_entries,
)
...
class TestAzureTrainingETL(unittest.TestCase):
    ...
    def test_fetch_azure_training_feed(self, mock_feedparser_parse):
        ...
        assert len(entries) == len(MOCK_AZURE_FEED_ENTRIES_RAW)

        first_entry_data = entries[0]
        assert first_entry_data["title"] == MOCK_AZURE_FEED_ENTRIES_RAW[0]["title"]
        assert first_entry_data["link"] == MOCK_AZURE_FEED_ENTRIES_RAW[0]["link"]
        assert first_entry_data["summary"] == MOCK_AZURE_FEED_ENTRIES_RAW[0]["summary"]
        assert first_entry_data["source"] == "azure_microsoft_learn_blog"
        self.assertListEqual(
            first_entry_data["categories"], ["Azure AI", "Workshop", "Microsoft Learn"]
        )

        dt_obj_gmt = datetime(2024, 7, 26, 11, 0, 0, tzinfo=timezone.utc)
        assert first_entry_data["published"] == dt_obj_gmt.isoformat()

        dt_obj_est_expected = datetime(
            2024, 7, 24, 13, 0, 0, tzinfo=timezone.utc
        )  # 8 AM EST is 1 PM UTC (assuming EST is UTC-5 during standard time)
        assert entries[2]["published"] == dt_obj_est_expected.isoformat()
        assert entries[2]["categories"] == []
