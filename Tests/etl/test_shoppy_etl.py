...
def test_shoppy_scraper_initialization():
    ...
    try:
        ...
        assert (
            scraper.base_url == "https://test.shoppy.gg/"
...
def test_shoppy_scraper_parse_product_data_mock():
    ...
    try:
        ...
        assert (
            parsed_data["name"] == f"Placeholder Product Name for {product_id}"
...
def test_run_shoppy_etl_mock_flow(MockShoppyScraper, mock_file_open, mock_os_makedirs):
    ...
    try:
        ...
        assert any(
            raw_data_path_part in str(path) for path in write_calls_args
