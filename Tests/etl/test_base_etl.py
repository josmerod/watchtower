...
from src.etl.base import (
    DataFrameETL as ActualDataFrameETL,
)
from src.etl.base import (
    SimpleETL as ActualSimpleETL,
)
...
def test_etl_run_extract_error():
    ...
    with pytest.raises(
        ETLError,
        match="ETL process 'extract_error_etl' failed: Simulated extraction error",
    ):
        etl.run()
    ...
    )  # error_count should be incremented by _retry_operation
...
def test_etl_run_transform_error():
    ...
    with pytest.raises(
        ETLError,
        match="ETL process 'transform_error_etl' failed: Simulated transform error for item 1",
    ):
        etl.run()
    assert etl.extract_called is True
...
def test_etl_run_load_error():
    ...
    etl.load_error = True
    with pytest.raises(
        ETLError, match="ETL process 'load_error_etl' failed: Simulated load error"
    ):
        etl.run()
    assert etl.extract_called is True
    assert etl.transform_called is True
...
def test_base_etl_abstract_methods_instantiation():
    with pytest.raises(
        TypeError,
        match="Can't instantiate abstract class BaseETL with abstract methods extract, load, transform",
    ):
    ...
    with pytest.raises(
        TypeError,
        match="Can't instantiate abstract class IncompleteETL with abstract method load",
    ):
...
def test_base_etl_abstract_methods_not_implemented():
    ...
    with pytest.raises(
        TypeError,
        match="Can't instantiate abstract class PartiallyImplementedETL with abstract methods extract, load, transform",
    ):
...
@patch(
    "time.sleep", side_effect=InterruptedError("Simulated interruption during sleep")
)
def test_etl_run_interrupted(mock_sleep, simple_etl):
    ...
    with pytest.raises(
        ETLError,
        match="ETL process 'test_etl' failed: Simulated interruption during sleep",
    ):
        simple_etl.run()
...
def test_etl_log_summary_on_success(simple_etl, caplog):
    ...
    assert (
        f"ETL process completed successfully. Success rate: {simple_etl.metrics.success_rate:.1f}%"
        in log_output
    )
...
def test_etl_log_summary_on_failure(caplog):
    ...
    assert (
        "ETL process 'fail_summary_etl' failed" in log_output_fail
...
def test_actual_simpleetl_save_as_json(mock_json_dump, mock_open):
    ...
    output_dir_for_actual_etl = (
        Path(settings.project_root) / "data" / actual_simple_etl.name / "output"
    )
...
def test_actual_dataframeetl_save_as_csv(mock_to_csv):
    try:
        ...
        raise ImportError("pandas is required for this test")
...
def test_dataframe_etl_init_pandas():
    try:
        ...
        class TestDF(ActualDataFrameETL[dict, SimpleTestModel]):
            ...
            def transform(self, data):
                return []  # Add type hint if InputType is not Any
...
def test_ensure_directories_called_on_init(mock_mkdir):
    ...
    settings = get_settings()
    Path(settings.project_root) / "data" / etl.name
...
def test_simpleetl_load_json_io_error(mock_get_logger, mock_open):
    ...
    for call_args_item in mock_etl_logger.error.call_args_list:
        if "Disk full" in str(call_args_item[0][0]) and call_args_item[1].get(
            "exc_info"
        ):
            logged_error = True
            break
...
def test_dataframeetl_save_as_csv_io_error(mock_get_logger, mock_to_csv):
    try:
        ...
        class TestDFETL(ActualDataFrameETL[dict, SimpleTestModel]):
            ...
            def transform_dataframe(self, df: pd.DataFrame) -> list[SimpleTestModel]:
                return [
                    SimpleTestModel(data=str(row["col1"])) for _, row in df.iterrows()
                ]

            def extract(self):
                return self.extract_to_dataframe().to_dict(orient="records")

            def transform(self, data):
                return self.transform_dataframe(pd.DataFrame(data))
        ...
        with pytest.raises(IOError, match="Permission denied"):
            etl_instance.save_as_csv(data_to_save_list_of_dicts, "test_csv_fail")
...
def test_dataframe_etl_init_pandas_import_error(mock_import_module):
    ...
    if "pandas" in sys.modules:
        del sys.modules[
            "pandas"
        ]  # Temporarily remove pandas if it was imported by other tests

    # Mock import_module to raise ImportError for pandas
    ...
    if original_import_module:
        ...
        "pandas" in sys.modules and mock_import_module.side_effect
    ):  # If mock caused it to be removed by side effect
        del sys.modules[
            "pandas"
        ]  # Clean up if our mock removed it and it wasn't there before
...
def test_etl_run_with_watchtower_error():
    ...
    with pytest.raises(
        ETLError,
        match="ETL process 'wt_error_etl' failed: Simulated WatchtowerError during extract",
    ):
        etl.run()
    metrics = etl.metrics
    assert metrics.error_count > 0
...
def test_etl_run_with_generic_error():
    ...
    with pytest.raises(
        ETLError,
        match="ETL process 'generic_error_etl' failed: A generic value error in transform",
    ):
        etl.run()
    metrics = etl.metrics
    assert metrics.error_count > 0
...
def test_log_final_status_on_failure_logs_error_details(mock_get_logger, caplog):
    ...
    for call_args_item in mock_etl_logger.error.call_args_list:
        if "Simulated load error" in str(call_args_item[0][0]) and call_args_item[
            1
        ].get("exc_info"):
            error_logged_by_handle_exception = True
            break
    assert error_logged_by_handle_exception, (
        "Original error ('Simulated load error') not found in logger.error calls from handle_exception"
    )
...
def test_etl_run_with_empty_extraction():
    ...
    assert metrics.error_count == 0
    assert (
        metrics_result.is_successful is False
    )  # is_successful requires records_loaded > 0
...
def test_ensure_data_path_permission_error(mock_mkdir):
    get_settings()
    with patch(
        "pathlib.Path.mkdir", side_effect=PermissionError("Cannot create dir for test")
    ) as mock_failing_mkdir:
        with pytest.raises(
            PermissionError, match="Cannot create dir for test"
        ) as exc_info:
...
def test_etl_run_when_not_pending_base_behavior(simple_etl):
    """Test ETL run behavior when not pending."""
    pass
...
def test_retry_operation_all_retries_fail(simple_etl):
    ...
    with pytest.raises(ETLError, match="persistent failure"):
        simple_etl._retry_operation("test_op", mock_op)
...
def test_checkpointing_load_and_save(
    mock_load_checkpoint, mock_save_checkpoint, simple_etl
):
...
def test_process_in_batches(simple_etl):
    ...
    mock_process_func = MagicMock(
        side_effect=lambda batch: [item["id"] * 2 for item in batch]
    )
    ...
    mock_process_func_with_error = MagicMock(
        side_effect=[[0, 2], ETLError("batch error"), [8]]
    )  # Error on 2nd batch

    with pytest.raises(ETLError, match="batch error"):  # Expect error to propagate
        simple_etl._process_in_batches(range(10), 3, mock_process_func_with_error)
...
def test_run_extract_has_data_transform_empty(simple_etl):
    ...
    assert (
        simple_etl.load_called is False
    )  # Load should not be called if transform is empty as per updated run()
...
def test_dataframe_etl_abstract_instantiation():
    ...
    with pytest.raises(
        TypeError,
        match="Can't instantiate abstract class IncompleteDFETL with abstract methods extract_to_dataframe, transform_dataframe",
    ):
...
def test_dataframe_etl_run_methods_if_pandas_becomes_unavailable(
    mock_import_module_std,
):
    try:
        import pandas as pd
    ...
    class ConcreteDFETL(ActualDataFrameETL[list[dict], SimpleTestModel]):
        ...
        def extract_to_dataframe(self) -> pd.DataFrame:
            if not hasattr(self, "pd"):
                raise ImportError("Pandas gone in extract_to_dataframe")
            return self.pd.DataFrame(self.extract_data_payload)

        def transform_dataframe(self, df: pd.DataFrame) -> list[SimpleTestModel]:
            if not hasattr(self, "pd"):
                raise ImportError("Pandas gone in transform_dataframe")
    ...
    if hasattr(etl, "pd"):
        del etl.pd
    with pytest.raises(ETLError, match="Pandas is not available"):
        etl.load([])  # Try to load empty list
    ...
    ):  # If it got re-added by some magic, ensure it's gone if it wasn't there
        del sys.modules["pandas"]
