...
class SimpleTestETL(BaseETL[dict, SimpleTestModel]):
    def __init__(self, name="test_etl", description=None, batch_size=None, enable_checkpointing=True, max_retries=3, retry_delay=5):
        super().__init__(name=name, description=description, batch_size=batch_size, enable_checkpointing=enable_checkpointing, max_retries=max_retries, retry_delay=retry_delay)
...
def test_dataframeetl_save_as_csv_io_error(mock_get_logger, mock_to_csv):
    try:
        ...
        class TestDFETL(ActualDataFrameETL[dict, SimpleTestModel]):
            ...
            def transform_dataframe(self, df: pd.DataFrame) -> list[SimpleTestModel]:
                return [SimpleTestModel(data=str(row["col1"])) for _, row in df.iterrows()]
        ...
        with pytest.raises(IOError, match="Permission denied"):
            etl_instance.save_as_csv(data_to_save_list_of_dicts, "test.csv")
...
def test_checkpointing_load_and_save(mock_load_checkpoint, mock_save_checkpoint, simple_etl):
    ...
    if simple_etl.enable_checkpointing:
        ...
        simple_etl.current_checkpoint = ETLCheckpoint(etl_name=simple_etl.name, checkpoint_id="test_checkpoint_1", timestamp=datetime.utcnow(), processed_count=1, last_processed_id="item1")
