...
class TestGamesTabComponent(unittest.TestCase):
    ...
    def test_games_data_loading(self):
        ...
        with patch(
            "src.web.dashboard.components.games_tab.DATA_BASE_PATH", f"{self.data_dir}/"
        ):
            with patch(
                "src.web.dashboard.components.games_tab.os.path.dirname"
            ) as mock_dirname:
    ...
    def test_missing_files_handling(self):
        ...
        with patch(
            "src.web.dashboard.components.games_tab.DATA_BASE_PATH",
            "/nonexistent/path/",
        ):
            with patch(
                "src.web.dashboard.components.games_tab.os.path.dirname"
    ...
    def test_pandas_warning_suppression(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if non_empty_dfs:
                ...
                self.assertEqual(
                    len(future_warnings), 0, "FutureWarning should be avoided"
                )
    ...
    def test_date_parsing_function(self):
        ...
        for date_str, source_format in test_cases:
            with self.subTest(date_str=date_str):
                ...
                self.assertTrue(
                    result is None or hasattr(result, "year"),
                ...
                if filename.endswith(".json"):
                    try:
                        ...
                        self.assertIsInstance(
                            data, (list, dict), f"Invalid JSON structure in {filename}"
                        )
