...
def test_load_data_file_not_found(
    mock_st_error, adhd_component_instance, caplog
):  # caplog can capture internal logs if any
...
def test_load_data_generic_exception(mock_st_error, adhd_component_instance, tmp_path):
    ...
    ) as m_open:  # Ensure this mock_open is from unittest.mock
...
@patch.object(
    ADHDPapersComponent, "_load_data", return_value=[]
)  # Mock _load_data directly on the class/instance
def test_render_no_data_shows_warning(
    mock_load_data, mock_st_warning, mock_st_title, adhd_component_instance
):
    ...
    mock_st_warning.assert_called_once_with(
        "No ADHD papers found. Please run the ADHDPublicationETL first."
    )
...
@patch.object(
    ADHDPapersComponent, "render_paper_card"
)  # Mock the card rendering method
def test_render_with_data_displays_papers(
    mock_render_card,
    mock_st_text_input,
    mock_st_markdown,
    mock_st_title,
    adhd_component_instance,
):
    ...
    with patch.object(
        adhd_component_instance, "_load_data", return_value=sample_papers
    ) as mock_load_data:
        adhd_component_instance.render()
...
def test_render_search_filters_papers(
    mock_render_card,
    mock_st_info,
    mock_st_text_input,
    mock_st_markdown,
...
def test_render_paper_card_displays_details(
    mock_st_write, mock_st_markdown, mock_st_expander, adhd_component_instance
):
    """Tests that render_paper_card displays all details of a paper."""
    paper = {"abstract": "This is a very detailed test abstract for the paper card."}
    ...
    mock_st_markdown.assert_any_call(
...
@patch("streamlit.markdown")
@patch("streamlit.write")
def test_render_paper_card_handles_missing_optional_fields(
    mock_st_write, mock_st_markdown, mock_st_expander, adhd_component_instance
):
    ...
    mock_st_markdown.assert_any_call(
        f"**Authors:** {', '.join(paper_minimal['authors'])}"
    )
    ...
    doi_markdown_call = (
        f"**DOI:** [{paper_minimal['doi']}]"  # This would be problematic if doi is None
    )

    called_markdowns = [call[0][0] for call in mock_st_markdown.call_args_list]
    assert not any(
        f"**DOI:** [{None}]" in called_md for called_md in called_markdowns
    )  # Ensure DOI for None isn't made
    assert not any(
        f"**DOI:** [" in called_md and paper_minimal["doi"] is None
        for called_md in called_markdowns
    )
