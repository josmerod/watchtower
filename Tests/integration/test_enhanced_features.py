...
try:
    ...
    try:
        raise WatchtowerError(
            message="Test error", error_code="TEST_ERROR", context={"test": "context"}
        )
    ...
    try:
        raise WatcherTimeoutError(
            message="Test timeout", url="https://example.com", timeout=30
        )
...
try:
    ...
    print(
        f"[PASS] Directory info: writable={info.is_writable}, files={info.file_count}"
    )
...
try:
    ...
    print(
        f"[PASS] ETL metrics: extracted={metrics.items_extracted}, loaded={metrics.items_loaded}"
    )
...
try:
    ...
    print(
        f"[PASS] Initial status: checks={status['check_count']}, errors={status['error_count']}"
    )
