...
try:
    ...
    try:
        raise WatchtowerError(message="Test error", error_code="TEST_ERROR", context={"test": "context"})
    except WatchtowerError as e:
        print(f"[PASS] Basic exception handling: {e.error_code}")

    # Test specialized exceptions
    try:
        raise WatcherTimeoutError(message="Test timeout", url="https://example.com", timeout=30)
...
try:
    ...
    article = NewsArticleModel(title="Test Article", url="https://example.com/article", content="This is a test article content.", source_name="Test Source")
...
try:
    ...
    class TestETL(SimpleETL[dict, TestDataModel]):
        def extract(self) -> list[dict]:
            return [{"name": "item1", "value": 10}, {"name": "item2", "value": 20}, {"name": "item3", "value": 30}]
