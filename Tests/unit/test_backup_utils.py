...
class TestBackupManager(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_settings.data_dir = (
            "data"  # Relative path as BackupManager might expect
        )
        ...
        self.patch_google_auth_constructor = patch(
            "src.utils.backup_utils.GoogleAuth", return_value=self.mock_google_auth
        )
        # Patch the GoogleDrive constructor to return our mock instance
        self.patch_google_drive_constructor = patch(
            "src.utils.backup_utils.GoogleDrive",
            return_value=self.mock_google_drive_instance,
        )

        self.mock_gauth_constructor_started = self.patch_google_auth_constructor.start()
        self.mock_gdrive_constructor_started = (
            self.patch_google_drive_constructor.start()
        )
        ...
        self.mock_gdrive_file.__getitem__.side_effect = lambda key: getattr(
            self.mock_gdrive_file, key, None
        )
    ...
    def test_init_success_with_credentials(self):
        ...
        self.mock_gdrive_constructor_started.assert_called_once_with(
            self.mock_google_auth
        )
        assert manager.drive == self.mock_google_drive_instance
    ...
    def test_init_missing_credentials_file(self):
        ...
        Path(
            self.mock_settings.google_drive.credentials_file
        ).unlink()  # Remove the fake file
        with pytest.raises(FileNotFoundError, match="client_secrets file not found"):
            BackupManager(self.mock_settings)
    ...
    def test_compress_folders_success(self):
        ...
        manager = BackupManager(
            self.mock_settings
        )  # Init to setup drive mocks, not strictly needed for this test part
        ...
        assert compressed_path.name == f"{archive_base_name}.zip"
        ...
        with zipfile.ZipFile(compressed_path, "r") as zf:
            ...
            assert "data/d1.txt" in zip_contents
            assert "data/d2.txt" in zip_contents
            assert "logs/l1.log" in zip_contents
    ...
    def test_compress_folders_empty_or_missing(self):
        ...
        with self.assertLogs(
            "watchtower.src.utils.backup_utils", level="WARNING"
        ) as log_watcher:
            compressed_path = manager.compress_folders(
                folders_to_backup, archive_base_name
            )

        assert any(
            "Folder" in msg and "non_existent_folder does not exist" in msg
        ...
        with zipfile.ZipFile(compressed_path, "r") as zf:
            ...
            assert (
                len(zf.namelist()) == 0
            )  # Or check if it contains the empty_data directory entry
    ...
    def test_upload_single_file(self):
        ...
        self.mock_google_drive_instance.ListFile.return_value.GetList.return_value = [
            mock_existing_gdrive_file
        ]
        ...
        assert uploaded_file_id == "new_id"

    @patch(
        "src.utils.backup_utils.datetime"
    ...
    def test_prepare_latest_in_drive_renames_existing(self, mock_datetime):
        ...
        mock_datetime.datetime.strptime.return_value = (
            fixed_datetime  # For strptime fallback
        )
        ...
        def create_file_side_effect(options):
            ...
            if file_id_option == "id_old_latest":
                ...
                if (
                    "fields" in options and options["fields"] == "modifiedDate"
                ):  # Call from _get_drive_file_modified_date
                    file_mock.FetchMetadata = MagicMock()
                    # __getitem__ is used by PyDrive2 to access metadata like file_mock['modifiedDate']
                    file_mock.__getitem__.side_effect = (
                        lambda k: "2023-01-01T10:00:00.000Z"
                        if k == "modifiedDate"
                        else None
                    )
        ...
        assert self.mock_google_drive_instance.ListFile.call_count == 2
        ...
        for call_args in self.mock_google_drive_instance.CreateFile.call_args_list:
            created_mock = self.mock_google_drive_instance.CreateFile.side_effect(
                call_args[0][0]
            )  # re-evaluate side_effect to get the mock
            if (
                created_mock.id == "id_old_latest"
                and hasattr(created_mock, "Upload")
                and created_mock.Upload.called
            ):
                found_rename_mock = created_mock
                break

        assert found_rename_mock is not None, (
            "Rename operation mock not found or Upload not called."
        )
        assert (
            found_rename_mock.title
            == f"{latest_filename_base}_{fixed_timestamp_str}.zip"
        )
    ...
    def test_enforce_retention_policy_deletes_oldest(self):
        ...
        files_data = [
            (
                "id1",
                f"{latest_filename_base}_20230101_100000.zip",
                datetime.datetime(2023, 1, 1, 10, 0, 0),
            ),  # Oldest
            (
                "id2",
                f"{latest_filename_base}_20230102_100000.zip",
                datetime.datetime(2023, 1, 2, 10, 0, 0),
            ),
            (
                "id3",
                f"{latest_filename_base}_20230103_100000.zip",
                datetime.datetime(2023, 1, 3, 10, 0, 0),
            ),  # Newest of these three
        ]
        ...
        for id_val, title_val, _ in files_data:
            ...
            # mf['id'] = id_val
            mf.get.side_effect = lambda k, i=id_val, t=title_val: {"id": i, "title": t}[
                k
            ]  # Allows mf['id'] or mf['title']
        ...
        manager._enforce_retention_policy(
            latest_filename_base, max_historical_copies=max_historical
        )
    ...
    def test_run_backup_process_full_flow_success(self, mock_os_remove):
        ...
        manager.compress_folders = MagicMock(
            return_value=self.project_root / "temp_archive.zip"
        )
        ...
        assert result
        manager.compress_folders.assert_called_once_with(
            folders_to_backup, "backup_data_logs_local_temp"
        )
        ...
        manager._upload_single_file.assert_called_once_with(
            self.project_root / "temp_archive.zip", "backup_data_logs_latest.zip"
        )
        manager._enforce_retention_policy.assert_called_once_with(
            "backup_data_logs", max_historical_copies=5
        )
    ...
    def test_run_backup_process_compression_fails(self, mock_os_remove):
        ...
        manager.compress_folders = MagicMock(
            side_effect=zipfile.BadZipFile("Compression failed")
        )
    ...
    def test_get_drive_file_modified_date(self):
        ...
        mock_file_meta.__getitem__.side_effect = (
            lambda key: "2023-10-27T10:30:45.123Z" if key == "modifiedDate" else None
        )
        ...
        expected_datetime = datetime.datetime(
            2023, 10, 27, 10, 30, 45, 123000, tzinfo=datetime.timezone.utc
        )
        ...
        assert (
            actual_datetime.replace(tzinfo=datetime.timezone.utc) == expected_datetime
        )
