...
class TestBackupManager(unittest.TestCase):
    ...
    def test_compress_folders_empty_or_missing(self):
        ...
        with self.assertLogs(
            f"watchtower.src.utils.backup_utils", level="WARNING"
        ) as log_watcher:
    ...
    def test_prepare_latest_in_drive_renames_existing(self, mock_datetime):
        ...
        def create_file_side_effect(options):
            ...
            if file_id_option == "id_old_latest":
                ...
                if (
                    ...
                    file_mock.__getitem__.side_effect = lambda k: (
                        "2023-01-01T10:00:00.000Z" if k == "modifiedDate" else None
                    )
        ...
        self.assertIsNotNone(
            found_rename_mock, "Rename operation mock not found or Upload not called."
        )
        self.assertEqual(
            found_rename_mock.title, f"{latest_filename_base}_{fixed_timestamp_str}.zip"
        )
    ...
    def test_get_drive_file_modified_date(self):
        ...
        mock_file_meta.__getitem__.side_effect = lambda key: (
            "2023-10-27T10:30:45.123Z" if key == "modifiedDate" else None
        )
