...
class TestShortcutsManager:
    ...
    def is_valid_shortcut(self, shortcut):
        ...
        return (
            shortcut and
            isinstance(shortcut.get('id'), str) and
            isinstance(shortcut.get('name'), str) and
            isinstance(shortcut.get('domain'), str) and
            isinstance(shortcut.get('source_filter'), dict) and
            isinstance(shortcut.get('order'), int) and
            len(shortcut.get('name', '').strip()) > 0
        )
    ...
    def get_all_shortcuts(self):
        ...
        try:
            ...
            valid_shortcuts = [
                shortcut for shortcut in data['shortcuts']
    ...
    def save_all_shortcuts(self, data):
        ...
        try:
            self.localStorage_mock.setItem(
                self.shortcuts_manager['storageKey'],
                json.dumps(data)
            )
            return True
        except Exception:
    ...
    def test_add_shortcut_success(self):
        ...
        shortcut = self.add_shortcut(
            'Test Paper',
            'arxiv',
            {'source': 'arxiv', 'title': 'Test Paper Title'}
        )
