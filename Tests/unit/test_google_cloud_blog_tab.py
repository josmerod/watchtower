...
import streamlit as st # We'll be mocking st functions
...
class TestGoogleCloudBlogTab(unittest.TestCase):
    ...
    def test_render_with_data(self, mock_st):
        ...
        # Check main header
        mock_st.header.assert_any_call("Google Cloud Blog - Training & Certification Focus")

        # Check that st.subheader and st.markdown (for links) are called for each post
        # Number of posts = 4
        self.assertEqual(mock_st.subheader.call_count, 4)
        ...
        self.assertEqual(mock_st.caption.call_count, 6)
        ...
        mock_st.caption.assert_has_calls(expected_caption_calls, any_order=False) # Order matters here relative to posts
        ...
        self.assertTrue(mock_st.markdown.call_count >= 4) # At least one per post

        # Check logger calls
        mock_logger.info.assert_any_call("Rendering Google Cloud Blog tab.")
        mock_logger.info.assert_any_call(f"Successfully rendered {len(sample_posts_data)} Google Cloud Blog posts.")
    ...
    def test_render_data_no_relevant_filters_message(self, mock_st):
        ...
        self.assertIn("Successfully loaded Google Cloud Blog posts, but no articles matching your keywords", mock_st.success.call_args[0][0])
        mock_logger.info.assert_any_call(f"Found {len(posts_data_no_keywords_match)} total Google Cloud Blog posts, but none matched the keywords: ['skill', 'skills', 'training', 'certification', 'learn', 'boost', 'course', 'courses', 'badge', 'prepare', 'exam']")
    ...
    def test_render_general_exception_handling(self, mock_st):
        ...
        self.assertIn("An error occurred in the Google Cloud Blog tab", mock_logger.error.call_args[0][0])
