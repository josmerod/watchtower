...
from src.web.fullstreamlit.components.azure_training_tab import (
    render as render_azure_training_tab,
)
...
class TestAzureTrainingTab(unittest.TestCase):
    ...
    def test_render_with_azure_data(self, mock_st):
        ...
        assert mock_st.caption.call_count == 4
        ...
        assert mock_st.markdown.call_count >= 1 + 2 + 2
        mock_st.markdown.assert_any_call(
            "Overview of Azure AI.", unsafe_allow_html=True
        )
        mock_st.markdown.assert_any_call(
            "Azure DevOps features.", unsafe_allow_html=True
        )
        mock_st.markdown.assert_any_call("---")

        mock_logger.info.assert_any_call("Rendering Azure Training tab.")
        mock_logger.info.assert_any_call(
            f"Successfully rendered {len(sample_posts_data)} Azure training posts."
        )
    ...
    def test_render_azure_error_handling(self, mock_st):
        faulty_data = [
            {"title": "Bad Post"}
        ]  # Missing other fields might cause issues or get default values
        ...
        assert "An error occurred in the Azure Training tab" in mock_logger.error.call_args[0][0]
