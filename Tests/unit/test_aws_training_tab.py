...
from src.web.fullstreamlit.components.aws_training_tab import (
    render as render_aws_training_tab,
)
...
class TestAWSTrainingTab(unittest.TestCase):
    ...
    def test_render_with_aws_data(self, mock_st):
        ...
        mock_st.header.assert_called_once_with(
            "AWS Training & Certification Blog Updates"
        )
        ...
        mock_st.markdown.assert_any_call(
            "Summary for AWS Lambda post.", unsafe_allow_html=True
        )
        ...
        mock_logger.info.assert_any_call(
            f"Successfully rendered {len(sample_posts_data)} AWS training posts."
        )
    ...
    def test_render_aws_error_handling(self, mock_st):
        ...
        assert "An error occurred in the AWS Training tab" in mock_logger.error.call_args[0][0]
