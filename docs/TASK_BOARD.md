...
| T-024 | todo | P1 | Fix latent `AlertRulesRepository.save_rule`/`delete_rule` bug | api | `AlertRulesRepository.save_rule` and `.delete_rule` in `src/web/dashboard/components/notifications_tab.py` call `self.load_rules()`, but the repository has no such method (only `NotificationsManager` does). The resulting `AttributeError` is swallowed by a bare `except Exception` → both methods always return `False`. Surfaced by `test_notifications_tab.py::test_repo_save_rule_is_broken` (marked `xfail`). Fix: use `self.get()` (the repo's cached loader) or add a `load_rules` method. |
