...
| T-047 | todo | P3 | Re-evaluate BridgeBench after V3 ships | etl | **Probe 2026-09-11: la nueva versión YA está live** (redirect a www.bridgebench.ai, shell nuevo de 549KB vs 55KB) pero sigue client-rendered: sin `<table>` estático, sin endpoints de API visibles, sin fetch hints, el único window.__ es el beacon de Cloudflare. **La vía viable = browserless** (patrón LiveBench: render + capturar DOM/interceptar XHR) — **bloqueado de facto por el mantenimiento del server** (ws://192.168.31.126:3000 caído, verificado 2026-09-11). Implementar cuando vuelva: ETL render-vía-browserless + degradación elegante al last-good. |
...
- 2026-09-11 — **T-094 cerrado** (dedup helpers: −742 líneas netas, contratos de fecha documentados) y **T-047 actualizado con probe** (la nueva versión de BridgeBench está live pero sigue client-rendered sin API estática; vía browserless bloqueada por mantenimiento). Board: 4 abiertas (T-047, T-048 ~09-16, T-096 a fuego lento, T-038 blocked).
