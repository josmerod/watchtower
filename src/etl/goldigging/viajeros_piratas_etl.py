...
class ViajerosPrivatasETL(BaseETL[TravelDealRawData, TravelDeal]):
    ...
    async def _extract_single_deal(self, element, page_num: int, position: int) -> Optional[TravelDealRawData]:
        ...
        try:
            ...
            if not time_text:
                ...
                time_match = re.search(r'hace\s+\d+\s+\w+|\d{1,2}/\d{1,2}/\d{4}', text_content, re.IGNORECASE)
                if time_match:
