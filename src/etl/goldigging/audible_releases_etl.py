...
class AudibleReleasesETL(BaseETL[dict[str, Any], dict[str, Any]]):
    ...
    def extract(self) -> list[dict[str, Any]]:
        ...
        while page <= max_limit:
            ...
            for p in products:
                ...
                all_products.append({"title": title, "link": link, "author": author, "narrator": narrator, "runtime": runtime, "published_str": date_str, "language": lang})
    ...
    def transform(self, data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in data:
            ...
            summary_parts = []
            if item.get("author"):
                summary_parts.append(f"👨‍💻 Autor: {item['author']}")
            if item.get("narrator"):
                summary_parts.append(f"🎙️ Narrador: {item['narrator']}")
            if item.get("runtime"):
                summary_parts.append(f"⏱️ {item['runtime']}")
            if item.get("language"):
                summary_parts.append(f"🌐 {item['language']}")
