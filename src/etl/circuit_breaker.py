...
class CircuitBreaker:
    ...
    def can_proceed(self) -> bool:
        ...
        logger.warning(f"Circuit breaker for {self.etl_name} is OPEN. Recovery at {self.state.recovery_time} (UTC).")
        return False
    ...
    def record_failure(self) -> None:
        ...
        if self.state.failure_count >= self.failure_threshold:
            if not self.state.is_open:
                ...
                logger.error(f"Circuit breaker for {self.etl_name} TRIPPED after {self.state.failure_count} failures.")
