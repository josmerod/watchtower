...
class TechConferenceETL(BaseETL):
        ...
        try:
            ...
            for source_name, config in self.data_sources.items():
                ...
                try:
                    ...
                    self.logger.info(
                        f"Extracted {len(filtered_events)} relevant events from {source_name}"
                    )
    ...
    def _extract_feed_events(
        self, feeds: list[str], source_type: str = "generic"
    ) -> list[dict[str, Any]]:
        ...
        for feed_url in feeds:
            try:
                ...
                try:
                    # Try to fetch the feed with a timeout and proper headers
                    headers = {
                        "User-Agent": "Watchtower-ETL/1.0 (Tech Event Aggregator)",
                    ...
                    # Try with feedparser directly as fallback
                    try:
                        feed = parse_feed(feed_url)
                    except Exception as fallback_e:
                        self.logger.warning(
                            f"Fallback also failed for {feed_url}: {fallback_e}"
                        )
                        continue
                ...
                if hasattr(feed, "status") and feed.status >= 400:
                    self.logger.warning(
                        f"Feed returned error status {feed.status}: {feed_url}"
                    )
                    continue
                ...
                successful_feeds += 1
                self.logger.info(
                    f"Successfully parsed {len(feed.entries)} entries from {feed_url}"
                )
                ...
                for entry in feed.entries[: self.max_events_per_source]:
                    try:
                        ...
                        event_data = {
                            "name": self._clean_text(entry.get("title", "")).strip(),
                            ...
                            "event_type": self._infer_event_type_from_text(
                                entry.get("title", "")
                            ),
                        ...
                        if (
                            ...
                            try:
                                event_data["start_date"] = datetime(
                                    *entry.published_parsed[:6]
                                ).isoformat()
                            except (TypeError, ValueError) as e:
                                self.logger.debug(
                                    f"Failed to parse date from entry: {e}"
                                )
                        ...
                        if event_data["name"] and len(event_data["name"]) > 3:
                            events.append(event_data)

                    except Exception as e:
                        self.logger.debug(
                            f"Failed to process entry from {feed_url}: {e}"
                        )
                        continue

            except Exception as e:
                self.logger.warning(
                    f"Failed to parse {source_type} feed {feed_url}: {e}"
                )
                continue

        self.logger.info(
            f"Successfully parsed {successful_feeds}/{len(feeds)} feeds for {source_type}, extracted {len(events)} events"
        )
        return events
    ...
    def _extract_location_from_text(self, text: str) -> dict[str, str] | None:
        ...
        for venue in self.valencia_venues:
            if venue.lower() in text_lower:
                return {"name": venue, "city": "Valencia", "country": "Spain"}
        ...
        for city_key, city_name in spanish_cities.items():
            if city_key in text_lower:
                return {"city": city_name, "country": "Spain"}
    ...
    def _extract_spanish_tech_events(
    ...
    def _extract_european_conferences(
        self, config: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _extract_tech_news_events(self, config: dict[str, Any]) -> list[dict[str, Any]]:
        """Extract tech news events from various sources.

        Args:
            config: Tech news events configuration.

        Returns:
            List of raw event data from tech news sources.
        """
        events = []

        # Extract from tech news sources
        sources = config.get("news_sources", [])
        events.extend(self._extract_feed_events(sources, "tech_news"))

        # Mock tech news events
        mock_events = [
            {
                "name": "TechCrunch Daily",
                "description": "Daily tech news roundup from TechCrunch.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {"name": "TechCrunch", "city": "Online", "country": "Global"},
                "organizer": "TechCrunch",
                "url": "https://techcrunch.com/feed/",
                "cost": 0.0,
                "is_virtual": True,
                "topics": ["tech news", "startup", "innovation"],
                "event_type": "news",
            },
            {
                "name": "The Verge Tech News",
                "description": "Daily tech news from The Verge.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {"name": "The Verge", "city": "Online", "country": "Global"},
                "organizer": "The Verge",
                "url": "https://www.theverge.com/rss/index.xml",
                "cost": 0.0,
                "is_virtual": True,
                "topics": ["tech news", "gadgets", "software"],
                "event_type": "news",
            },
            {
                "name": "Ars Technica Tech News",
                "description": "Daily tech news from Ars Technica.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {
                    "name": "Ars Technica",
                    "city": "Online",
                ...
                "organizer": "Ars Technica",
                "url": "https://arstechnica.com/rss.xml",
                "cost": 0.0,
                "is_virtual": True,
                "topics": ["tech news", "hardware", "software"],
                "event_type": "news",
            },
            {
                "name": "Wired Tech News",
                "description": "Daily tech news from Wired.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {"name": "Wired", "city": "Online", "country": "Global"},
                "organizer": "Wired",
    ...
    def _extract_open_source_events(
        self, config: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """Extract open source events from various sources.

        Args:
            config: Open source events configuration.

        Returns:
            List of raw event data from open source communities.
        """
        events = []

        # Extract from open source feeds
        feeds = config.get("oss_feeds", [])
        events.extend(self._extract_feed_events(feeds, "open_source"))

        # Mock open source events
        mock_events = [
            {
    ...
    def _extract_developer_platforms(
        self, config: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """Extract developer platforms from various sources.

        Args:
            config: Developer platforms configuration.

        Returns:
            List of raw event data from developer platforms.
        """
        events = []

        # Extract from platform feeds
        feeds = config.get("platform_feeds", [])
        events.extend(self._extract_feed_events(feeds, "developer_platforms"))

        # Mock developer platforms
        mock_events = [
            {
                "name": "Stack Overflow Developer Survey",
                "description": "Annual survey of developer opinions and trends.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {
                    "name": "Stack Overflow",
                    "city": "Online",
                ...
                "cost": 0.0,
                "is_virtual": True,
                "topics": ["developer opinions", "trends", "survey"],
                "event_type": "survey",
            },
            {
                "name": "GitHub Engineering Blog",
                "description": "Insights and news from GitHub's engineering team.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {"name": "GitHub", "city": "Online", "country": "Global"},
                "organizer": "GitHub",
            ...
            {
                "name": "GitLab Developer Survey",
                "description": "Annual survey of developer opinions and trends from GitLab.",
                "start_date": (datetime.utcnow() + timedelta(days=1)).isoformat(),
                "venue": {"name": "GitLab", "city": "Online", "country": "Global"},
                "organizer": "GitLab",
                "url": "https://about.gitlab.com/atom.xml",
                "cost": 0.0,
                "is_virtual": True,
                "topics": ["gitlab", "git", "open source", "development", "AI coding"],
                "event_type": "survey",
            },
        ]
    ...
    def _extract_valencia_local_sources(
        self, config: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """Extract Valencia-specific events from local sources.

        Args:
            config: Valencia local sources configuration.

        Returns:
            List of raw event data from Valencia local sources.
        """
        events = []

        # Extract from local feeds
        feeds = config.get("local_feeds", [])
        events.extend(self._extract_feed_events(feeds, "valencia_local"))
    ...
    def _filter_events_by_location(
        ...
        for event in events:
            ...
            self.logger.debug(
                f"Skipping event outside target area: {event.get('name', 'Unknown')} in {location_text}"
            )
    ...
    def _determine_event_type(self, event_data: dict[str, Any]) -> EventType:
        ...
        if any(
            keyword in name_and_desc
            for keyword in ["workshop", "taller", "hands-on", "training", "formación"]
        ):
            return EventType.WORKSHOP
        elif any(
            keyword in name_and_desc
            for keyword in ["meetup", "encuentro", "gathering", "meet-up", "quedada"]
        ):
            ...
            keyword in name_and_desc
            for keyword in ["hackathon", "hack day", "coding competition", "hackaton"]
        ):
            return EventType.HACKATHON
        elif any(
            keyword in name_and_desc
            for keyword in ["bootcamp", "intensive", "immersive", "intensivo"]
        ):
    ...
    def _transform_single_event(
        self, event_data: dict[str, Any], index: int
    ) -> TechEventModel | None:
        ...
        try:
            ...
            speaker_influence_score = self._analyze_speaker_influence(
                event_data.get("speakers", [])
            )
    ...
    def _determine_event_format(self, event_data: dict[str, Any]) -> EventFormat:
        ...
        elif (is_virtual and has_venue) or "hybrid" in name_and_desc:
    ...
    def _create_venue_model(
        self, venue_data: dict[str, Any] | None
    ) -> VenueModel | None:
    ...
    def _calculate_quality_score(
        self, speaker_influence: float, relevance: float, networking: float
    ) -> float:
        ...
        quality_score = speaker_influence * 0.4 + relevance * 0.4 + networking * 0.2
    ...
    def _classify_event_categories(self, event_data: dict[str, Any]) -> list[str]:
        ...
        if any(
            keyword in text_content
            for keyword in ["mobile", "ios", "android", "flutter", "react native"]
        ):
            categories.append("Mobile Development")

        if any(
            keyword in text_content
            for keyword in ["data science", "analytics", "big data", "data"]
        ):
        ...
        if any(
            keyword in text_content
            for keyword in ["cloud", "aws", "azure", "kubernetes", "docker"]
        ):
            categories.append("Cloud Computing")

        if any(
            keyword in text_content
            for keyword in ["security", "cybersecurity", "infosec"]
        ):
            categories.append("Security")

        if any(
            keyword in text_content
            for keyword in ["devops", "ci/cd", "automation", "deployment"]
        ):
    ...
    def generate_event_recommendations(
        ...
        events: list[TechEventModel] | None = None,
    ) -> list[dict[str, Any]]:
        ...
        for event in events:
            try:
                ...
                self.logger.warning(
                    f"Failed to generate recommendation for event {event.name}: {e}"
                )
                continue
    ...
    def _calculate_location_convenience(
        self, event_location: str, user_location: str
    ) -> float:
    ...
    def _extract_virtual_events(self, config: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # Extract from platform feeds
        platform_feeds = config.get("platform_feeds", [])
        events.extend(self._extract_feed_events(platform_feeds, "virtual_platforms"))

        # Extract from tech community feeds
        tech_feeds = config.get("tech_community_feeds", [])
        events.extend(self._extract_feed_events(tech_feeds, "tech_community"))
