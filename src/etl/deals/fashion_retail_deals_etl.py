...
class FashionRetailDealsETL(BaseETL):
    ...
    def _get_curated_fashion_retail_deals(self) -> List[Dict[str, Any]]:
        ...
        curated = [
            ...
            },
            {
                ...
                'authenticity': 'guaranteed',
                'membership_required': False,
                'tags': ['kohls cash', 'rewards', 'stacking discounts', 'department store'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            },
            {
                ...
                'authenticity': 'guaranteed',
                'membership_required': False,
                'tags': ['off-price', 'designer finds', 'treasure hunt', 'daily arrivals'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            },
    ...
    def _determine_shopping_tier(self, deal: Dict[str, Any]) -> str:
        ...
        if brand_tier == 'luxury' and savings > 75:
            return 'luxury_deal'
        elif any(name in platform for name in ['nordstrom', 'rue la la']) and brand_tier == 'luxury':
            return 'luxury_deal'
