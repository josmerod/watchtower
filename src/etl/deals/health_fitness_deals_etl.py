...
class HealthFitnessDealsETL(BaseETL):
    ...
    def _get_curated_health_fitness_deals(self) -> List[Dict[str, Any]]:
        ...
        curated = [
            ...
            },
            {
                ...
                'tags': ['home workouts', 'structured programs', 'free trial', 'variety'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            },
            ...
            },
            {
                ...
                'health_focus': 'nutritional_supplements',
                'experience_level': 'all_levels',
                'tags': ['supplements', 'natural products', 'first order', 'free shipping'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            }
    ...
    def _determine_accessibility_level(self, deal: Dict[str, Any]) -> str:
        ...
        if (any(keyword in location_type for keyword in ['home', 'mobile', 'anywhere']) and 
            ('none' in equipment or 'bodyweight' in equipment) and 
            not commitment and 'none' in fees):
            return 'highly_accessible'
        
        # Very accessible indicators
        if (any(keyword in location_type for keyword in ['home', 'mobile']) and 
            'minimal' in equipment and not commitment):
