...
class SoftwareDealsETL(BaseETL):
    ...
    def _get_curated_software_deals(self) -> List[Dict[str, Any]]:
        ...
        curated = [
            ...
            },
            {
                ...
                'trial_period_days': 0,
                'support_level': 'community',
                'tags': ['gimp', 'image editing', 'open source', 'photoshop alternative'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            },
            ...
            },
            {
                ...
                'trial_period_days': 0,
                'support_level': 'basic',
                'tags': ['slack', 'team communication', 'collaboration', 'integrations'],
                'created_date': datetime.now(timezone.utc).isoformat(),
                'fetched_at': datetime.now(timezone.utc).isoformat(),
                'source': 'Curated'
            },
    ...
    def _determine_software_tier(self, deal: Dict[str, Any]) -> str:
        ...
        if savings > 1000 and tools_count > 20:
            return 'enterprise'
        elif any(name in platform for name in ['microsoft', 'google']) and savings > 500:
            return 'enterprise'
        
        # Professional tier indicators
        if license_type == 'open_source' and savings > 500:
            return 'professional'
        elif any(name in platform for name in ['jetbrains', 'github']) and savings > 200:
            return 'professional'
        elif tools_count > 15:
            return 'professional'
