...
class MusicDealsETL(BaseETL):
    ...
    def _determine_quality_rating(self, deal: Dict[str, Any]) -> str:
        ...
        if drm_free and any(platform_name in platform for platform_name in [
            'bandcamp', 'archive.org', 'free music archive'
        ]):
            return 'high'
        
        # Good quality indicators
        if any(platform_name in platform for platform_name in [
            'spotify', 'youtube music', 'humble'
        ]):
            return 'good'
