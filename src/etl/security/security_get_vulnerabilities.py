...
def fetch_cve_vulnerabilities(session: requests.Session, days_back: int = 7, max_results: int = 100) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for item in cve_items:
            try:
                ...
                technology_stack = determine_technology_stack(affected_packages, description)
                ...
                vulnerability = {
                    ...
                    "cwe_ids": cwe_ids,
                    "exploit_available": check_exploit_available(description, reference_urls),
                    "patch_available": check_patch_available(description, reference_urls),
...
def fetch_github_security_advisories(session: requests.Session, max_results: int = 50) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for advisory in advisories:
            try:
                ...
                # Determine technology stack
                technology_stack = determine_technology_stack(affected_packages, advisory.get('description', ''))
    ...
    try:
        ...
        for advisory in sample_advisories:
            try:
                ...
                severity_score = severity_mapping.get(advisory.get('severity', 'low'), 2.5)
...
def calculate_severity_score(cvss_score: Optional[float], cwe_ids: List[str], 
                           affected_packages_count: int, description: str) -> float:
    """
    ...
    high_impact_keywords = [
        ...
        'authentication bypass', 'critical', 'zero-day'
    ]
...
def determine_technology_stack(affected_packages: List[str], description: str) -> List[str]:
    """
...
def check_exploit_available(description: str, references: List[str]) -> bool:
    ...
    exploit_indicators = [
        'exploit', 'poc', 'proof of concept', 'exploit-db', 'metasploit',
        'actively exploited', 'exploit available', 'public exploit'
    ]
...
def check_patch_available(description: str, references: List[str]) -> bool:
    ...
    patch_indicators = [
        'patch', 'fix', 'update', 'fixed in', 'resolved in', 'patched in',
        'upgrade to', 'version', 'release'
    ]
...
def process_vulnerabilities(vulnerabilities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if published_date_str:
                try:
                    ...
                    days_since_published = (current_time.replace(tzinfo=published_date.tzinfo) - published_date).days
                except:
            ...
            enriched_vuln = {
                **vuln,
                ...
                'needs_urgent_attention': (
                    severity_score >= 9.0 and 
                    vuln.get('exploit_available', False) and 
                    not vuln.get('patch_available', False) and
    ...
    logger.info(f"Successfully processed {len(processed_vulnerabilities)} vulnerabilities")
...
def save_data(data: List[Dict[str, Any]], output_dir: str) -> Dict[str, str]:
    ...
    try:
        ...
        timestamped_json = os.path.join(output_dir, f"security_vulnerabilities_{timestamp}.json")
...
def main():
    ...
    try:
        ...
        critical_vulns = sum(1 for v in processed_vulnerabilities if v.get('risk_level') == 'critical')
        high_vulns = sum(1 for v in processed_vulnerabilities if v.get('risk_level') == 'high')
        with_exploits = sum(1 for v in processed_vulnerabilities if v.get('exploit_available'))
        with_patches = sum(1 for v in processed_vulnerabilities if v.get('patch_available'))
        ...
        if file_paths:
            logger.info(f"Data files created:")
