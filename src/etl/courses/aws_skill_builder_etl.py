...
class AwsSkillBuilderETL(BaseETL):
    ...
    def transform(self, data: str) -> list[dict]:
        ...
        for a in soup.find_all("a", class_="course-name"):
            ...
            extracted_courses.append({"title": title, "url": url, "duration": duration, "level": level, "is_free": is_free, "description": description})
        ...
        for ec in extracted_courses:
            ...
            model = AwsSkillBuilderModel(title=ec["title"], url=ec["url"], duration=ec["duration"], level=ec["level"], is_free=ec["is_free"], description=ec["description"], first_detected_at=detected_at)
