...
def fetch_azure_training_feed() -> List[Dict[str, Any]]: # Renamed function
    ...
    for entry in feed.entries:
        ...
        if not categories: # Log if categories are empty, even if tags attribute exists but has no actual terms
            logger.info(f"No valid categories extracted for entry '{entry.get('title')}'. Tags attribute might be empty or malformed.")
...
def save_azure_training_entries(entries: List[Dict[str, Any]]) -> None: # Renamed function
    ...
    output_dir = os.path.join(project_root, "data/courses") # Directory remains data/courses
    ensure_directories([output_dir])

    json_path = os.path.join(output_dir, "azure_training_updates.json") # Updated filename
    csv_path = os.path.join(output_dir, "azure_training_updates.csv")   # Updated filename
    ...
    try:
        ...
        logger.warning("pandas library not found. Skipping CSV generation for Azure Training entries.")
    except Exception as e:
