...
def process_ask_hn_posts(posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for post in posts:
        try:
            ...
            if any(word in combined_text for word in ['career', 'job', 'interview', 'salary', 'work', 'employment']):
                category = 'career'
            elif any(word in combined_text for word in ['startup', 'business', 'entrepreneur', 'revenue', 'funding']):
                category = 'startup'
            elif any(word in combined_text for word in ['ai', 'machine learning', 'ml', 'artificial intelligence', 'gpt', 'llm']):
                category = 'ai_ml'
            elif any(word in combined_text for word in ['programming', 'code', 'developer', 'software', 'language', 'framework']):
                category = 'programming'
            elif any(word in combined_text for word in ['advice', 'recommendation', 'suggest', 'opinion', 'experience']):
                category = 'advice'
            elif any(word in combined_text for word in ['tool', 'app', 'service', 'platform', 'product']):
                category = 'tools'
            elif any(word in combined_text for word in ['learn', 'education', 'course', 'book', 'study', 'tutorial']):
                category = 'learning'
            ...
            question_indicators = ['how', 'what', 'why', 'which', 'where', 'when', 'who']
...
def main():
    ...
    try:
        ...
        logger.info(f"HackerNews Ask ETL completed successfully!")
