...
class LobstersAPI:
    ...
    def _extract_stories_from_html(self, html: str, filter_tag: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for i, story_html in enumerate(story_matches):
            try:
                ...
                if title_match:
                    ...
                    story['title'] = re.sub(r'<[^>]+>', '', title_match.group(2)).strip()
                ...
                if discussion_match:
                    story['lobsters_url'] = urljoin(self.base_url, discussion_match.group(1))
                ...
                if tag_matches:
                    story['tags'] = [{'slug': tag[0], 'name': tag[1].strip()} for tag in tag_matches]
...
def get_lobsters_data(tags_to_track: List[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Successfully fetched {len(all_stories)} unique stories from Lobsters")
        return all_stories
...
def process_story(story: Dict[str, Any], tracked_tag: Optional[str] = None) -> Dict[str, Any]:
    """
...
def process_lobsters_stories(stories: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for story in stories:
        ...
        if any(tag in ['ai', 'machinelearning', 'ml', 'deeplearning'] for tag in tag_names):
            category = 'ai_ml'
        elif any(tag in ['security', 'cryptography', 'privacy'] for tag in tag_names):
            category = 'security'
        elif any(tag in ['web', 'javascript', 'frontend', 'react', 'vue'] for tag in tag_names):
            ...
            category = 'databases'
        elif any(tag in ['devops', 'docker', 'kubernetes', 'deployment'] for tag in tag_names):
            category = 'devops'
        ...
        if any(domain.endswith(site) for site in ['.edu', 'arxiv.org', 'scholar.google.com']):
            ...
            content_type = 'video'
        elif any(site in domain for site in ['news', 'techcrunch', 'ycombinator', 'arstechnica']):
            content_type = 'news'
        ...
        if any(tag in tag_names for tag in ['politics', 'privacy', 'ethics', 'philosophy']):
        ...
        if published_at:
            try:
                ...
                hours_since_pub = (datetime.utcnow().replace(tzinfo=pub_date.tzinfo) - pub_date).total_seconds() / 3600
...
def main():
    ...
    try:
        ...
        print(f"Average discussion potential: {sum(s.get('discussion_potential', 0) for s in processed_stories) / len(processed_stories):.2f}")
