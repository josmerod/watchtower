...
def get_dev_articles(session: requests.Session, per_page: int = 100, max_pages: int = 5) -> List[Dict[str, Any]]:
    """
    ...
    popular_tags = [
        "webdev", "javascript", "python", "react", "tutorial", "beginners",
        "programming", "productivity", "career", "opensource", "devops",
        "ai", "machinelearning", "database", "security", "cloud",
        "docker", "kubernetes", "typescript", "nodejs", "backend"
    ]
            ...
            for article in articles:
                ...
                processed_article = {
                    ...
                    "user": {
                        ...
                        "twitter_username": article.get('user', {}).get('twitter_username'),
                        "github_username": article.get('user', {}).get('github_username'),
                        "website_url": article.get('user', {}).get('website_url'),
                        "profile_image": article.get('user', {}).get('profile_image'),
                        "profile_image_90": article.get('user', {}).get('profile_image_90')
                    },
...
def process_dev_data(articles: List[Dict[str, Any]], tags: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    for article in articles:
        try:
            ...
            if published_at:
                ...
                hours_since_published = (current_time.replace(tzinfo=pub_date.tzinfo) - pub_date).total_seconds() / 3600
            else:
            ...
            freshness_factor = max(0, (48 - hours_since_published) / 48) if hours_since_published <= 48 else 0
            trend_score = engagement_score * (1 + freshness_factor)
            
            # Tag analysis
            article_tags = article.get('tag_list', [])
            tag_popularity_score = sum(tag_lookup.get(tag, {}).get('taggings_count', 0) for tag in article_tags[:5])
            ...
            elif any(word in title_lower for word in ["tips", "tricks", "best practices", "advice"]):
                content_type = "tips"
            elif any(word in title_lower for word in ["review", "comparison", "vs", "versus"]):
                content_type = "review"
            elif any(word in title_lower for word in ["news", "release", "update", "announcement"]):
                content_type = "news"
            elif any(word in title_lower for word in ["career", "job", "interview", "salary"]):
                content_type = "career"
...
def main():
    ...
    try:
        ...
        high_engagement = len([a for a in processed_data if a.get('engagement_score', 0) >= 50])
        viral_content = len([a for a in processed_data if a.get('popularity_category') == 'viral'])
        
        logger.info(f"DEV Community ETL completed successfully!")
