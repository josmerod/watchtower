...
def get_valencia_events(date: str, max_retries: int = 3, retry_delay: int = 5) -> List[Dict[str, Any]]:
    """
        ...
        try:
            ...
            for block in event_blocks:
                try:
                    ...
                    # Extract date
                    date_element = block.find(string=lambda t: t and isinstance(t, str) and 
                                           ("Del" in t or "al" in t or "2025" in t))
                    ...
                    logger.error(f"Error in Approach 1 parsing event: {str(e)}")
                    continue
            ...
            if not events:
                ...
                potential_entries = soup.find_all(class_=lambda x: x and isinstance(x, str) and 
                                               (x.startswith("###") or "event" in x.lower()))
                ...
                if not potential_entries:
                    ...
                    potential_entries = soup.find_all(lambda tag: tag.name in ["article", "div", "section"] and 
                                                        tag.find(["h3", "h2", "h4"]))
                
                event_entries.extend(potential_entries)
                
                # Also try with HTML structure seen in the example
                event_items = soup.find_all(lambda tag: tag.name and tag.get_text() and
                                         ("exposición" in tag.get_text().lower() or 
                                          "música" in tag.get_text().lower() or
                                          "2025" in tag.get_text()))
                
                event_entries.extend(event_items)
                
                logger.debug(f"Approach 2: Found {len(event_entries)} potential event entries")
                ...
                for entry in event_entries:
                    try:
                        ...
                        date_text = ""
                        date_element = entry.find(string=lambda t: t and isinstance(t, str) and 
                                              ("Del" in t or "al" in t or "2025" in t))
                        ...
                        if entry.get("class"):
                            ...
                            for cat in ["exposición", "música", "gastronomía", "espectáculo", "deporte", "naturaleza", "fiestas"]:
                        ...
                        logger.error(f"Error in Approach 2 parsing event: {str(e)}")
                        continue
            ...
            if not events:
                ...
                for heading in heading_elements:
                    try:
                        ...
                        category = ""
                        category_keywords = ["exposición", "música", "gastronomía", "espectáculo", "deporte", 
                                             "naturaleza", "fiestas", "festival"]
                        ...
                        logger.error(f"Error in Approach 3 parsing event: {str(e)}")
                        continue
            
            logger.info(f"Retrieved {len(events)} events from Valencia website for {date}")
            return events
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Error fetching data from Valencia website after {max_retries} attempts: {str(e)}")
                return []
    ...
    for event in events:
        try:
            ...
            if "Del" in date_info and "al" in date_info:
                ...
                date_parts = [part.strip() for part in date_info.split() if "2025" in part]
                if date_parts:
            ...
            if not category and description:
                ...
                for cat in ["exposición", "música", "gastronomía", "espectáculo", "deporte", 
                           "naturaleza", "fiestas", "festival"]:
...
def main():
    ...
    try:
        ...
        logger.info(f"Saved {len(unique_events)} processed events to {output_file} and {csv_file}")
    
    except Exception as e:
        logger.error(f"Error in Valencia events ETL process: {str(e)}", exc_info=True)
