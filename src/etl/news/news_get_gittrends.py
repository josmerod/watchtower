...
def get_trending_repositories(session: requests.Session, language: str = None, since: str = "daily") -> List[Dict[str, Any]]:
    ...
    languages = ["python", "javascript", "typescript", "rust", "go", "java", "cpp", "csharp", "php", "ruby"] if not language else [language]
...
def process_github_data(repositories: List[Dict[str, Any]], topics: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for repo in repositories:
        try:
            ...
            if any(word in description for word in ['ai', 'machine learning', 'neural', 'tensorflow', 'pytorch']):
                category = 'ai_ml'
...
def main():
    ...
    try:
        ...
        logger.info(f"GitHub Trends ETL completed successfully!")
