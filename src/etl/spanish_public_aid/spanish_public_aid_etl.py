...
class SpanishPublicAidETL(SimpleETL):
    ...
    def _extract_from_bdns(self, source_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            aid_elements = (
                soup.find_all('div', class_=re.compile(r'.*convocatoria.*|.*ayuda.*|.*subvencion.*', re.I)) or
                soup.find_all('li', class_=re.compile(r'.*convocatoria.*|.*ayuda.*|.*subvencion.*', re.I)) or
                soup.find_all('a', href=re.compile(r'.*convocatoria.*|.*ayuda.*|.*subvencion.*', re.I)) or
                soup.find_all('div', attrs={'data-type': re.compile(r'.*grant.*|.*aid.*', re.I)}) or
                soup.find_all('article') or
                soup.find_all('div', class_=re.compile(r'.*item.*|.*entry.*|.*post.*', re.I))
            )
        ...
        try:
            ...
            for url in search_urls:
                try:
                    ...
                    aid_elements = (
                        soup.find_all(['div', 'article', 'li'], class_=re.compile(r'.*ayuda.*|.*subven.*|.*procedure.*|.*dataset.*', re.I)) or
                        soup.find_all('a', href=re.compile(r'.*ayuda.*|.*subven.*|.*procedimiento.*', re.I)) or
                        soup.find_all('h3') or  # Often titles are in h3 tags
                        soup.find_all(['div', 'article'], attrs={'data-type': re.compile(r'.*grant.*|.*aid.*', re.I)})
                    )
        ...
        try:
            ...
            for link in aid_links:
                ...
                if (any(keyword in text.lower() for keyword in 
                       ['ayuda', 'subven', 'beca', 'fomento', '2025', 'apoyo', 'convocatoria']) and
                    not text.lower().strip() in ['subvenciones', 'ayudas', 'tramites'] and
                    len(text) > 20):  # Must be descriptive
    ...
    def _extract_from_labora(self, source_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for link in aid_links:
                ...
                if (any(keyword in text.lower() for keyword in 
                       ['empleo', 'contratación', 'formación', 'laboral', 'trabajo', 'desempleo', 
                        'fomento', 'incentivo', 'beca', 'ayuda', 'subven']) and
                    not text.lower().strip() in ['busco ayudas - subvenciones', 'ayudas', 'subvenciones']):
    ...
    def _parse_bdns_aid(self, element: BeautifulSoup, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Parse BDNS aid element."""
        try:
            # More flexible title extraction
            title_elem = (
                element.find(['h2', 'h3', 'h4'], class_=re.compile(r'.*titulo.*|.*title.*', re.I)) or
                element.find('a', href=True) or
                element.find(['span', 'div'], class_=re.compile(r'.*name.*|.*title.*', re.I)) or
                element
            )
            ...
            if len(title_text) < 10 or not any(keyword in title_text.lower() for keyword in 
                ['ayuda', 'subven', 'beca', 'fomento', 'apoyo', 'grant', 'aid']):
                return None
            
            # More flexible description extraction
            description_elem = (
                element.find(['p', 'div'], class_=re.compile(r'.*desc.*|.*resumen.*|.*content.*', re.I)) or
                element.find_next_sibling(['p', 'div']) or
                element.parent.find(['p', 'div']) if element.parent else None
            )
    ...
    def _parse_gva_aid(self, element: BeautifulSoup, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Skip if title is too short or doesn't look like a subsidy
            if len(title_text) < 10 or not any(keyword in title_text.lower() for keyword in 
                ['ayuda', 'subven', 'beca', 'fomento', 'apoyo', 'procedimiento', 'grant', 'aid', '2025']):
                return None
    ...
    def _parse_valencia_aid(self, element: BeautifulSoup, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Parse Valencia city aid element."""
        try:
            # More flexible title extraction for Valencia
            title_elem = (
                element.find(['h2', 'h3', 'h4']) or
                element if element.name == 'a' else
                element.find('a', href=True) or
                element.find(['span', 'div'])
            )
            ...
            # Skip if title is too short or doesn't look like a subsidy
            if len(title_text) < 10 or not any(keyword in title_text.lower() for keyword in 
                ['ayuda', 'subven', 'beca', 'fomento', 'apoyo', 'grant', 'aid', '2025', 'municipal']):
                return None
    ...
    def _parse_labora_aid(self, element: BeautifulSoup, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            title_elem = (
                ...
                element if element.name == 'a' else
                element.find('a', href=True) or
                element.find(['span', 'div'], class_=re.compile(r'.*title.*|.*name.*', re.I))
            )
            ...
            if len(title_text) < 10 or not any(keyword in title_text.lower() for keyword in 
                ['ayuda', 'subven', 'empleo', 'contratación', 'fomento', 'beca', 'formación', 'laboral', '2025']):
                return None
    ...
    def _parse_valencia_aid_detailed(self, link_element, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if detail_response.status_code == 200:
                    ...
                    desc_elem = (
                        detail_soup.find(['p', 'div'], class_=re.compile(r'.*desc.*|.*resumen.*|.*content.*', re.I)) or
                        detail_soup.find('meta', attrs={'name': 'description'}) or
                        detail_soup.find(['p', 'div'], string=re.compile(r'.{50,}', re.I))  # Long text
                    )
    ...
    def _parse_labora_aid_detailed(self, link_element, source_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                if detail_response.status_code == 200:
                    ...
                    desc_elem = (
                        detail_soup.find(['p', 'div'], class_=re.compile(r'.*desc.*|.*resumen.*|.*content.*', re.I)) or
                        detail_soup.find('meta', attrs={'name': 'description'}) or
                        detail_soup.find(['p', 'div'], string=re.compile(r'.{50,}', re.I))
                    )
    ...
    def _enhance_aid_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        amount = AmountModel(
            payment_type=PaymentType.LUMP_SUM,  # Default value
            currency="EUR"
        )
