...
class ADHDPublicationETL(BaseETL):
    ...
    def transform(self, data: list[dict]) -> list[ADHDPublication]:
        ...
        for paper_data in data:
            try:
                ...
                self.logger.error(
                    f"Error transforming article titled '{paper_data.get('title')}': {e}",
                    exc_info=True,
                )
    ...
    def load(self, data: list[ADHDPublication]):
        ...
        # pandas 3.x removed errors.PandasError; to_csv failures surface as
        # OSError/ValueError, both handled here (anything else by the clause below)
