...
class GitHubTrendingRSSETL(BaseETL[GitHubTrendingFeed, GitHubRepositoryModel]):
    ...
    def _parse_rss_entry(self, entry: Any, feed: GitHubTrendingFeed) -> GitHubRepositoryModel | None:
        ...
        try:
            ...
            repository = GitHubRepositoryModel(
                ...
                ) if repo_info.get('owner_login') else None,
    ...
    def _extract_description_from_html(self, html_content: str) -> str | None:
        ...
        try:
            ...
            clean_content = re.sub(r'<(script|style)[^>]*>.*?</\1>', '', decoded_content, flags=re.IGNORECASE | re.DOTALL)
    ...
    def load(self, repositories: List[GitHubRepositoryModel]) -> None:
        ...
        metadata = {
            ...
            "feed_summary": {
                key: len(repos) for key, repos in feed_groups.items()
            }
        }
