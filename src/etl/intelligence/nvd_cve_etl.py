...
class NVDEtl(BaseETL):
    ...
    def __init__(self, **kwargs):
        ...
        self.endpoints = {"cves": (f"https://services.nvd.nist.gov/rest/json/cves/2.0?resultsPerPage=50&pubStartDate={since}&pubEndDate={now}")}
    ...
    def extract(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not recent_vulns and vulnerabilities:
                self.logger.warning(f"NVD returned {len(vulnerabilities)} CVEs but none are from the last 90 days. The API date filter may be failing. Check NVD API status.")
