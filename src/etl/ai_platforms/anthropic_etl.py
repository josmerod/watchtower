...
class AnthropicETL(BaseETL):
    ...
    def load(self, data: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully loaded {len(data)} Anthropic records to {output_file}"
            )

        except OSError as e:
            self.logger.error(f"Failed to save Anthropic data to {output_file}: {e}")
            raise LoadError(
                f"Failed to save Anthropic data to {output_file}: {e}",
                destination=str(output_file),
                destination_type="file",
            ) from e
        except Exception as e:  # Catch any other unexpected errors during load
            self.logger.error(
                f"An unexpected error occurred during saving Anthropic data to {output_file}: {e}",
                exc_info=True,
            )
            raise LoadError(
                f"An unexpected error occurred during saving Anthropic data to {output_file}: {e}",
