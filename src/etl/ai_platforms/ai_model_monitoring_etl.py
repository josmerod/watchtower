...
class AIModelMonitoringETL:
    ...
    async def _scrape_openai_changelog(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not changelog_sections:
                ...
                changelog_sections = soup.find_all(['h2', 'h3', 'div'], string=re.compile(r'202[3-9]|January|February|March|April|May|June|July|August|September|October|November|December'))
    ...
    async def _scrape_anthropic_news(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for url_key, url in [("news_url", self.sources["anthropic"]["news_url"]), 
    ...
    async def _scrape_gemini_changelog(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not changelog_sections:
                ...
                changelog_sections = soup.find_all(['h2', 'h3'], string=re.compile(r'202[3-9]|January|February|March|April|May|June|July|August|September|October|November|December'))
    ...
    def _is_model_related(self, text: str, provider: str = None) -> bool:
        ...
        model_keywords = [
            "gpt", "claude", "gemini", "bard", "model", "api", "launch", "release",
            "update", "version", "capability", "feature", "improvement", "performance",
            "new model", "model update", "api update", "beta", "preview", "general availability"
        ]
        
        # Provider-specific keywords
        if provider == "openai":
            model_keywords.extend(["gpt-4", "gpt-3.5", "turbo", "davinci", "curie", "babbage", "ada", "embedding", "fine-tuning"])
        elif provider == "anthropic":
            model_keywords.extend(["claude-3", "claude-2", "claude instant", "opus", "sonnet", "haiku", "constitutional ai"])
