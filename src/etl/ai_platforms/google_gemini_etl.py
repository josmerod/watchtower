...
class GoogleGeminiETL(BaseETL[dict[str, Any], dict[str, Any]]):
    ...
    def transform(self, updates: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        self.logger.info(f"Transformed and filtered to {len(processed)} unique model-related updates")
        return processed
    ...
    def load(self, updates: list[dict[str, Any]]) -> None:
        ...
        json_file.write_text(json.dumps(updates, indent=2, ensure_ascii=False), encoding="utf-8")

        # Save latest JSON
        latest_json = self.output_dir / "google_updates_latest.json"
        latest_json.write_text(json.dumps(updates, indent=2, ensure_ascii=False), encoding="utf-8")
        ...
        try:
            ...
            if "metadata" in df.columns:
                for key in ["api_source", "processed_at", "entry_id", "feed_url"]:
                    df[f"metadata_{key}"] = df["metadata"].apply(lambda x: x.get(key, "") if isinstance(x, dict) else "")
            ...
            self.logger.info(f"Saved {len(updates)} updates to {json_file} and {csv_file}")
    ...
    def _filter_model_updates(self, updates: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        self.logger.info(f"Filtered {len(filtered)} model-related updates from {len(updates)} total")
        return filtered
