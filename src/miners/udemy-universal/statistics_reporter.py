...
class StatisticsReporter:
    ...
    def _save_daily_statistics(self):
        ...
        try:
            with open(self.daily_stats_file, 'w') as f:
                json.dump({date: asdict(stats) for date, stats in self.daily_stats.items()}, f, indent=2)
        except Exception as e:
    ...
    def end_session(self) -> Optional[EnrollmentStats]:
    ...
    def _update_daily_statistics(self, session: EnrollmentStats):
        ...
        daily.avg_session_duration = (
            (daily.avg_session_duration * (daily.sessions - 1) + session.duration) / daily.sessions
        )
    
    def record_enrollment(self, status: str, amount_saved: float = 0.0, 
    ...
    def record_site_activity(self, site_name: str, courses_found: int = 0, 
                           processing_time: float = 0.0, errors: int = 0):
        ...
        if site_stat.courses_processed > 0:
            site_stat.avg_processing_time = (
                (site_stat.avg_processing_time * (site_stat.courses_processed - 1) + processing_time) / 
        ...
        if site_stat.courses_processed > 0:
            site_stat.success_rate = (
                (site_stat.courses_processed - site_stat.errors) / site_stat.courses_processed
            ) * 100
    ...
    def display_overall_statistics(self, days: int = 30):
        ...
        recent_stats = [
            stat for stat in self.all_stats
        ...
        try:
            if format == "json":
                ...
                with open(filename, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        "Session ID", "Timestamp", "Duration", "Enrolled", "Already Enrolled",
                        "Expired", "Excluded", "Failed", "Total Processed", "Amount Saved",
                        "Sites Scraped", "Success Rate"
                    ])
                    for stat in self.all_stats:
                        success_rate = (stat.enrolled / max(1, stat.total_processed)) * 100
                        writer.writerow([
                            stat.session_id, stat.timestamp, stat.duration, stat.enrolled,
                            stat.already_enrolled, stat.expired, stat.excluded, stat.failed,
                            stat.total_processed, stat.amount_saved, ";".join(stat.sites_scraped),
                            success_rate
                        ])
