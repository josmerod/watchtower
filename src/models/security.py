...
class VulnerabilitySourceModel(TimestampedModel):
    ...
    total_vulnerabilities_fetched: int = Field(
        default=0,
        ge=0,
        description="Total vulnerabilities fetched from this source"
    )
...
class VulnerabilityModel(TimestampedModel):
    ...
    # CVSS Scoring
    cvss_version: Optional[str] = Field(default=None, description="CVSS version (e.g., '3.1')")
    ...
    cvss_environmental_score: Optional[float] = Field(
        default=None, 
        ge=0.0, 
        le=10.0, 
        description="CVSS environmental score"
    )
    
    # CVSS Metrics
    attack_vector: Optional[AttackVector] = Field(
        default=None, 
        description="CVSS attack vector"
    )
    attack_complexity: Optional[AttackComplexity] = Field(
        default=None, 
        description="CVSS attack complexity"
    )
    privileges_required: Optional[PrivilegesRequired] = Field(
        default=None, 
        description="CVSS privileges required"
    )
    user_interaction: Optional[UserInteraction] = Field(
        default=None, 
    ...
    severity_score: float = Field(
        ge=0.0, 
        le=10.0, 
        description="Watchtower calculated severity score"
    )
    ...
    affected_packages: List[str] = Field(
        default=[], 
        description="List of affected packages/components"
    )
    ...
    technology_stack: List[str] = Field(
        default=[], 
        description="Technology stack categories"
    )
    
    # Patch and mitigation info
    patch_available: bool = Field(default=False, description="Whether patch is available")
    patch_version: Optional[str] = Field(default=None, description="Version with patch")
    patch_url: Optional[HttpUrl] = Field(default=None, description="Patch download URL")
    estimated_fix_time: Optional[str] = Field(
        default=None, 
        description="Estimated time to fix (e.g., '24h', '1w')"
    )
    ...
    disclosed_date: Optional[datetime] = Field(
        default=None, 
    ...
    exploit_available: bool = Field(
        default=False, 
        description="Whether public exploit is available"
    )
    ...
    advisory_urls: List[HttpUrl] = Field(
        default=[], 
        description="Security advisory URLs"
    )
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, 
        description="Additional vulnerability metadata"
    )
    ...
    def needs_urgent_attention(self) -> bool:
        ...
        return (
            ...
            self.exploit_available and 
            not self.patch_available and
            self.days_since_published <= 7
        )
    ...
    @classmethod
    def auto_assign_risk_level(cls, v: Optional[RiskLevel], info) -> RiskLevel:
...
class ThreatIntelligenceModel(TimestampedModel):
    ...
    affected_technologies: List[str] = Field(
        default=[], 
        description="Affected technologies"
    )
    ...
    estimated_timeline: Optional[str] = Field(
        default=None, 
        description="Estimated timeline for threat"
    )
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, 
        description="Additional threat metadata"
    ) 