...
class AllKeyShopGameModel(TimestampedModel):
    ...
    image_url: Optional[HttpUrl] = Field(default=None, description="Game cover image URL")
    
    # Price information
    current_price: Optional[float] = Field(default=None, description="Current price in EUR")
    original_price: Optional[float] = Field(default=None, description="Original price in EUR")
    discount_percentage: Optional[int] = Field(default=None, description="Discount percentage")
    
    # Deal information
    deal_score: Optional[int] = Field(default=None, description="AllKeyShop deal score")
    store_name: Optional[str] = Field(default=None, description="Store offering the deal")
    store_rating: Optional[float] = Field(default=None, description="Store rating")
    
    # Game information
    platform: GamePlatform = Field(default=GamePlatform.PC, description="Game platform")
    genre: Optional[GameGenre] = Field(default=None, description="Game genre")
    rating: Optional[float] = Field(default=None, description="Game rating")
    metacritic_score: Optional[int] = Field(default=None, description="Metacritic score")
    
    # Release information
    release_date: Optional[datetime] = Field(default=None, description="Game release date")
    ...
    def is_good_deal(self) -> bool:
        ...
        if self.deal_score and self.deal_score >= 80:
            return True
        if self.discount_percentage and self.discount_percentage >= 50:
            return True
        return False
...
class GameBundleModel(TimestampedModel):
    ...
    original_total_price: Optional[float] = Field(default=None, description="Original total price of all games")
    savings_percentage: Optional[int] = Field(default=None, description="Bundle savings percentage")
    ...
    expires_at: Optional[datetime] = Field(default=None, description="Bundle expiration date")
...
class GameGiveawayModel(TimestampedModel):
    ...
    original_price: Optional[float] = Field(default=None, description="Original price of the game")
    giveaway_type: str = Field(description="Type of giveaway (permanent, limited, etc.)")
    
    # Time information
    starts_at: Optional[datetime] = Field(default=None, description="When giveaway starts")
    expires_at: Optional[datetime] = Field(default=None, description="When giveaway expires")
