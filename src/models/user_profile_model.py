...
class LearningGoal(TimestampedModel):
    ...
    deadline: datetime | None = Field(default=None, description="Optional deadline for goal")
...
class UserProfile(TimestampedModel):
    ...
    # Preferences
    preferred_domains: list[ResearchDomain] = Field(default=[], description="Preferred AI/ML research domains")
    skill_level: SkillLevel = Field(default=SkillLevel.INTERMEDIATE, description="Overall skill level")

    # Learning goals
    learning_goals: list[LearningGoal] = Field(default=[], description="Active learning goals")
    ...
    # Progress tracking
    completed_papers: list[str] = Field(default=[], description="IDs of papers user has read/completed")
    bookmarked_papers: list[str] = Field(default=[], description="IDs of papers saved for later")

    # Interaction history
    viewed_papers: dict[str, datetime] = Field(default={}, description="Paper IDs mapped to last viewed timestamp")

    # Metadata
    profile_created_at: datetime = Field(default_factory=datetime.utcnow, description="When profile was created")
    last_active_at: datetime = Field(default_factory=datetime.utcnow, description="Last time user was active")
    ...
    team_size: str | None = Field(default=None, description="Team size (Solo, Small 2-5, Medium 6-20, Large 20+)")
