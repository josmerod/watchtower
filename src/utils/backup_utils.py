...
class BackupManager:
    ...
    def _upload_single_file(self, local_file_path: Path, file_name_on_drive: str) -> str:
        ...
        try:
            ...
            existing_files = self.drive.ListFile({"q": f"'{self.backup_folder_id}' in parents and title='{file_name_on_drive}' and trashed=false"}).GetList()
    ...
    def _prepare_latest_in_drive(self, latest_filename_base: str):
        ...
        try:
            existing_latest_files = self.drive.ListFile({"q": f"'{self.backup_folder_id}' in parents and title='{latest_file_title_on_drive}' and trashed=false"}).GetList()
            ...
            for old_latest in existing_latest_files:  # Should typically be at most one
                ...
                try:
                    ...
                    collision_check = self.drive.ListFile({"q": f"'{self.backup_folder_id}' in parents and title='{new_historical_title}' and trashed=false"}).GetList()
                    if collision_check:
    ...
    def run_backup_process(self, folders_to_backup: list[str]):
        ...
        try:
            ...
            compressed_archive_path = self.compress_folders(folders_to_backup, archive_name_base=local_temp_archive_base)
...
if __name__ == "__main__":
    ...
    if not mock_settings_instance.google_drive.backup_folder_id or mock_settings_instance.google_drive.backup_folder_id == "YOUR_ACTUAL_GDRIVE_FOLDER_ID_HERE":
