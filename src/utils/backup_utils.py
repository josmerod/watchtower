...
try:
    ...
    print(
        "ERROR: PyDrive2 library is not installed. Google Drive functionality will not work."
    )
...
class BackupManager:
    def __init__(self, settings: Settings):
        ...
        if not GoogleAuth or not GoogleDrive:  # Check if PyDrive2 loaded
            module_logger.error(
                "PyDrive2 not loaded. Cannot initialize BackupManager for Google Drive."
            )
        ...
        if not hasattr(self.settings, "google_drive") or not isinstance(
            self.settings.google_drive, GoogleDriveConfig
        ):
            module_logger.error("GoogleDriveConfig not found in settings.")
            raise ValueError(
                "GoogleDriveConfig not found or misconfigured in settings."
            )
    ...
    def _authenticate_gdrive(self) -> GoogleDrive:
        ...
        if not credentials_file.is_absolute():
            ...
            if (
                not self.settings.project_root
                or not Path(self.settings.project_root).is_absolute()
            ):
                module_logger.error(
                    "Project root is not an absolute path or not set. Cannot resolve relative credentials_file path."
                )
                raise ValueError("Project root is not an absolute path or not set.")
            credentials_file = (
                Path(self.settings.project_root) / credentials_file_path_str
            )

        if not credentials_file.exists():
            module_logger.error(
                f"Google Drive client_secrets file not found at {credentials_file}"
            )
        ...
        self.gauth.settings["get_refresh_token"] = (
            True  # Important for non-interactive refresh
        )
        ...
        try:
            ...
            module_logger.info(
                f"Could not load saved credentials from {saved_credentials_path}: {e}. Will attempt new auth."
            )
            self.gauth.credentials = None  # Ensure it's None if loading failed

        if self.gauth.credentials is None:
            module_logger.info(
                "No valid saved credentials found. Attempting local web server authentication."
            )
            try:
                self.gauth.LocalWebserverAuth()  # Opens browser for user authorization
            except Exception as e:
                module_logger.error(
                    f"LocalWebserverAuth failed: {e}. This requires user interaction or a pre-generated '{saved_credentials_path}'."
                )
                module_logger.error(
                    f"For server environments, ensure '{saved_credentials_path}' is present and valid, or use a service account."
                )
            ...
            try:
                ...
                module_logger.error(
                    f"Consider re-authenticating by removing '{saved_credentials_path}' and running interactively once."
                )
                raise RuntimeError(f"Google Drive token refresh failed: {e}")
        else:
            module_logger.info(
                "Google Drive credentials loaded successfully and are valid."
            )
    ...
    def compress_folders(
        self, folders_to_backup: List[str], archive_name_base: str
    ) -> Path:
        ...
        module_logger.info(
            f"Starting compression of {folders_to_backup} into {zip_file_path}"
        )
        try:
                ...
                for folder_path_str in folders_to_backup:
                    ...
                    if not folder_path.exists() or not folder_path.is_dir():
                        module_logger.warning(
                            f"Folder {folder_path} does not exist or is not a directory. Skipping."
                        )
                        continue
            ...
            module_logger.info(
                f"Successfully created archive: {zip_file_path} (Size: {zip_file_path.stat().st_size} bytes)"
            )
            return zip_file_path
        except Exception as e:
            module_logger.error(
                f"Failed to create archive {zip_file_path}: {e}", exc_info=True
            )
            if zip_file_path.exists():
                try:
                    ...
                    module_logger.error(
                        f"Failed to remove partial archive {zip_file_path}: {ose}"
                    )
            raise

    def _upload_single_file(
        self, local_file_path: Path, file_name_on_drive: str
    ) -> str:
        """
        ...
        module_logger.info(
            f"Uploading {local_file_path} as '{file_name_on_drive}' to Drive folder ID {self.backup_folder_id}..."
        )
        try:
            ...
            for old_file in existing_files:
                module_logger.info(
                    f"Deleting existing file '{old_file['title']}' (ID: {old_file['id']}) in Drive to replace with new version."
                )
            ...
            module_logger.info(
                f"Successfully uploaded '{file_name_on_drive}' to Google Drive. File ID: {gfile['id']}"
            )
            return gfile["id"]
        except Exception as e:
            module_logger.error(
                f"Failed to upload {local_file_path} as '{file_name_on_drive}' to Google Drive: {e}",
                exc_info=True,
            )
            raise
    ...
    def _rename_drive_file(self, file_id: str, new_title: str):
        ...
        try:
            ...
            module_logger.info(
                f"Successfully renamed file ID {file_id} to '{new_title}'."
            )
        except Exception as e:
            module_logger.error(
                f"Failed to rename file ID {file_id} to '{new_title}': {e}",
                exc_info=True,
            )
            raise
    ...
    def _get_drive_file_modified_date(self, file_id: str) -> datetime.datetime:
        ...
        try:
            ...
            module_logger.error(
                f"Failed to get modified date for file ID {file_id}: {e}", exc_info=True
            )
            raise
    ...
    def _prepare_latest_in_drive(self, latest_filename_base: str):
        ...
        if not self.drive:
            module_logger.error(
                "Google Drive not authenticated. Cannot prepare latest."
            )
            raise ConnectionError("Google Drive not authenticated.")

        latest_file_title_on_drive = f"{latest_filename_base}_latest.zip"
        module_logger.info(
            f"Preparing for new 'latest' backup. Checking for existing '{latest_file_title_on_drive}'."
        )
        ...
        try:
            ...
            for old_latest in existing_latest_files:  # Should typically be at most one
                module_logger.info(
                    f"Found existing latest file: {old_latest['title']} (ID: {old_latest['id']}). Renaming it."
                )
                try:
                    ...
                    if collision_check:
                        timestamp_str = modified_date.strftime(
                            "%Y%m%d_%H%M%S_%f"
                        )  # Add microseconds
                        new_historical_title = (
                            f"{latest_filename_base}_{timestamp_str}.zip"
                        )
                        module_logger.warning(
                            f"Timestamp collision for historical rename. Using microseconds: {new_historical_title}"
                        )

                    self._rename_drive_file(old_latest["id"], new_historical_title)
                    module_logger.info(
                        f"Renamed previous latest file (ID: {old_latest['id']}) to '{new_historical_title}'."
                    )
                except Exception as e:
                    module_logger.error(
                        f"Could not rename previous latest file {old_latest['title']} (ID: {old_latest['id']}): {e}",
                        exc_info=True,
                    )
            ...
            if not existing_latest_files:
                module_logger.info(
                    f"No existing '{latest_file_title_on_drive}' found. Safe to upload new one."
                )
        except Exception as e:
            module_logger.error(
                f"Error while preparing for new latest backup (listing existing latest): {e}",
                exc_info=True,
            )
            raise

    def _enforce_retention_policy(
        self, latest_filename_base: str, max_historical_copies: int = 5
    ):
        """
        ...
        if not self.drive:
            module_logger.error(
                "Google Drive not authenticated. Cannot enforce retention."
            )
            raise ConnectionError("Google Drive not authenticated.")

        module_logger.info(
            f"Enforcing retention policy for base '{latest_filename_base}' (max {max_historical_copies} historical copies)."
        )
        try:
            ...
            for file_item in file_list:
                ...
                if title.startswith(f"{latest_filename_base}_") and title.endswith(
                    ".zip"
                ):
                    try:
                        # Extract timestamp part: YYYYMMDD_HHMMSS or YYYYMMDD_HHMMSS_ffffff
                        ts_part_match = title[
                            len(latest_filename_base) + 1 : -4
                        ]  # Remove prefix and .zip

                        # Try parsing with microseconds, then without
                        try:
                            ts_obj = datetime.datetime.strptime(
                                ts_part_match, "%Y%m%d_%H%M%S_%f"
                            )
                        except ValueError:
                            ts_obj = datetime.datetime.strptime(
                                ts_part_match, "%Y%m%d_%H%M%S"
                            )

                        historical_backups.append((ts_obj, file_item))
                    except ValueError:
                        module_logger.warning(
                            f"Could not parse timestamp from historical backup filename: {title}. Skipping this file for retention."
                        )

            historical_backups.sort(
                key=lambda x: x[0], reverse=True
            )  # Sort newest first (most recent timestamp)

            if len(historical_backups) > max_historical_copies:
                files_to_delete_count = len(historical_backups) - max_historical_copies
                module_logger.info(
                    f"Found {len(historical_backups)} historical backups. Deleting {files_to_delete_count} oldest ones."
                )
                files_to_delete = historical_backups[
                    max_historical_copies:
                ]  # The tail end of the sorted list

                for ts_obj, file_to_delete in files_to_delete:
                    module_logger.info(
                        f"Deleting old backup: {file_to_delete['title']} (ID: {file_to_delete['id']}, Timestamp: {ts_obj.isoformat()})"
                    )
                    try:
                        file_to_delete.Delete()  # PyDrive Delete() permanently deletes
                    except Exception as e_del:
                        module_logger.error(
                            f"Failed to delete old backup {file_to_delete['title']} (ID: {file_to_delete['id']}): {e_del}",
                            exc_info=True,
                        )
            else:
                module_logger.info(
                    f"Found {len(historical_backups)} historical backups. No deletion needed (max {max_historical_copies})."
                )

        except Exception as e:
            module_logger.error(
                f"Error during backup retention policy enforcement: {e}", exc_info=True
            )
    ...
    def run_backup_process(self, folders_to_backup: List[str]):
        ...
        if not self.drive:  # Check if authentication failed in constructor
            module_logger.error(
                "BackupManager not properly initialized (Google Drive auth failed). Aborting backup."
            )
            return False  # Indicate failure

        latest_filename_base = (
            "backup_data_logs"  # Consistent base name for this type of backup
        )
        local_temp_archive_base = (
            f"{latest_filename_base}_local_temp"  # Temporary local archive name
        )
        ...
        try:
            ...
            compressed_archive_path = self.compress_folders(
                folders_to_backup, archive_name_base=local_temp_archive_base
            )
            module_logger.info(
                f"Compression complete. Archive: {compressed_archive_path}"
            )

            # 2. Prepare Drive: Find current "{base}_latest.zip" on Drive, rename it to historical with timestamp
            module_logger.info(
                f"Step 2: Preparing Google Drive for new '{latest_zip_name_on_drive}'."
            )
            ...
            module_logger.info(
                f"Step 3: Uploading {compressed_archive_path} to Google Drive as '{latest_zip_name_on_drive}'."
            )
            ...
            module_logger.info(
                f"Step 4: Enforcing retention policy for historical backups based on '{latest_filename_base}'."
            )
            self._enforce_retention_policy(
                latest_filename_base, max_historical_copies=5
            )
            ...
            module_logger.error(
                f"Backup process failed due to a missing file: {e}", exc_info=True
            )
        except ConnectionError as e:  # For auth/network issues with GDrive
            module_logger.error(
                f"Backup process failed due to a connection or authentication error: {e}",
                exc_info=True,
            )
        except Exception as e:
            module_logger.error(
                f"An unexpected error occurred during the backup process: {e}",
                exc_info=True,
            )
        finally:
            ...
            if compressed_archive_path and compressed_archive_path.exists():
                try:
                    module_logger.info(
                        f"Cleaning up local archive: {compressed_archive_path}"
                    )
                    os.remove(compressed_archive_path)
                    module_logger.info(f"Successfully cleaned up local archive.")
                except OSError as e_os:
                    module_logger.error(
                        f"Failed to clean up local archive {compressed_archive_path}: {e_os}",
                        exc_info=True,
                    )
        return success
...
if __name__ == "__main__":
    ...
    TEST_GDRIVE_BACKUP_FOLDER_ID = (
        "YOUR_ACTUAL_GDRIVE_FOLDER_ID_HERE"  # <--- REPLACE THIS
    )
    # --- END USER SETUP ---
    ...
    try:
        ...
        (test_data_dir / "sample_data1.txt").write_text(
            "This is some sample data for backup testing."
        )
        (test_data_dir / "sample_data2.txt").write_text("Another piece of data here.")
        test_logs_dir.mkdir(parents=True, exist_ok=True)
        (test_logs_dir / "sample_log1.txt").write_text(
            "Log entry: ETL process started."
        )
        (test_logs_dir / "sample_log2.txt").write_text(
            "Log entry: ETL process finished."
        )
        module_logger.info(
            f"Created dummy data in {test_data_dir} and {test_logs_dir} for testing."
        )
    except Exception as e:
        module_logger.error(f"Could not create dummy data/logs for testing: {e}")

    if (
        not mock_settings_instance.google_drive.backup_folder_id
        or mock_settings_instance.google_drive.backup_folder_id
        == "YOUR_ACTUAL_GDRIVE_FOLDER_ID_HERE"
    ):
        module_logger.warning(
            "Skipping direct execution: `TEST_GDRIVE_BACKUP_FOLDER_ID` is not set for testing in `backup_utils.py`."
        )
        module_logger.warning(
            "Please set a valid Google Drive Folder ID to test `BackupManager` directly."
        )
    else:
        try:
            module_logger.info(f"Attempting backup test with BackupManager...")
            module_logger.info(
                f"Using project root: {mock_settings_instance.project_root}"
            )
            module_logger.info(
                f"Using data_dir (relative): {mock_settings_instance.data_dir}"
            )
            module_logger.info(
                f"Using logs_dir (relative): {mock_settings_instance.logs_dir}"
            )
            module_logger.info(
                f"Credentials file: {mock_settings_instance.google_drive.credentials_file}"
            )
            module_logger.info(
                f"Backup Folder ID: {mock_settings_instance.google_drive.backup_folder_id}"
            )
            ...
            module_logger.info(
                f"Calling manager.run_backup_process with folders: {folders_to_backup_list}"
            )
            backup_succeeded = manager.run_backup_process(folders_to_backup_list)
            if backup_succeeded:
                module_logger.info(
                    "Direct test run of BackupManager completed successfully."
                )
            else:
            ...
            module_logger.error(
                f"Error during direct test run of BackupManager: {e}", exc_info=True
            )
