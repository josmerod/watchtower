...
class YouTubeOCRConverter:
    ...
    def get_domain_statistics(self) -> pd.DataFrame:
        ...
        domain_stats.columns = [
            'unique_videos', 'total_mentions', 'avg_confidence', 
            'min_confidence', 'max_confidence', 'valid_urls'
        ]
    ...
    def get_processing_statistics(self) -> pd.DataFrame:
        ...
        stats = {
            ...
            'videos_with_urls': len(basic_df[basic_df['has_extracted_urls'] == True]),
            'videos_without_urls': len(basic_df[basic_df['has_extracted_urls'] == False]),
    ...
    def get_text_analysis_table(self) -> pd.DataFrame:
        ...
        for item in self.data:
            ...
            record = {
                ...
                'contains_website_keywords': any(keyword in ocr_text.lower() for keyword in 
                                               ['website', 'site', '.com', '.org', '.net', 'http']),
                'contains_productivity_keywords': any(keyword in ocr_text.lower() for keyword in 
                                                    ['productivity', 'tool', 'app', 'software']),
                'contains_design_keywords': any(keyword in ocr_text.lower() for keyword in 
                                              ['design', 'template', 'mockup', 'graphic']),
                'processing_status': item.get('processing_status', ''),
                'processed_at': item.get('processed_at', '')
            }
            records.append(record)
    ...
    def get_dashboard_summary(self) -> Dict:
        ...
        return {
            ...
            'successful_processing_rate': round(
                len(basic_df[basic_df['processing_status'] == 'success']) / len(basic_df) * 100, 1
            ),
            'videos_with_urls': len(basic_df[basic_df['has_extracted_urls'] == True]),
            ...
            } if 'processed_at' in basic_df.columns else {}
        }


def load_youtube_ocr_data(json_file_path: str = "data/youtube_shorts_ocr/youtube_shorts_ocr_results.json") -> YouTubeOCRConverter:
    """
