...
class WatcherState(TimestampedModel):
    ...
    last_value: Optional[str] = Field(None, description="Last extracted value")
...
class WatcherEvent(TimestampedModel):
    ...
    new_value: Optional[Any] = Field(None, description="Current value")
    url: Optional[str] = Field(None, description="URL being watched")
...
class WatcherConfig(BaseWatchtowerModel):
    ...
    alert_threshold: Optional[int] = Field(None, description="Alert after N consecutive failures")
...
class EnhancedWatcher(ABC):
    ...
    def __init__(self, config: Union[WatcherConfig, Dict[str, Any]]):
    ...
    def _record_event(self, event_type: str, old_value: Any = None, new_value: Any = None, 
                     details: Optional[Dict[str, Any]] = None) -> WatcherEvent:
    ...
    async def fetch_page(self) -> str:
        ...
        try:
            ...
            raise WatcherError(
                message=f"Failed to fetch {self.config.url}: {str(e)}",
    ...
    def trigger_alert(self, old_value: Any, new_value: Any, details: Optional[Dict[str, Any]] = None) -> None:
            ...
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    self.logger.warning(
                        ...
                        f"{self.config.name}: {e}. Retrying in {self.config.retry_delay}s..."
                    )
    ...
    async def run_continuous(self, max_iterations: Optional[int] = None) -> None:
