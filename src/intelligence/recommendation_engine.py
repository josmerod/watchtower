...
class ContentBasedRecommendationEngine:
    ...
    def recommend_papers(
        ...
        if exclude_completed:
            available_papers = [p for p in available_papers if p.id not in user_profile.completed_papers]
    ...
    def score_relevance(self, user_profile: UserProfile, paper: AIResearchPaper) -> tuple[float, dict[str, float]]:
    ...
    def _score_complexity_match(self, user_profile: UserProfile, paper: AIResearchPaper) -> float:
        ...
        Returns:
            Score from 0.0 to 1.0
        """
        appropriate_complexities = self.COMPLEXITY_SKILL_MAPPING.get(user_profile.skill_level, [ImplementationComplexity.MEDIUM])
    ...
    def explain_recommendation(self, paper: AIResearchPaper, score_breakdown: dict[str, float]) -> str:
        ...
        for factor, score in sorted_factors[:2]:  # Top 2 factors
            if factor == "domain_match" and score > 0.3:
                domain_str = getattr(paper.primary_domain, "value", str(paper.primary_domain))
                reasons.append(f"Matches your interest in {domain_str}")
            elif factor == "complexity_match" and score > 0.2:
                complexity_str = getattr(paper.complexity, "value", str(paper.complexity))
...
class LearningPathOptimizer:
    ...
    def suggest_next_steps(self, user_profile: UserProfile, completed_papers: list[AIResearchPaper]) -> list[str]:
