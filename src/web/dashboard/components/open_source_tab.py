...
def render_open_source_tab():
    ...
    settings = get_settings()
    Path(settings.data_dir) / "open_source_intelligence" / "output" / "latest.json"
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            dcc.Dropdown(id="os-tag-filter", options=[{"label": "All Tags", "value": "ALL"}] + [{"label": t, "value": t} for t in sorted_tags], value="ALL", clearable=False),
                                        ],
                                        md=6,
                                    ),
                                    dbc.Col(
                                        [
                                            html.Label("Sort By"),
                                            dcc.Dropdown(
                                                id="os-sort-filter", options=[{"label": "Newest First", "value": "newest"}, {"label": "Oldest First", "value": "oldest"}], value="newest", clearable=False
                                            ),
                                        ],
                                        md=6,
                                    ),
...
@callback(Output("os-content-grid", "children"), [Input("os-tag-filter", "value"), Input("os-sort-filter", "value")])
...
def _create_project_card(item: OpenSourceProjectItem):
    ...
    tags_badges = [html.Span(tag, className="badge bg-light text-dark border me-1 mb-1") for tag in item.tags[:5]]  # Limit tags
        ...
        [
            dbc.CardBody(
                [
                    html.Div(
                        [
                            html.H5(html.A(item.title, href=item.url, target="_blank", className="text-decoration-none text-dark"), className="card-title text-truncate"),
                        ],
                        className="mb-2",
                    ),
                    ...
                    html.Div(
                        [
                            html.Small([html.I(className="bi bi-calendar-event text-secondary me-1"), date_str], className="me-3"),
                    ...
                    html.Div([html.A("View Project", href=item.url, target="_blank", className="btn btn-sm btn-outline-primary mt-2 w-100")]),
                ],
