...
def load_research_data(file_path):
    """Load and parse research data from JSON file"""
    try:
        ...
        if isinstance(data, list):
            ...
            if 'papers' in data:
                ...
                for paper in data['papers']:
                    processed_item = process_research_item(paper)
                    if processed_item:
                        processed_data.append(processed_item)
                return processed_data
            else:
                processed_item = process_research_item(data)
...
def process_research_item(item):
    ...
    try:
        ...
        abstract = item.get('abstract', item.get('description', item.get('summary', 'No abstract available')))
    ...
    for source_id, config in RESEARCH_SOURCES_CONFIG.items():
        ...
        if config['category'] == 'Medical Research':
            metrics_content = [
                html.Div([
                    ...
                    html.Span(f"{avg_citation:.1f}" if avg_citation > 0 else "N/A", className="text-info")
                ], className="mb-1"),
                html.Div([
                    html.Strong("Avg Sample Size: "),
                    html.Span(f"{avg_sample_size:.0f}" if avg_sample_size > 0 else "N/A", className="text-warning")
                ], className="mb-1")
            ]
        else:
            metrics_content = [
                html.Div([
                    ...
                    html.Span(f"{avg_rating:.1f}/5" if avg_rating > 0 else "N/A", className="text-success")
                ], className="mb-1")
            ]
        ...
        card = dbc.Card([
            dbc.CardHeader([
                ...
                dbc.Badge(f"{item_count} items", color=status_color, className="float-end")
            ]),
            dbc.CardBody([
...
def create_research_table(source_id, items):
    ...
    if config['category'] == 'Medical Research':
        sorted_items = sorted(items, key=lambda x: (x['citation_count'], x['publication_date'] or datetime.min.replace(tzinfo=timezone.utc)), reverse=True)
    else:
    ...
    if config['category'] == 'Medical Research':
        ...
        columns = [
            ...
            {"name": "Accessibility Features", "id": "Accessibility Features", "type": "text"},
        ...
        tooltip_data=[
            {
                ...
                for col in ['Abstract', 'Description'] if col in row
            }
            ...
            dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(len(research_papers), className="text-primary mb-0"),
                                html.P("Research Papers", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(f"{total_citations:,}", className="text-success mb-0"),
                                html.P("Total Citations", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(f"{avg_sample_size:.0f}" if avg_sample_size > 0 else "N/A", className="text-info mb-0"),
                                html.P("Avg Sample Size", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(len(locations), className="text-warning mb-0"),
                                html.P("ADHD-Friendly Locations", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3)
                ], className="mb-4")
