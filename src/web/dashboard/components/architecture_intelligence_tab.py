...
from src.models.architecture_pattern_model import ArchitecturePattern, ArchitecturalStyle
        ...
        dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader([
                            dbc.Row([
                                dbc.Col(html.H4("📚 Pattern Catalog", className="mb-0"), width=8),
                                dbc.Col(
                                    dbc.Input(
                            ...
                            dbc.Row([
                                dbc.Col([
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        options=[
                                            {"label": style.value, "value": style.value}
            ...
            dbc.Modal(
                [
                    ...
                    dbc.ModalFooter(
                        dbc.Button("Close", id="arch-modal-close", className="ms-auto", n_clicks=0)
                    ),
                ],
...
def register_architecture_callbacks(app):
    ...
    @app.callback(
        ...
        [Input("arch-pattern-search", "value"),
         Input("arch-style-filter", "value")]
    )
    def update_pattern_grid(search_term, style_filter):
        ...
        if search_term:
            ...
            filtered_patterns = [
                ...
                if term in p.name.lower() or term in p.description.lower() or term in p.summary.lower()
            ]
            
        if not filtered_patterns:
            return html.Div("No patterns found matching your criteria.", className="text-center text-muted my-5")
            ...
            card = dbc.Col(
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.P(pattern.summary, className="card-text small text-muted"),
                        dbc.Button(
                            "View Details", 
                            id={"type": "arch-pattern-btn", "index": pattern.pattern_id},
                            color="primary",
                            size="sm",
                            outline=True,
                            className="mt-2"
    ...
    def toggle_modal(n_clicks, close_click, is_open):
        ...
        if "arch-pattern-btn" in trigger_id:
            ...
            # Build modal content
            content = html.Div([
                dbc.Badge(pattern.complexity.value + " Complexity", color="warning" if pattern.complexity.value in ["High", "Very High"] else "success", className="me-2"),
                dbc.Badge(pattern.scalability_level + " Scalability", color="info", className="me-2"),
                html.Hr(),
                ...
                html.Div([
                    dbc.Badge(tech, color="light", text_color="dark", className="me-1 border") 
                    for tech in pattern.compatible_technologies
                ]),
                
                html.H6("Real World Examples", className="mt-3"),
                html.Div([
                    dbc.Badge(ex, color="secondary", className="me-1") 
                    for ex in pattern.real_world_examples
                ])
            ])
        ...
        for pattern, score, breakdown in recommendations:
            ...
            cards.append(
                dbc.Col(
                    dbc.Card([
                        dbc.CardBody([
                            html.H6("Recommended for You", className="text-success small mb-2"),
                            html.H5(pattern.name, className="card-title"),
                            html.P(explanation, className="small text-muted"),
                            dbc.Progress(value=score*100, color="success", className="mb-2", style={"height": "5px"}),
                            dbc.Button(
                                "View", 
                                id={"type": "arch-pattern-btn", "index": pattern.pattern_id},
                                color="link",
