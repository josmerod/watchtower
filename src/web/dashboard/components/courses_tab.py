...
def load_coursera_data():
    ...
    expected_cols = ['title', 'url', 'description', 'institution', 'subject', 'language', 'duration', 'start_date_str', 'is_free', 'certificate_offered', 'scraped_at_str']
...
def load_udemy_data():
    ...
    expected_cols = ['title', 'url', 'scraped_at_str'] # Udemy data is simpler from this source
    for col in expected_cols:
        if col not in df.columns:
            df[col] = None
...
def format_coursera_display_date(dt_obj):
    if pd.isna(dt_obj) or dt_obj is None: return "N/A"
...
def create_coursera_table(df_subset):
    ...
    table_header = [html.Thead(html.Tr([
        html.Th("Title"), html.Th("Institution"), html.Th("Subject"),
        html.Th("Language"), html.Th("Duration"), html.Th("Start Date"),
        html.Th("Added"), html.Th("Free"), html.Th("Certificate")
    ]))]
...
def render_coursera_courses_sub_tab(df):
    if not COURSES_DATA_LOADED['coursera']: # Check if loading was even attempted and successful
        return dbc.Alert("Coursera courses data failed to load. Check logs.", color="danger", className="mt-3")
    if df.empty : # Check if the dataframe itself is empty after successful load
        return dbc.Alert("No Coursera courses data currently available (file might be empty or all data filtered out).", color="info", className="mt-3")
    ...
    return html.Div([
        dbc.Row([
            dbc.Col(dbc.Input(id="coursera-search-input", placeholder="Search by title/description..."), md=4, className="mb-2"),
            dbc.Col(dcc.Dropdown(id="coursera-subject-dropdown", options=subject_options, placeholder="Filter by subject"), md=3, className="mb-2"),
            dbc.Col(dcc.Dropdown(id="coursera-language-dropdown", options=language_options, placeholder="Filter by language"), md=3, className="mb-2"),
...
def render_udemy_courses_sub_tab(df):
    if not COURSES_DATA_LOADED['udemy']:
        return dbc.Alert("Udemy courses data failed to load. Check logs.", color="danger", className="mt-3")
    if df.empty:
        return dbc.Alert("No Udemy courses data currently available (file might be empty).", color="info", className="mt-3")
