...
def render_deals_tab():
    ...
    total_categories = len({deal["source_category"] for deal in ALL_DEALS})
...
def create_deals_tabs_content():
    ...
    tab_definitions = [
        ...
        {"label": "✈️ Travel", "keys": "travel_deals", "id": "travel"},
        {
            ...
            "keys": "crypto_finance_deals",
            "id": "crypto",
        },
        {
            "label": "👗 Fashion & Retail",
            "keys": "fashion_retail_deals",
            "id": "fashion",
        },
        {
            "label": "🏃 Health & Fitness",
            "keys": "health_fitness_deals",
            "id": "health",
        },
        {
            "label": "🔧 Hardware & Tech",
            "keys": "hardware_tech_deals",
            "id": "hardware",
        },
    ]
...
def create_deals_category_tab_content(category_key):
    ...
    if not deals:
        return dbc.Alert("No deals available for this category.", color="info")
        ...
        def update_deals_search(search_term, deals_data, current_search_id=search_id):
            ...
            try:
                ...
                if not filtered_deals:
                    return dbc.Alert(f"No deals found matching '{search_term}'", color="info")
...
if __name__ == "__main__":
    ...
    total = len(ALL_DEALS)
    categories = len({deal["source_category"] for deal in ALL_DEALS})
    total_savings = sum(deal["savings"] for deal in ALL_DEALS if deal["savings"] > 0)
