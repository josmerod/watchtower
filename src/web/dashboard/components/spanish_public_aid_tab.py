...
def create_aid_summary_cards(aids_data: List[Dict], stats_data: Dict) -> html.Div:
    ...
    closing_soon = len([aid for aid in aids_data 
                       if aid.get('status') == 'abierta' and 
                       aid.get('closing_date') and
                       parse_aid_date(aid['closing_date']) and
                       (parse_aid_date(aid['closing_date']) - datetime.now()).days <= 7])
    ...
    cards = [
        ...
        dbc.Card([
            dbc.CardBody([
                html.H4(most_common_category.title(), className="card-title text-info"),
...
def create_aids_table(aids_data: List[Dict]) -> html.Div:
    ...
    return html.Div([
        html.H5("Tabla de Ayudas Públicas", className="mb-3"),
        table
    ])
...
def render_spanish_public_aid_tab():
    ...
    if not aids_data:
        return dbc.Container([
            dbc.Alert([
                ...
                html.Hr(),
                html.P("Asegúrate de ejecutar el ETL de ayudas públicas españolas primero:", 
                       className="mb-0"),
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                create_category_chart(aids_data)
            ], width=6),
            
            dbc.Col([
                create_scope_chart(aids_data)
            ], width=6),
        ], className="mb-4"),
        ...
        dbc.Row([
            dbc.Col([
                create_status_timeline(aids_data)
            ], width=12),
        ], className="mb-4"),
        ...
        # Aids table
        html.Div(id='filtered-aids-table', children=[
            create_aids_table(aids_data)
        ]),
...
def register_spanish_aid_callbacks(app):
    ...
    def update_aids_table(category_filter, status_filter, scope_filter, urgent_filter, 
                         search_text, search_clicks, refresh_clicks):
        ...
        if category_filter and category_filter != 'all':
            filtered_data = [aid for aid in filtered_data 
                            if aid.get('category') == category_filter]
        
        # Status filter
        if status_filter and status_filter != 'all':
            filtered_data = [aid for aid in filtered_data 
                            if aid.get('status') == status_filter]
        ...
        if search_text:
            ...
            search_filtered = []
            for aid in filtered_data:
                if (search_lower in aid.get('title', '').lower() or
                    search_lower in aid.get('description', '').lower() or
                    any(search_lower in keyword.lower() for keyword in aid.get('keywords', []))):
