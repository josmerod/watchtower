...
GITHUB_TRENDING_FEEDS = {
    # All Languages
    "daily_all": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_daily_all.json"),
        "name": "Daily - All Languages",
        "description": "Trending repositories today across all languages"
    },
    "weekly_all": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_weekly_all.json"),
        "name": "Weekly - All Languages", 
        "description": "Trending repositories this week across all languages"
    },
    "monthly_all": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_monthly_all.json"),
    ...
    # Python
    "daily_python": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_daily_python.json"),
        "name": "Daily - Python",
        "description": "Trending Python repositories today"
    },
    "weekly_python": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_weekly_python.json"),
        "name": "Weekly - Python",
        "description": "Trending Python repositories this week"
    },
    "monthly_python": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_monthly_python.json"),
        "name": "Monthly - Python",
    ...
    # Jupyter Notebook
    "weekly_jupyter": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_weekly_jupyter-notebook.json"),
        "name": "Weekly - Jupyter Notebook",
        "description": "Trending Jupyter Notebook repositories this week"
    },
    "monthly_jupyter": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_monthly_jupyter-notebook.json"),
    ...
    # CUDA
    "monthly_cuda": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_monthly_cuda.json"),
        "name": "Monthly - CUDA",
    ...
    # Terraform (HCL)
    "monthly_terraform": {
        "path": get_data_path("github_trending_rss/output/github_trending", "github_trending_monthly_hcl.json"),
...
def get_all_github_trending_data():
    """Load fresh GitHub trending data from all configured feeds."""
    return {
...
def create_github_trending_tab_content(feed_keys, combined_name=None):
    ...
    for repo in repos_to_display:
        ...
        table_body_rows.append(html.Tr([
            html.Td([
                ...
                ) if url != '#' else full_name
            ]),
            html.Td(description, style={"fontSize": "0.9em", "color": "#666"}),
            html.Td(language_badge),
            html.Td([
                html.I(className="fas fa-star", style={"color": "#ffd700", "marginRight": "5px"}),
                stars
            ], style={"whiteSpace": "nowrap"}),
            html.Td([
                html.I(className="fas fa-code-branch", style={"color": "#666", "marginRight": "5px"}),
                forks
            ], style={"whiteSpace": "nowrap"}),
            ...
            html.Td(date_display, style={"fontSize": "0.8em", "whiteSpace": "nowrap"})
        ]))
    ...
    summary_info = html.Div([
        ...
        dbc.Badge(f"{format_number(total_stars)} total stars", color="warning", className="me-2"),
    ], className="mb-3")
    
    return html.Div([
        summary_info,
        html.Div(table, style={"maxHeight": "800px", "overflowY": "auto", "paddingRight": "15px"})
    ])
...
def render_github_trending_tab():
    ...
    tab_definitions = [
        ...
        {"label": "Monthly - Jupyter", "keys": "monthly_jupyter", "id": "monthly-jupyter"},
        {"label": "Monthly - CUDA", "keys": "monthly_cuda", "id": "monthly-cuda"},
        {"label": "Monthly - Terraform", "keys": "monthly_terraform", "id": "monthly-terraform"},
    ]
