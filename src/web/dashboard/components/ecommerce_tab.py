    ...
    for source_id, config in ECOMMERCE_SOURCES_CONFIG.items():
        ...
        card = dbc.Card([
            dbc.CardHeader([
                ...
                dbc.Badge(f"{item_count} items", color="primary", className="float-end")
            ]),
            dbc.CardBody([
                ...
                html.Div([
                    ...
                    html.Span(f"${avg_price:.2f}" if avg_price > 0 else "Free", className="text-success")
                ], className="mb-1"),
                html.Div([
                    html.Strong("Avg Rating: "),
                    html.Span(f"{avg_rating:.1f}/5" if avg_rating > 0 else "N/A", className="text-warning")
                ], className="mb-1"),
                html.Div([
                    html.Strong("Total Sales: "),
                    html.Span(f"{total_sales:,}" if total_sales > 0 else "N/A", className="text-info")
                ], className="mb-2"),
                dbc.Button(
                    f"View {config['name']}",
...
def create_ecommerce_table(source_id, items):
    ...
    df['URL'] = df.apply(lambda row: f"[🛒 View Product]({row['URL']})" if row['URL'] != '#' else 'N/A', axis=1)
    ...
    return dash_table.DataTable(
        ...
        style_data_conditional=[
            ...
            {'if': {'filter_query': '{Price} contains Free'}, 'backgroundColor': '#3C5A3C', 'color': '#AEF4AE'}
        ]
    )
            ...
            dbc.Col([
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(f"${avg_price:.2f}", className="text-success mb-0"),
                                html.P("Average Price", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(free_items, className="text-info mb-0"),
                                html.P("Free Items", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(total_categories, className="text-warning mb-0"),
