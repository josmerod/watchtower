...
def render_add_condition_modal() -> dbc.Modal([
    dbc.ModalHeader(dbc.ModalTitle("Add Condition")),
    dbc.ModalBody([
        dbc.Label("Condition Type:", html_for="new-condition-type"),
        dbc.Select(
            id="new-condition-type",
            options=[
                {"label": "Source Match", "value": "source_match"},
                {"label": "Keyword Match", "value": "keyword_match"},
                {"label": "Category Match", "value": "category_match"},
                {"label": "Price Threshold", "value": "price_threshold"},
            ],
            value="keyword_match",
        ),
    ]),
    dbc.ModalFooter([
        dbc.Button("Cancel", id="add-condition-cancel", color="secondary", outline=True),
        dbc.Button("Add", id="add-condition-confirm", color="primary"),
    ]),
], id="add-condition-modal", is_open=False, size="md")
...
def render_quiet_hours_config() -> dbc.Container([
    ...
    dbc.Alert(
        html.Small([
            "Note: Time ranges that cross midnight (e.g., 22:00 to 08:00) are handled correctly."
        ]),
        color="info",
    ),
], fluid=True)
