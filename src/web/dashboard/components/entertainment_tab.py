...
def process_entertainment_item(item):
    ...
    try:
        ...
        description = item.get('description', item.get('synopsis', item.get('summary', 'No description available')))
        
        # Handle different timestamp formats
        timestamp = None
        for time_field in ['timestamp', 'date', 'created_at', 'showtime', 'release_date']:
        ...
        card = dbc.Card([
            ...
            dbc.CardBody([
                ...
                html.Div([
                    ...
                    html.Span(f"{avg_rating:.1f}/10" if avg_rating > 0 else "N/A", className="text-warning")
                ], className="mb-1"),
                html.Div([
...
def create_entertainment_table(source_id, items):
    ...
    sorted_items = sorted(items, key=lambda x: (x['rating'], x['timestamp'] or datetime.min.replace(tzinfo=timezone.utc)), reverse=True)
...
def render_entertainment_tab():
    ...
    total_genres = len(set(item['genre'] for item in ALL_ENTERTAINMENT if item['genre'] != 'Unknown'))
            ...
            dbc.Col([
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(f"{avg_rating:.1f}/10", className="text-success mb-0"),
                    ...
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(total_genres, className="text-info mb-0"),
                                html.P("Genres", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=3),
                    dbc.Col([
