    ...
    cards = [
        dbc.Card(
            dbc.CardBody([
                ...
                html.P(f"Last check: {last_check}", className="card-text small text-muted")
            ]),
            color=status_color, outline=True, className="mb-3"
        ),
        dbc.Card(
            dbc.CardBody([
                html.H4("📊 Watcher Health", className="card-title"),
                html.H5("Active" if watcher_state else "Unknown", 
                        className="text-success" if watcher_state else "text-warning"),
                html.P("System monitoring status", className="card-text small text-muted")
            ]),
...
def create_ms_skills_table(data):
    ...
    if not data:
        return html.Div([
            html.P("No MS Skills events available.", 
    ...
    for item in recent_data:
        ...
        row = html.Tr([
            ...
            html.Td([
                html.A(title, href=url, target="_blank", className="text-decoration-none") if url != '#' else title
            ]),
    ...
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col([
                ...
                html.P("System watchers, change detection, and monitoring intelligence", 
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader([
                        ...
                        html.Small(f"Showing latest {min(50, len(ms_skills_data))} events", className="text-muted")
                    ]),
                    dbc.CardBody([
