...
# Import shared utilities from dashboard directory
import sys
...
# Load data at import time to ensure it's available
load_videos_data()
...
def create_video_card(video_row):
    ...
    # Create thumbnail container with fallback
    if thumbnail_url and thumbnail_url.strip():
        # Use actual thumbnail URL
        thumbnail_content = html.Img(
            src=thumbnail_url,
            alt=video_row.get('title', 'Video thumbnail'),
            style={
                "maxHeight": "180px", 
                "width": "100%", 
                "objectFit": "cover",
        ...
        # Fallback placeholder that will be shown if image fails
        fallback_placeholder = html.Div([
            html.Div([
                html.I(className="fas fa-video", style={"fontSize": "2rem", "color": "#A37FFF", "marginBottom": "0.5rem"}),
                html.Div("Video Thumbnail", style={"fontSize": "0.9rem", "color": "#A37FFF"})
            ], style={
                "display": "flex",
            ...
            "display": "none",  # Hidden by default, shown by JavaScript if image fails
            "backgroundColor": "#3C3970",
            "height": "180px",
            "width": "100%",
            "borderRadius": "8px 8px 0 0",
        ...
        thumbnail_element = html.Div([thumbnail_content, fallback_placeholder])
    else:
        # No thumbnail URL - show placeholder immediately
        thumbnail_element = html.Div([
            html.Div([
                html.I(className="fas fa-video", style={"fontSize": "2rem", "color": "#A37FFF", "marginBottom": "0.5rem"}),
                html.Div("Video Thumbnail", style={"fontSize": "0.9rem", "color": "#A37FFF"})
            ], style={
...
def register_video_callbacks(app):
    ...
    def update_video_display(selected_channel, search_term, date_filter_value, current_page, page_size):
        print(f"[DEBUG] Video callback called with: channel={selected_channel}, search={search_term}, page={current_page}")
        
        # Ensure data is loaded
        if not VIDEO_DATA_LOADED or not ALL_VIDEOS_DATA:
            print(f"[DEBUG] Data not loaded, attempting to load...")
            load_videos_data()  # Try to load data if not already loaded
            
        if not VIDEO_DATA_LOADED or not ALL_VIDEOS_DATA:
            print(f"[DEBUG] Data still not available after load attempt")
            return dbc.Alert("Video data not available.", color="warning"), 1, 1
            
        print(f"[DEBUG] Data loaded successfully: {len(ALL_VIDEOS_DATA)} channels")
        ...
        print(f"[DEBUG] Created {len(video_cards)} video cards from {len(df_paginated)} videos")

        # Ensure current_page is not out of bounds after filtering
        actual_page = min(current_page, max_pages) if max_pages > 0 else 1
        
        print(f"[DEBUG] Returning: {len(video_cards)} cards, max_pages={max_pages}, actual_page={actual_page}")
