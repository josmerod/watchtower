...
def process_museum_item(item):
    ...
    try:
        return {
            ...
            'admission': item.get('admission', item.get('price', 'Contact for pricing')),
...
def create_museums_table():
    ...
    if not MUSEUMS_DATA:
        return dbc.Alert("No museums data available.", color="info", className="text-center")
    
    df_data = []
    for museum in MUSEUMS_DATA:
        exhibitions_str = ', '.join(museum['exhibitions'][:3]) if museum['exhibitions'] else 'None listed'
        ...
        row = {
            ...
            'Description': museum['description'][:150] + '...' if len(museum['description']) > 150 else museum['description'],
    ...
    df['Website'] = df.apply(lambda row: f"[🌐 Visit]({row['Website']})" if row['Website'] != '#' else 'N/A', axis=1)
...
def render_museums_tab():
    ...
    avg_rating = sum(m['rating'] for m in MUSEUMS_DATA if m['rating'] > 0) / len([m for m in MUSEUMS_DATA if m['rating'] > 0]) if any(m['rating'] > 0 for m in MUSEUMS_DATA) else 0
            ...
            dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(museum_count, className="text-primary mb-0"),
                                html.P("Total Museums", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=4),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4(f"{avg_rating:.1f}/5" if avg_rating > 0 else "N/A", className="text-success mb-0"),
                                html.P("Average Rating", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=4),
                    dbc.Col([
                        dbc.Card([
                            dbc.CardBody([
                                html.H4("Cultural", className="text-info mb-0"),
                                html.P("Focus", className="text-muted small mb-0")
                            ])
                        ])
                    ], md=4)
                ], className="mb-4")
