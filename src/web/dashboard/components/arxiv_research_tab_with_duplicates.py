...
def load_arxiv_data():
    ...
    try:
        ...
        df['duplicate_display'] = df.apply(
            lambda row: create_duplicate_badge(row), axis=1
        )

        # Process quality score
        df['quality_display'] = df.apply(
            lambda row: create_quality_badge(row), axis=1
        )
...
def create_papers_table(df_subset):
    ...
    for _, paper in df_subset.iterrows():
        ...
        # Published date cell
        published_cell = html.Td(
            paper.get('published_display', 'N/A'),
            className="small"
        )
        ...
        summary_cell = html.Td(
            html.Div(paper.get('summary_preview', 'No summary available'), className="small")
        )
        ...
        table_body_rows.append(
            html.Tr([title_cell, authors_cell, category_cell, published_cell, quality_cell, summary_cell, actions_cell],
                   className=row_class)
        )
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                ...
                html.P("Latest research papers from ArXiv with intelligent duplicate filtering and quality scoring.",
        ...
        *create_duplicate_filter_component("arxiv", "arxiv-data-store", "papers"),
        ...
        dbc.Row([
            dbc.Col([
                html.Div(id="arxiv-content", children=[
        ...
        dbc.Row([
            dbc.Col([
                html.Div(id="arxiv-loading", className="text-center my-5", children=[
                    dbc.Spinner(color="primary", size="lg"),
                    html.P("Loading ArXiv papers...", className="mt-3 text-muted")
                ])
            ])
        ])
    ])
...
def register_callbacks(app):
    ...
    @app.callback(
        [
            ...
            Output("arxiv-loading", "children")
        ],
        [
            Input("arxiv-filtered-data", "data"),
            Input("arxiv-items-per-page", "data")
        ]
    )
