...
watchtower_root = os.path.abspath(
...
)  # Reduced TTL for better data freshness
...
try:
    ...
    required_methods = [
        ...
        "get_home_server_trends_data",
        "get_ai_platforms_data",
    ]
    missing_methods = []
...
@st.cache_data(
    ttl=600, max_entries=1, show_spinner=True
)  # Reduced TTL for more frequent updates
def get_cached_data():
...
def display_anime_calendar_tab():
    ...
    season_tab, popular_tab, rated_tab = st.tabs(
        [
            f"🌸 Current Season ({len(seasonal_anime)})"
            if seasonal_anime
            else "🌸 Current Season",
            f"🔥 Top Popular ({len(popular_anime)})"
            if popular_anime
            else "🔥 Top Popular",
    ...
    with season_tab:
        display_anime_section("🌸 Current Season Anime", seasonal_anime, num_columns=3)

    with popular_tab:
        display_anime_section("🔥 Top Popular Anime", popular_anime, num_columns=3)

    with rated_tab:
        display_anime_section("⭐ Top Rated Anime", rated_anime, num_columns=3)
...
with main_tabs[3]:  # Google Cloud Blog
    render_tab_safely(
        "Google Cloud Blog",
        google_cloud_blog_tab.render,
        logger,
        google_cloud_blog_data,
    )

with main_tabs[4]:  # AWS Training
    render_tab_safely(
        "AWS Training", aws_training_tab.render, logger, aws_training_data
    )

with main_tabs[5]:  # Azure Training
    render_tab_safely(
        "Azure Training", azure_training_tab.render, logger, azure_training_data
    )
...
with main_tabs[6]:  # Juegos
    ...
    if isinstance(games_data, tuple) and len(games_data) == 4:
        ...
        logger.info(
            f"Games data loaded: deals={len(deals_df)}, bundles={len(bundles_df)}, giveaways={len(giveaways_df)}, trending={len(trending_df)}"
        )
    elif isinstance(games_data, tuple) and len(games_data) == 3:
        deals_df, bundles_df, giveaways_df = games_data
        trending_df = pd.DataFrame()
        logger.info(
            f"Games data loaded (3-tuple): deals={len(deals_df)}, bundles={len(bundles_df)}, giveaways={len(giveaways_df)}"
        )
    else:
        deals_df = bundles_df = giveaways_df = trending_df = pd.DataFrame()
        logger.warning(
            f"Games data format unexpected: {type(games_data)} - creating empty dataframes"
        )
...
with main_tabs[7]:  # AllKeyShop Deals
    render_tab_safely(
        "AllKeyShop Deals", allkeyshop_tab.render, allkeyshop_data, logger
    )
...
with main_tabs[12]:  # Innovación
    render_tab_safely(
        "Innovación", enhanced_innovation_tab.render, logger, data_service
    )
...
with main_tabs[16]:  # ArXiv
    ...
    with arxiv_subtabs[0]:
        render_tab_safely(
            "ArXiv Mejorado", enhanced_arxiv_papers.display_enhanced_papers
        )
