...
class DataService:
    ...
    def __init__(self, logger=None):
        ...
        Args:
            logger: Optional logger instance. If None, logging will be disabled
                    or use a default logger if implemented in _log.
        """
        self.logger = logger
    ...
    def get_games_data(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            deals = self._safe_load_json(deals_file, "game deals")
            if deals:
            ...
            regular_bundles = self._safe_load_json(bundles_file, "regular bundles")
            if regular_bundles:
            ...
            humble_bundles = self._safe_load_json(humble_file, "humble bundles")
            if humble_bundles:
            ...
            giveaways = self._safe_load_json(giveaways_file, "game giveaways")
            if giveaways:
            ...
            self._log(f"Error loading games data: {e!s}", "error")
    ...
    def get_courses_data(self) -> dict[str, pd.DataFrame]:
        ...
        self._log("Loading courses data")

        courses_data = {}

        try:
            # Coursera courses
            coursera_file = self.data_dir / "classcentral" / "coursera_courses.json"
            coursera_data = self._safe_load_json(coursera_file, "Coursera courses")
            if coursera_data:
                courses_data["coursera"] = pd.DataFrame(coursera_data)
                self._log(f"Loaded {len(courses_data['coursera'])} Coursera courses")

            # Check for other course platforms
            udemy_file = self.data_dir / "udemy" / "udemy_courses.json"
            if udemy_file.exists():
                udemy_data = self._safe_load_json(udemy_file, "Udemy courses")
                if udemy_data:
                    courses_data["udemy"] = pd.DataFrame(udemy_data)
                    self._log(f"Loaded {len(courses_data['udemy'])} Udemy courses")

        except Exception as e:
            self._log(f"Error loading courses data: {e!s}", "error")
    ...
    def get_news_data(self) -> dict[str, pd.DataFrame]:
        ...
        self._log("Loading news data")

        news_data = {}

        try:
            # HackerNews - check multiple possible file names
            hn_dir = self.data_dir / "hackernews"
            ...
            ft_dir = self.data_dir / "futuretools"
            ...
            # Medium GenAI
            medium_file = self.data_dir / "medium_genai" / "articles.json"
            medium_data = self._safe_load_json(medium_file, "Medium GenAI articles")
            if medium_data:
                news_data["medium"] = pd.DataFrame(medium_data)
                self._log(f"Loaded {len(news_data['medium'])} Medium articles")
    ...
    def get_videos_data(self) -> dict[str, pd.DataFrame]:
        ...
        try:
            ...
            self._log(f"Error loading videos data: {e!s}", "error")
    ...
    def get_video_categories(self) -> dict[str, str]:
        ...
        self._log("Loading video categories")
        ...
        try:
            ...
            self._log(f"Error loading video categories: {e!s}", "error")
    ...
    def get_arxiv_data(self) -> pd.DataFrame:
        ...
        self._log("Loading ArXiv data")

        try:
            # Try CSV first
            arxiv_file = self.data_dir / "arxiv" / "processed" / "csv" / "papers.csv"
            if arxiv_file.exists():
                arxiv_df = pd.read_csv(arxiv_file)
                self._log(f"Loaded {len(arxiv_df)} ArXiv papers from CSV")
                return arxiv_df

            # Try JSON format
            arxiv_json = self.data_dir / "arxiv" / "processed" / "json" / "papers.json"
            arxiv_data = self._safe_load_json(arxiv_json, "ArXiv papers")
            if arxiv_data:
                arxiv_df = pd.DataFrame(arxiv_data)
                self._log(f"Loaded {len(arxiv_df)} ArXiv papers from JSON")
                return arxiv_df
            ...
            self._log(f"Error loading ArXiv data: {e!s}", "error")
    ...
    def get_events_data(self) -> pd.DataFrame:
        ...
        try:
            events_file = self.data_dir / "valencia_events" / "events.json"
            events_data = self._safe_load_json(events_file, "Valencia events")
            if events_data:
                events_df = pd.DataFrame(events_data)
                self._log(f"Loaded {len(events_df)} Valencia events")
                return events_df

        except Exception as e:
            self._log(f"Error loading events data: {e!s}", "error")
