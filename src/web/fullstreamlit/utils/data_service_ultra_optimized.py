...
class UltraOptimizedDataService:
    ...
    def _get_cache_key(self, file_path: str, operation: str) -> str:
        ...
        try:
            ...
            if path_obj.exists():
                ...
                return hashlib.md5(
                    f"{file_path}_{operation}_{stat.st_size}_{stat.st_mtime}".encode()
                ).hexdigest()
        ...
        try:
            if not file_path.exists():
                self._track_performance(
                    f"load_{file_path.name}", time.time() - start_time, False
                )
                return []
            ...
            if isinstance(data, list):
                ...
                self._track_performance(
                    f"load_{file_path.name}", time.time() - start_time, True
                )
                return data
                ...
                self._log(
                    f"Unexpected data type in {file_path}: {type(data)}", "warning"
                )
                self._track_performance(
                    f"load_{file_path.name}", time.time() - start_time, False
                )
    ...
    def get_games_data_ultra(
        ...
        for op_name, file_path in file_operations:
            ...
            self._log(
                f"🔍 DEBUG: Loaded {len(data) if data else 0} items for {op_name}"
            )
        ...
        self._log(
            f"🔍 DEBUG: Processing deals, raw data length: {len(loaded_data.get('deals', []))}"
        )
        if loaded_data["deals"]:
            ...
            if not deals_df.empty:
                ...
                if (
                    "published" in deals_df.columns
                    and "published_date" not in deals_df.columns
                ):
                    deals_df["published_date"] = deals_df["published"]
                    deals_df = deals_df.drop("published", axis=1)

                deals_df = self._optimize_dataframe_dtypes(
                    deals_df, {"published_date": "datetime", "price": "float"}
                )
                self._log(
                    f"🔍 DEBUG: Deals DataFrame before cleaning: {deals_df.shape}"
                )
                # TEMPORARILY DISABLED: deals_df = clean_dataframe_for_caching(deals_df)
                self._log(
                    f"🔍 DEBUG: Deals DataFrame after cleaning (SKIPPED): {deals_df.shape}"
                )

        # Process bundles (combine regular and humble)
        bundles_data = []
        self._log(
            f"🔍 DEBUG: Processing bundles, raw bundles: {len(loaded_data.get('bundles', []))}, humble: {len(loaded_data.get('humble', []))}"
        )
        ...
        if bundles_data:
            ...
            self._log(
                f"🔍 DEBUG: Created bundles DataFrame with shape: {bundles_df.shape}"
            )
            if not bundles_df.empty:
                ...
                bundles_df = self._optimize_dataframe_dtypes(
                    bundles_df, {"published_date": "datetime"}
                )
                self._log(
                    f"🔍 DEBUG: Bundles DataFrame before cleaning: {bundles_df.shape}"
                )
                # TEMPORARILY DISABLED: bundles_df = clean_dataframe_for_caching(bundles_df)
                self._log(
                    f"🔍 DEBUG: Bundles DataFrame after cleaning (SKIPPED): {bundles_df.shape}"
                )

        # Process giveaways
        giveaways_df = pd.DataFrame()
        self._log(
            f"🔍 DEBUG: Processing giveaways, raw data length: {len(loaded_data.get('giveaways', []))}"
        )
        if loaded_data["giveaways"]:
            giveaways_df = pd.DataFrame(loaded_data["giveaways"])
            self._log(
                f"🔍 DEBUG: Created giveaways DataFrame with shape: {giveaways_df.shape}"
            )
                ...
                for col in ["published_date", "expires_date"]:
                    if col in giveaways_df.columns:
                        ...
                        if giveaways_df[col].dtype in ["int64", "float64"]:
                            ...
                            giveaways_df[col] = pd.to_datetime(
                                giveaways_df[col], unit="ms", errors="coerce"
                            ).dt.strftime("%Y-%m-%d %H:%M:%S")

                giveaways_df = self._optimize_dataframe_dtypes(
                    giveaways_df,
                    {"published_date": "datetime", "expires_date": "datetime"},
                )
                self._log(
                    f"🔍 DEBUG: Giveaways DataFrame before cleaning: {giveaways_df.shape}"
                )
                # TEMPORARILY DISABLED: giveaways_df = clean_dataframe_for_caching(giveaways_df)
                self._log(
                    f"🔍 DEBUG: Giveaways DataFrame after cleaning (SKIPPED): {giveaways_df.shape}"
                )
        ...
        if loaded_data.get("itchio"):
            ...
            if not itchio_df.empty:
                ...
                self._log(
                    f"🔍 DEBUG: Itch.io DataFrame cleaning (SKIPPED): {itchio_df.shape}"
                )

        self._log(
            f"🔍 DEBUG: Ultra-loaded: {len(deals_df)} deals, {len(bundles_df)} bundles, {len(giveaways_df)} giveaways, {len(itchio_df)} itch.io trending"
        )
        self._log(
            f"🔍 DEBUG: Returning tuple with shapes: {deals_df.shape}, {bundles_df.shape}, {giveaways_df.shape}, {itchio_df.shape}"
        )
        return deals_df, bundles_df, giveaways_df, itchio_df

    @st.cache_data(
        ttl=1800, max_entries=10, show_spinner=False, hash_funcs=SAFE_HASH_FUNCS
    )
                ...
                for filename in video_files:
                    ...
                    if videos_file.exists():
                        cache_key = self._get_cache_key(
                            str(videos_file), f"videos_{channel_dir.name}"
                        )
                        channel_data = self._ultra_fast_json_load(
                            videos_file, cache_key
                        )
                        ...
                        if channel_data:
                            ...
                            if not df.empty:
                                ...
                                self._log(
                                    f"Ultra-loaded {len(df)} videos from {channel_dir.name}"
                                )
                        break
    ...
    def get_news_data_ultra(self) -> dict[str, pd.DataFrame]:
        ...
        news_sources = [
            (
                "hackernews",
                self.cached_paths["hackernews_dir"],
                ["stories.json", "hackernews.json", "hackernews_simple.json"],
            ),
            (
                "futuretools",
                self.cached_paths["futuretools_dir"],
                ["news.json", "futuretoolsnews.json"],
            ),
        ...
        for source_name, source_dir, file_names in news_sources:
            ...
            for filename in file_names:
                ...
                if file_path.exists():
                    cache_key = self._get_cache_key(
                        str(file_path), f"news_{source_name}"
                    )
                    data = self._ultra_fast_json_load(file_path, cache_key)
                    ...
                    if data:
                        ...
                        if not df.empty:
                            df = self._optimize_dataframe_dtypes(
                                df, {"published_date": "datetime"}
                            )
                            df = clean_dataframe_for_caching(df)
    ...
    @st.cache_data(
        ttl=3600, max_entries=5, show_spinner=False, hash_funcs=SAFE_HASH_FUNCS
    )
    def get_courses_data_ultra(self) -> dict[str, pd.DataFrame]:
        ...
        course_sources = [
            ...
            (
                "deeplearningai",
                self.cached_paths["deeplearningai_dir"] / "deeplearningai_courses.json",
            ),
        ]
        ...
        for source_name, file_path in course_sources:
            if file_path.exists():
                cache_key = self._get_cache_key(
                    str(file_path), f"courses_{source_name}"
                )
                data = self._ultra_fast_json_load(file_path, cache_key)
    ...
    def _optimize_dataframe_dtypes(
        ...
        for column, dtype in type_mapping.items():
            if column in optimized_df.columns:
                try:
                    if dtype == "datetime":
                        optimized_df[column] = pd.to_datetime(
                            optimized_df[column], errors="coerce"
                        )
                        ...
                        if optimized_df[column].max() < np.finfo(np.float32).max:
                            optimized_df[column] = optimized_df[column].astype(
                                np.float32
                            )
                    elif dtype == "category":
    ...
    @st.cache_data(
        ttl=600, show_spinner=False, hash_funcs=SAFE_HASH_FUNCS
    )  # 10 minute cache for summary
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "streamlit_cache_stats": "Available"
    ...
    def get_games_data(
        ...
        if isinstance(result, tuple):
            self._log(
                f"🔍 DEBUG: get_games_data wrapper tuple lengths: {[len(df) if hasattr(df, '__len__') else 'no len' for df in result]}"
            )
        return result
    ...
    def get_enhanced_arxiv_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            possible_paths = [
                self.data_dir
                / "streamlit_enhanced_arxiv"
                / "output"
                / "latest_enhanced_papers.json",
                self.data_dir
                / "enhanced_arxiv"
                / "output"
                / "latest_enhanced_papers.json",
    ...
    def get_new_game_releases_data(self) -> pd.DataFrame:
        ...
        if not games_dir or not games_dir.exists():
            self._log(
                "Games directory not found in cached_paths or does not exist.",
                "warning",
            )
            return pd.DataFrame()
        ...
        if not file_path.exists():
            self._log(
                f"New releases file not found at {file_path}. Returning empty DataFrame.",
                "warning",
            )
            return pd.DataFrame()
        ...
        try:
            ...
            self._log(
                f"Successfully loaded and processed new game releases data. Shape: {df.shape}",
                "info",
            )
            return df
        except Exception as e:
            self._log(
                f"Error converting new game releases data to DataFrame: {e}", "error"
            )
            return pd.DataFrame()
    ...
    def get_google_cloud_blog_data(self) -> list[dict[str, Any]]:
        ...
        if not gcb_file_path.exists():
            self._log(
                f"Google Cloud Blog data file not found: {gcb_file_path}", "error"
            )
            return []
        ...
        if cache_key in self.memory_cache:
            self._log(
                f"Returning cached Google Cloud Blog data for key: {cache_key}", "debug"
            )
            return self.memory_cache[cache_key]
        ...
        try:
            ...
            if not isinstance(data, list):
                self._log(
                    f"Google Cloud Blog data is not a list: {type(data)}", "warning"
                )
            ...
            FileNotFoundError
        ):  # This case is technically covered by the gcb_file_path.exists() check
            self._log(
                f"Google Cloud Blog data file not found during read: {gcb_file_path}",
                "error",
            )
            return []
            ...
            self._log(
                f"An unexpected error occurred while reading {gcb_file_path}: {e}",
                "error",
            )
            return []
    ...
    def get_aws_training_data(self) -> list[dict[str, Any]]:
        ...
        if cache_key in self.memory_cache:
            self._log(
                f"Returning cached AWS Training data for key: {cache_key}", "debug"
            )
            return self.memory_cache[cache_key]
        ...
        try:
            ...
            self._log(
                f"An unexpected error occurred while reading {file_path}: {e}", "error"
            )
            return []
    ...
    def get_azure_training_data(self) -> list[dict[str, Any]]:
        ...
        if cache_key in self.memory_cache:
            self._log(
                f"Returning cached Azure Training data for key: {cache_key}", "debug"
            )
            return self.memory_cache[cache_key]
    ...
    def get_tech_events_intelligence(self) -> dict[str, Any]:
        ...
        try:
            ...
            events_file = (
                self.data_dir / "tech_conference" / "output" / "tech_events_latest.json"
            )
            ...
            avg_quality = sum(
                e.get("quality_score", 0) for e in processed_events
            ) / max(total_events, 1)
            avg_relevance = sum(
                e.get("relevance_score", 0) for e in processed_events
            ) / max(total_events, 1)
            avg_networking = sum(
                e.get("networking_score", 0) for e in processed_events
            ) / max(total_events, 1)
            avg_roi = sum(e.get("roi_score", 0) for e in processed_events) / max(
                total_events, 1
            )
            ...
            for event in processed_events:
                ...
                format_distribution[format_type] = (
                    format_distribution.get(format_type, 0) + 1
                )
        ...
        return {
            ...
            "statistics": {
                ...
                "high_quality_count": len(
                    [e for e in demo_events if e["quality_score"] >= 75]
                ),
    ...
    def get_security_intelligence(self) -> dict[str, Any]:
        ...
        try:
            ...
            with_patches = sum(
                1 for v in vulnerabilities if v.get("patch_available", False)
            )
            patch_availability = (
                (with_patches / len(vulnerabilities) * 100) if vulnerabilities else 0
            )
            ...
            return {
                ...
                "high_count": sum(
                    1 for v in vulnerabilities if v.get("risk_level") == "high"
                ),
                "medium_count": sum(
                    1 for v in vulnerabilities if v.get("risk_level") == "medium"
                ),
                "low_count": sum(
                    1 for v in vulnerabilities if v.get("risk_level") == "low"
                ),
                "with_exploits": sum(
                    1 for v in vulnerabilities if v.get("exploit_available", False)
                ),
                "needs_urgent_attention": sum(
                    1 for v in vulnerabilities if v.get("needs_urgent_attention", False)
                ),
                "recent_vulnerabilities": sum(
                    1 for v in vulnerabilities if v.get("is_recent", False)
                ),
            }
    ...
    def get_github_trends(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if gh_dir:
                for filename in [
                    "github_trends_latest.json",
                    "github_trending_latest.json",
                ]:
                    ...
                    if gh_file.exists():
                        ...
                        data = self._ultra_fast_json_load(gh_file, cache_key)
                        self._log(
                            f"Loaded {len(data)} GitHub repositories from {filename}"
                        )
                        return data
    ...
    def _update_cache_expiry(self) -> None:
        ...
        self._cache_expiry = datetime.utcnow() + timedelta(
    ...
    async def get_technology_radar(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and "technology_radar" in self._tech_intelligence_cache
            ):
            ...
            # Compile results
            radar_data = {
                "framework_battles": self._serialize_framework_battles(
                    framework_battles
                ),
                "adoption_predictions": self._serialize_predictions(
            ...
            self._log(
                f"Technology radar generated with {len(framework_battles)} battles and {len(adoption_predictions)} predictions"
            )
            return radar_data
            ...
            try:
                ...
                for framework in battle.frameworks:
                    framework_data = {
                        ...
                        if hasattr(framework.maturity_level, "value")
                        else str(framework.maturity_level),
        ...
        for tech_name, prediction in predictions.items():
            try:
                prediction_data = {
                    ...
                    if hasattr(prediction.current_adoption_level, "value")
                    else str(prediction.current_adoption_level),
                    "predicted_score": prediction.predicted_score,
                    "predicted_adoption_level": prediction.predicted_adoption_level.value
                    if hasattr(prediction.predicted_adoption_level, "value")
                    else str(prediction.predicted_adoption_level),
                    "growth_rate": prediction.growth_rate,
                    "trend_direction": prediction.trend_direction.value
                    if hasattr(prediction.trend_direction, "value")
                    else str(prediction.trend_direction),
                ...
                self._log(
                    f"Failed to serialize prediction for {tech_name}: {e}", "warning"
                )
                continue
            ...
            for tech_name, prediction in predictions.items():
                ...
                if "strong buy" in investment_rec:
                    ...
                    recommendations["investment_grades"]["hold"].append(
                        {"technology": tech_name, "reason": prediction.recommendation}
                    )
                ...
                trend_direction = (
                    prediction.trend_direction.value
                    if hasattr(prediction.trend_direction, "value")
                    else str(prediction.trend_direction)
                )
                if (
                    trend_direction in ["rising", "explosive"]
    ...
    def _analyze_market_trends(self, battles, predictions) -> dict[str, Any]:
        ...
        try:
            ...
            for name, pred in predictions.items():
                trend_direction = (
                    pred.trend_direction.value
                    if hasattr(pred.trend_direction, "value")
                    else str(pred.trend_direction)
                )
            ...
            for tech_name, prediction in predictions.items():
                lifecycle_stage = (
                    prediction.current_adoption_level.value
                    if hasattr(prediction.current_adoption_level, "value")
                    else str(prediction.current_adoption_level)
                )
                trend_direction = (
    ...
    def get_home_server_trends_data(self):
        file_path = (
            self.cached_paths["home_server_trends_dir"]
            / "home_server_trends_latest.json"
        )
        if not file_path.exists():
        ...
        try:
            ...
            self._log(
                f"Error loading home server trends data from {file_path}: {e}", "error"
            )
            return []
        ...
        try:
            ...
            if allkeyshop_dir.exists():
                ...
                if json_files:
                    ...
                    latest_timestamped = max(
                        json_files, key=lambda x: x.stat().st_mtime
                    )
                    cache_key = self._get_cache_key(
                        str(latest_timestamped), "allkeyshop_timestamped"
                    )
                    data = self._ultra_fast_json_load(latest_timestamped, cache_key)
                    self._log(
                        f"Loaded {len(data)} AllKeyShop games from timestamped file: {latest_timestamped.name}"
                    )
                    return data
    ...
    @st.cache_data(
        ttl=1800, max_entries=5, show_spinner=False, hash_funcs=SAFE_HASH_FUNCS
    )
    def get_museum_data(self) -> pd.DataFrame:
        ...
        if not museums_output_dir.exists():
            self._log(
                f"Museums output directory not found: {museums_output_dir}", "warning"
            )
            return pd.DataFrame()
        ...
        try:
            ...
            if not json_files:
                self._log(
                    f"No museum JSON files found in {museums_output_dir}", "warning"
                )
                return pd.DataFrame()
            ...
            self._log(
                f"Error finding latest museum file in {museums_output_dir}: {e}",
                "error",
            )
            return pd.DataFrame()
        ...
        if not data:
            self._log(
                f"No data loaded from {latest_file_path} or file is empty.", "warning"
            )
            return pd.DataFrame()
        ...
        try:
            ...
            df = clean_dataframe_for_caching(
                df
            )  # Apply cleaning for cache compatibility

            self._log(
                f"Successfully loaded and processed {len(df)} museum entries from {latest_file_path}."
            )
            return df

        except Exception as e:
            self._log(
                f"Error processing museum data from {latest_file_path} into DataFrame: {e}",
                "error",
            )
            return pd.DataFrame()
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        try:
            total_files = sum(
                1 for meta in self.file_metadata.values() if meta.get("exists", False)
            )
            ...
            if self._performance_stats["load_times"]:
                avg_load_time = sum(
                    self._performance_stats["load_times"].values()
            ...
            return {
                ...
                "memory_usage_mb": sum(len(str(v)) for v in self.memory_cache.values())
                / (1024 * 1024),
            }
        except Exception as e:
    ...
    def get_ai_platforms_data(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                self._log(
                    f"AI platforms data is an integer ({data}), returning empty list",
                    "warning",
                )
                return []
            elif not isinstance(data, list):
                self._log(
                    f"AI platforms data is unexpected type {type(data)}, converting to empty list",
                    "warning",
                )
                return []
