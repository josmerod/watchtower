...
def display_enhanced_papers():
    ...
    with col2:
        st.selectbox("📅 Days Back", [1, 3, 7, 14, 30], index=2)

    with col3:
        st.selectbox("📊 Max Papers", [50, 100, 200, 500], index=1)

    with col4:
        view_mode = st.selectbox(
            "👁️ View Mode", ["Intelligence", "Standard", "Breakdown"], index=0
        )
    ...
    if not enhanced_papers:
        st.info(
            "📝 No enhanced papers found. Run the Enhanced ETL to generate intelligent analysis."
        )
...
def run_enhanced_etl():
    ...
    try:
        ...
        st.error(f"❌ ETL failed: {e!s}")
        st.exception(e)


def load_enhanced_papers() -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        st.error(f"Error loading enhanced papers: {e!s}")
        return None
...
def display_enhanced_statistics(papers: list[dict[str, Any]]):
    ...
    high_impact_papers = sum(
        1 for p in papers if p.get("industry_impact_score", 0) >= 7.0
    )
    commercial_ready = sum(
        1 for p in papers if p.get("commercial_potential") in ["high", "medium"]
    )

    # Average scores
    avg_impact = (
        sum(p.get("industry_impact_score", 0) for p in papers) / total_papers
        if total_papers
        else 0
    )
    avg_innovation = (
        sum(p.get("innovation_score", 0) for p in papers) / total_papers
        if total_papers
        else 0
    )
    avg_citation = (
        sum(p.get("citation_potential", 0) for p in papers) / total_papers
        if total_papers
        else 0
    )
    avg_overall = (
        sum(p.get("overall_significance_score", 0) for p in papers) / total_papers
        if total_papers
        else 0
    )
    ...
    with col2:
        st.metric(
            ...
            delta=f"{(breakthrough_papers / total_papers * 100):.1f}%"
            if total_papers
            else "0%",
    ...
    with col3:
        st.metric(
            ...
            delta=f"{(high_impact_papers / total_papers * 100):.1f}%"
            if total_papers
            else "0%",
    ...
    with col4:
        st.metric(
            ...
            delta=f"{(commercial_ready / total_papers * 100):.1f}%"
            if total_papers
            else "0%",
...
def display_intelligence_view(papers: list[dict[str, Any]]):
    ...
    with col2:
        commercial_filter = st.selectbox(
            "Commercial Potential", ["All", "High", "Medium", "Low", "Research"]
        )
    ...
    if min_impact > 0:
        filtered_papers = [
            ...
            if p.get("industry_impact_score", 0) >= min_impact
        ]
    ...
    if commercial_filter != "All":
        filtered_papers = [
            p
            for p in filtered_papers
            if p.get("commercial_potential", "").lower() == commercial_filter.lower()
        ]

    if breakthrough_only:
        filtered_papers = [
            p for p in filtered_papers if p.get("is_breakthrough", False)
        ]
    ...
    filtered_papers.sort(
        key=lambda x: x.get("overall_significance_score", 0), reverse=True
    )
...
def display_enhanced_paper_card(paper: dict[str, Any], rank: int):
    ...
    with st.container():
        ...
        with col1:
            ...
            st.markdown(
                f"**#{rank} {paper.get('title', 'Unknown Title')}** {breakthrough_badge}"
            )

        with col2:
            st.metric(
                "Significance", f"{paper.get('overall_significance_score', 0):.1f}/10"
            )
        ...
        with score_cols[4]:
            ...
            color_map = {"high": "🟢", "medium": "🟡", "low": "🔴", "research": "🔵"}
...
def display_breakdown_view(papers: list[dict[str, Any]]):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🏆 Top Papers", "📊 Distributions", "🔬 Technology Analysis", "📈 Trends"]
    )
...
def display_top_papers_analysis(papers: list[dict[str, Any]]):
    ...
    with col1:
        ...
        top_impact = sorted(
            papers, key=lambda x: x.get("industry_impact_score", 0), reverse=True
        )[:5]
    ...
    with col2:
        ...
        top_innovation = sorted(
            papers, key=lambda x: x.get("innovation_score", 0), reverse=True
        )[:5]
...
def display_trends_analysis(papers: list[dict[str, Any]]):
    ...
    for trend, scores in trend_averages.items():
        if scores:  # Only show trends with data
            ...
            st.write(
                f"• **{trend_name}**: {avg_score:.1f}/10 (in {papers_count} papers)"
            )
