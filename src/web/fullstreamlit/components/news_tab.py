...
def render(logger=None):
    ...
    if (
        futuretools_news_df.empty
        and bensbites_news_df.empty
        and medium_news_df.empty
        and kdnuggets_news_df.empty
        and gooddevs_df.empty
        and meneame_general_df.empty
        and meneame_tecnologia_df.empty
        and podcasts_df.empty
        and ycombinator_news_df.empty
    ):
        st.warning("📭 No hay noticias disponibles para mostrar.")
        st.info(
            "💡 **Sugerencia:** Ejecuta los procesos ETL para recopilar noticias actualizadas."
        )
        ...
        with col1:
            if st.button(
                ...
                help="Ejecuta FutureTools, HackerNews, Medium",
            ):
                st.info("Ejecutando ETL principal... esto puede tomar unos minutos.")

        with col2:
            if st.button(
                "📡 Ejecutar ETL Completo", help="Ejecuta todos los ETL de noticias"
            ):
    ...
    if (
        st.session_state.active_df_for_export is not None
        and not st.session_state.active_df_for_export.empty
        and st.session_state.active_source_name
    ):
        st.markdown("---")
        ...
        if source_name == "ft_bensbites":
            display_source_title = (
                "FutureTools & Ben's Bites"  # Custom title for this specific case
            )
        ...
        if not combined_news_df.empty:
            ...
            if "published_date" not in combined_news_df.columns:
                ...
                if "published_at" in combined_news_df.columns:
                    combined_news_df["published_date"] = pd.to_datetime(
                        combined_news_df["published_at"], errors="coerce"
                    )
                else:
            ...
            combined_news_df = combined_news_df.sort_values(
                "published_date", ascending=False
            )

            # Format date for display - ensure it's a string to avoid column type issues
            combined_news_df["published_display"] = (
                combined_news_df["published_date"]
            ...
            df_for_editor = combined_news_df.rename(
                columns={
                    "title": "Título",
                    "source": "Fuente",
            ...
            cols_to_display = [
                col for col in required_cols if col in df_for_editor.columns
            ]
            ...
            st.data_editor(
                ...
                column_config={
                    "Título": st.column_config.TextColumn(
                        width="medium", help="Título del artículo"
                    ),
                    "Fuente": st.column_config.TextColumn(
                        width="small", help="Fuente de la noticia"
                    ),
                    "Fecha de Publicación": st.column_config.TextColumn(
                        width="small", help="Fecha de publicación original"
                    ),
            ...
            source_name_for_download = (
                "ft_bensbites"  # Must be defined in else branch too
            )
...
def render_hackernews(ycombinator_news_df):
    ...
    if ycombinator_news_df.empty:
        ...
        if "published_date" not in ycombinator_news_df.columns:
            ...
            if "published_at" in ycombinator_news_df.columns:
                ycombinator_news_df["published_date"] = pd.to_datetime(
                    ycombinator_news_df["published_at"], errors="coerce"
                )
            else:
                # Create a default date column
        ...
        # Sort by published_date (now guaranteed to exist)
        display_news_df = ycombinator_news_df.sort_values(
            "published_date", ascending=False
        )

        # Format date for display - ensure it's a string to avoid column type issues
        display_news_df["published_display"] = (
            display_news_df["published_date"].dt.strftime("%Y-%m-%d %H:%M").astype(str)
        )

        # Prepare DataFrame for st.data_editor
        source_name_for_download = "hackernews"  # Define unique source name
        df_for_editor = display_news_df.rename(
            columns={
                "title": "Título",
                "source": "Fuente",
                "published_display": "Fecha de Publicación",
    ...
    if medium_news_df.empty:
        ...
        if "published_date" not in display_news_df.columns:
            ...
            if "published_at" in display_news_df.columns:
                display_news_df["published_date"] = pd.to_datetime(
                    display_news_df["published_at"], errors="coerce"
                )
        ...
        if (
            "published_date" in display_news_df.columns
            and pd.api.types.is_datetime64_any_dtype(display_news_df["published_date"])
        ):
            date_column_to_format = "published_date"
        elif (
            "published_at" in display_news_df.columns
            and pd.api.types.is_datetime64_any_dtype(display_news_df["published_at"])
        ):
            date_column_to_format = "published_at"

        if date_column_to_format:
            display_news_df["formatted_date"] = display_news_df[
                date_column_to_format
            ].dt.strftime("%Y-%m-%d %H:%M")
        ...
        df_for_editor = display_news_df.rename(
            columns={
                ...
                "source": "Fuente",
                date_col_for_editor
                if date_col_for_editor
                else "non_existent_date_col": "Fecha de Publicación",
                "url": "URL_Enlace",
            }
        )
        ...
        if (
            not df_for_editor.empty
        ):  # Check if df_for_editor itself is empty after selection/rename
            st.session_state.active_df_for_export = df_for_editor
            st.session_state.active_source_name = source_name_for_download
...
def render_kdnuggets(kdnuggets_news_df):
    ...
    if kdnuggets_news_df.empty:
        ...
        df_for_editor = display_df.rename(
            columns={
                "title": "Título",
                "source": "Fuente",
                "published_display": "Fecha de Publicación",
...
def render_gooddevs(gooddevs_df):
    ...
    if gooddevs_df.empty:
        ...
        if "published_at" in display_df.columns:
            try:
                ...
                display_df["published_at_parsed"] = pd.to_datetime(
                    display_df["published_at"], errors="coerce", utc=True
                )
                ...
                if not display_df.empty:
                    ...
                    display_df["formatted_date"] = display_df[
                        "published_at_parsed"
                    ].dt.strftime("%Y-%m-%d %H:%M")
            ...
            try:
                display_df["published_date"] = pd.to_datetime(
                    display_df["published_date"], errors="coerce"
                )
                # Remove rows with invalid dates
        ...
        # Base column mapping
        column_mapping = {"title": "Título", "source": "Fuente", "url": "URL_Enlace"}
        ...
        if not df_for_editor.empty:
            ...
            with col2:
                if date_col_present:
                    st.metric(
                        "📅 Date Coverage",
                        f"{len(df_for_editor)}/{len(gooddevs_df)} parsed",
                    )
                else:
                    st.metric("⚠️ Date Issues", "No valid dates")
            with col3:
                unique_sources = (
                    display_df["source"].nunique()
...
def render_meneame_general(df):
    ...
    if df.empty:
        ...
        display_df["formatted_date"] = display_df["published_date"].dt.strftime(
            "%Y-%m-%d"
        )
...
def render_podcasts(podcasts_df):
    ...
    if podcasts_df.empty:
        ...
        df["Fecha de Publicación"] = df.get("published_at", pd.Series()).apply(
            safe_format
        )
        df = df.sort_values("Fecha de Publicación", ascending=False)

        source_name_for_download = "podcasts"  # Define unique source name
        df_for_editor = df.rename(
            columns={"title": "Título", "source": "Fuente", "url": "URL_Enlace"}
        )
        ...
        st.data_editor(
            ...
            column_config={
                "Título": st.column_config.TextColumn(
                    width="medium", help="Título del episodio"
                ),
                "Fuente": st.column_config.TextColumn(
                    width="small", help="Fuente del podcast"
                ),
                "Fecha de Publicación": st.column_config.TextColumn(
                    width="small", help="Fecha de publicación original"
                ),
