...
def render_anime_item(anime: AnimeItem, col_index: int):
    ...
    with st.container():
        ...
        with col3:
            st.metric(
                label="Popularity",
                value=f"#{anime.popularity}" if anime.popularity else "N/A",
            )
...
def display_anime_section(
    title: str, anime_list: list[AnimeItem], num_columns: int = 3
):
    ...
    if not anime_list:
        st.info(
            "No data available for this section or an error occurred while loading."
        )
        return
