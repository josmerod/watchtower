...
def update_theme_config(theme_name: str) -> bool:
    ...
    try:
        ...
        st.error(f"Error updating theme: {e!s}")
        return False
...
def render(logger):
    ...
    with tab1:
        ...
        try:
            ...
            st.error(f"Error loading statistics: {e!s}")
        ...
        if os.path.exists(user_profiles_dir):
            ...
            for profile_file in profile_files:
                try:
                    ...
                    st.error(f"Error loading profile {profile_file}: {e!s}")
    ...
    with tab3:
        ...
        if st.button("Run ArXiv ETL"):
            ...
            try:
                ...
                st.error(f"Error running ArXiv ETL: {e!s}")
        ...
        if st.button("Run Humble Bundle ETL"):
            ...
            try:
                ...
                st.error(f"Error running Humble Bundle ETL: {e!s}")
        ...
        if st.button("Run Subreddits ETL"):
            ...
            try:
                ...
                st.error(f"Error running Subreddits ETL: {e!s}")
        ...
        if st.button("Run Media RSS ETL"):
            ...
            try:
                ...
                st.error(f"Error running Media RSS ETL: {e!s}")
        ...
        if st.button("Clear All User Profiles"):
            ...
            if os.path.exists(user_profiles_dir):
                try:
                    ...
                    st.error(f"Error removing profiles: {e!s}")
            else:
