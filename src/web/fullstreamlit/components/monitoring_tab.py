...
class MonitoringDataCollector:
    ...
    def get_etl_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if etl_metrics['total_runs'] > 0:
                etl_metrics['success_rate'] = (etl_metrics['successful_runs'] / etl_metrics['total_runs']) * 100
            else:
    ...
    def _parse_etl_log(self, content: str, etl_name: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if 'completed successfully' in line.lower() or 'etl process completed' in line.lower():
                metrics['successes'] += 1
                metrics['runs'] += 1
            ...
            # Count failures
                ...
                metrics['failures'] += 1
                if 'error' in line.lower() and len(metrics['errors']) < 5:  # Keep last 5 errors
                    metrics['errors'].append(line.strip())
            
            # Extract duration information
            duration_match = re.search(r'duration.*?(\d+\.?\d*)\s*seconds?', line.lower())
            if duration_match:
                try:
            ...
            records_match = re.search(r'(?:processed|extracted|loaded|transformed).*?(\d+).*?(?:records?|items?|articles?)', line.lower())
            if records_match:
                try:
                ...
                if watcher_dir.is_dir() and watcher_dir.name != 'debug':
                    ...
                    if state_file.exists():
                        try:
                            ...
                            logger.warning(f"Failed to read state for watcher {watcher_name}: {e}")
                    ...
                    if events_dir.exists():
                        ...
                        # Get recent events
                        recent_events = sorted(event_files, key=lambda x: x.stat().st_mtime, reverse=True)[:3]
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if category_dir.is_dir():
                        ...
                        if latest_files:
                            ...
                            system_health['data_freshness'][category_dir.name] = {
                                'age_hours': age_hours,
                                'status': 'Fresh' if age_hours < 24 else 'Stale' if age_hours < 72 else 'Very Stale'
                            }
            ...
            fresh_data_count = sum(1 for info in system_health['data_freshness'].values() 
        ...
        try:
            ...
            if log_files:
                ...
                try:
                    ...
                    orchestrator_status['recent_logs'] = [line.strip() for line in recent_lines if line.strip()]
                    ...
                    orchestrator_status['status'] = 'Active' if orchestrator_status['active_orchestrators'] > 0 else 'Inactive'
...
def display_system_overview(monitoring_data: Dict[str, Any]):
    ...
    with col2:
        ...
        if 'error' not in watcher_data:
            ...
            st.metric(
                "Active Watchers",
                active_watchers,
                delta=f"{total_events} total events"
            )
        else:
    ...
    with col3:
        ...
        if 'error' not in system_data:
            ...
            fresh_sources = sum(1 for info in system_data.get('data_freshness', {}).values() 
                              if info.get('status') == 'Fresh')
            st.metric(
                "System Health",
                status,
                delta=f"{fresh_sources} fresh data sources"
            )
        else:
        ...
        for etl_name, data in etl_metrics['performance_by_etl'].items():
            if data.get('runs', 0) > 0:
                etl_details.append({
                    ...
                    'Last Run': data.get('last_run').strftime('%Y-%m-%d %H:%M') if data.get('last_run') else 'Unknown'
                })
    ...
    if watcher_status.get('watcher_details'):
        ...
        for watcher_name, details in watcher_status['watcher_details'].items():
            ...
            watcher_data.append({
                ...
                'Last Value': str(details.get('last_value', 'Unknown'))[:50] + '...' if details.get('last_value') and len(str(details.get('last_value'))) > 50 else str(details.get('last_value', 'Unknown'))
            })
...
def display_system_health(system_health: Dict[str, Any]):
    ...
    if system_health.get('data_freshness'):
        ...
        for source, info in system_health['data_freshness'].items():
            ...
            status_emoji = "🟢" if status == "Fresh" else "🟡" if status == "Stale" else "🔴"
        ...
        if freshness_data:
            ...
            # Table view
            st.dataframe(df_freshness[['Data Source', 'Status', 'Age']], use_container_width=True)
...
def display_logs_viewer():
    ...
    log_files = sorted(list(logs_dir.glob("*.log")), key=lambda x: x.stat().st_mtime, reverse=True)
    ...
    if selected_log:
        ...
        try:
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                show_lines = st.selectbox("Show lines", [50, 100, 200, 500, "All"], index=1)
            with col2:
                log_level_filter = st.selectbox("Filter by level", ["All", "ERROR", "WARNING", "INFO", "DEBUG"])
            with col3:
            ...
            if log_level_filter != "All":
                filtered_lines = [line for line in filtered_lines if log_level_filter in line]
...
def render(logger, data_service=None):
    ...
    st.markdown("Real-time monitoring of ETL processes, watchers, system health, and logs")
