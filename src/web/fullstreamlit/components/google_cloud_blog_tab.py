...
def render(logger: logging.Logger, posts_data: List[Dict[str, Any]]) -> None:
    ...
    try:
        if not posts_data:
            st.info("No Google Cloud Blog posts found or an error occurred while loading the data. Please check the logs or try again later.")
        ...
        for post in posts_data: # Iterate over posts_data directly
            ...
            if isinstance(categories, list):
                category_text = " ".join(str(cat).lower() for cat in categories if isinstance(cat, str))
        ...
        if not relevant_posts:
            st.success("Successfully loaded Google Cloud Blog posts, but no articles matching your keywords (e.g., 'training', 'certification', 'learn') were found in the 'Training and Certifications' feed section.")
            logger.info(f"Found {len(posts_data)} total Google Cloud Blog posts, but none matched the keywords: {KEYWORDS}")
        ...
        st.markdown(f"Displaying **{len(relevant_posts)}** relevant posts based on keywords: `{', '.join(KEYWORDS)}`.")
        st.markdown("---")
        ...
        for post in relevant_posts:
            ...
            if published_str != "No Date Available":
                try:
                    ...
                    st.caption(f"Published: {published_str}") # Display raw if parsing fails
            else:
        ...
        logger.info(f"Successfully rendered {len(relevant_posts)} Google Cloud Blog posts.")

    except Exception as e:
        logger.error(f"An error occurred in the Google Cloud Blog tab: {e}", exc_info=True)
        st.error("An unexpected error occurred while rendering the Google Cloud Blog posts. Please check the logs for more details.")
