...
def render(
    deals_df, bundles_df, giveaways_df, trending_df, new_releases_df, logger=None
):
    ...
    tab_data = _prepare_tab_data(
        deals_df, bundles_df, giveaways_df, trending_df, new_releases_df
    )
    ...
    if not tab_data:
        ...
        with st.expander("🔍 Debug Info", expanded=True):
            ...
            st.write(
                f"- deals_df: tipo={type(deals_df)}, shape={getattr(deals_df, 'shape', 'no shape')}, vacío={getattr(deals_df, 'empty', 'unknown')}"
            )
            st.write(
                f"- bundles_df: tipo={type(bundles_df)}, shape={getattr(bundles_df, 'shape', 'no shape')}, vacío={getattr(bundles_df, 'empty', 'unknown')}"
            )
            st.write(
                f"- giveaways_df: tipo={type(giveaways_df)}, shape={getattr(giveaways_df, 'shape', 'no shape')}, vacío={getattr(giveaways_df, 'empty', 'unknown')}"
            )
            st.write(
                f"- trending_df: tipo={type(trending_df)}, shape={getattr(trending_df, 'shape', 'no shape')}, vacío={getattr(trending_df, 'empty', 'unknown')}"
            )
    ...
    # Render each tab
    for i, (_tab_key, tab_info) in enumerate(tab_data.items()):
        with tabs[i]:
            _render_games_section(
                tab_info["title"], tab_info["data"], tab_info["config"], logger
            )
...
def _prepare_display_data(data: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    # Apply formatting
    display_df = _apply_data_formatting(
        cast(pd.DataFrame, display_df), config["columns"], rename_map
    )
        ...
        if config["sort_by"] == "discount":
            ...
            if sort_col in display_df.columns:
                display_df = display_df.sort_values(
                    by=sort_col, ascending=config.get("sort_ascending", True)
                )
...
def _apply_data_formatting(
    df: pd.DataFrame, column_config: dict, rename_map: dict
) -> pd.DataFrame:
    """Apply formatting to data based on column types."""
    for _col_key, col_config in column_config.items():
...
def _sort_by_discount(
    original_df: pd.DataFrame, display_df: pd.DataFrame, rename_map: dict
) -> pd.DataFrame:
...
def _create_column_config(columns_config: dict) -> dict:
    ...
    for _col_key, col_config in columns_config.items():
        display_name = col_config["display_name"]
        col_type = col_config.get("type", "text")
        ...
        if col_type == "link":
            ...
            column_config[display_name] = st.column_config.NumberColumn(
                label=display_name, width=width
            )
        else:
...
def _render_expandable_games(data: pd.DataFrame, logger) -> None:
    ...
    try:
        ...
        if "released" in sorted_df.columns:
            try:
                sorted_df["released_dt"] = pd.to_datetime(
                    sorted_df["released"], errors="coerce"
                )
        ...
        for _idx, game in sorted_df.iterrows():
            ...
            with st.expander(f"🎮 {game_name}"):
                ...
                with col1:
                    ...
                    if (
                        ...
                        and release_date != "Fecha no disponible"
                        and hasattr(release_date, "strftime")
                    ):
                    ...
                    if isinstance(platforms, list):
                        platforms_str = (
                            ", ".join(platforms) if platforms else "No especificadas"
                        )
                    else:
                        platforms_str = (
                            str(platforms) if platforms else "No especificadas"
                        )
                ...
                with col2:
                    ...
                    if (
                        metacritic is not None
                        and metacritic != "N/A"
                        and str(metacritic).replace(".", "").isdigit()
                    ):
                        metacritic_num = float(metacritic)
                        score_color = (
                            "🟢"
                            if metacritic_num >= 80
                            else "🟡"
                            if metacritic_num >= 60
                            else "🔴"
                        )
                        st.markdown(f"**Metacritic:** {score_color} {metacritic}")
                    else:
                        st.markdown("**Metacritic:** N/A")
