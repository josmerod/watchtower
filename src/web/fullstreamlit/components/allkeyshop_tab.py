...
def render(allkeyshop_data: list[dict[str, Any]], logger=None):
    ...
    if not allkeyshop_data:
        ...
        st.info(
            "💡 Ejecuta el ETL de AllKeyShop para obtener los últimos deals de juegos."
        )
        return
...
def display_summary_stats(df: pd.DataFrame):
    ...
    with col2:
        ...
        st.metric(
            "Precio Promedio", f"€{avg_price:.2f}" if pd.notna(avg_price) else "N/A"
        )
    ...
    with col3:
        ...
        st.metric(
            "Descuento Promedio",
            f"{avg_discount:.1f}%" if pd.notna(avg_discount) else "N/A",
        )
    ...
    with col5:
        best_deals = len(
            df[(df["discount_percentage"] >= 50) | (df["deal_score"] >= 80)]
        )
    ...
    for i, (range_name, (min_price, max_price)) in enumerate(price_ranges.items()):
        with range_tabs[i]:
            ...
            filtered_df = filtered_df.sort_values(
                "discount_percentage", ascending=False, na_last=True
            )
...
def display_all_games(df: pd.DataFrame):
    ...
    search_term = st.text_input(
        "🔍 Buscar juegos:", placeholder="Escribe el nombre del juego..."
    )
    ...
    if sort_by == "Precio (menor a mayor)":
        ...
        filtered_df = filtered_df.sort_values(
            "current_price", ascending=False, na_last=True
        )
        ...
        filtered_df = filtered_df.sort_values(
            "deal_score", ascending=False, na_last=True
        )
    elif sort_by == "Nombre":
    ...
    with col2:
        page = st.number_input(
            f"Página (1-{total_pages}):", min_value=1, max_value=total_pages, value=1
        )
    ...
    st.write(
        f"Mostrando {len(page_df)} de {total_games} juegos (página {page} de {total_pages})"
    )
...
def display_advanced_filters(df: pd.DataFrame):
    ...
    with col1:
        ...
        min_discount = st.slider(
            "Descuento Mínimo (%):", min_value=0, max_value=100, value=0, step=5
        )

    with col2:
        # Deal score filter
        min_deal_score = st.slider(
            "Deal Score Mínimo:", min_value=0, max_value=100, value=0, step=5
        )

        # Store filter
        stores = df["store_name"].dropna().unique()
        selected_stores = st.multiselect(
            "Filtrar por Tienda:", options=sorted(stores), default=[]
        )
    ...
    # Deal score filter
    if min_deal_score > 0:
        filtered_df = filtered_df[filtered_df["deal_score"] >= min_deal_score]
...
def display_game_card(
    game: dict[str, Any], compact: bool = False, show_deal_score: bool = False
):
