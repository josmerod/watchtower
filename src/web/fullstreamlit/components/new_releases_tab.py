...
)  # Go up from components -> fullstreamlit -> web -> src
...
try:
    ...
    print(f"❌ Failed to import get_logger in new_releases_tab: {e}")
...
def render(new_releases_df: pd.DataFrame, logger) -> None:
    ...
    try:
        if new_releases_df is None or new_releases_df.empty:
            ...
            logger.info(
                "New releases DataFrame is empty or None, displaying info message."
            )
            return
        ...
        if "released" in new_releases_df.columns:
            try:
                ...
                new_releases_df["released_dt"] = pd.to_datetime(
                    new_releases_df["released"], errors="coerce"
                )
                # Sort by the new datetime column, NaT values (errors in conversion) will be last
                sorted_df = new_releases_df.sort_values(
                    by="released_dt", ascending=False
                ).drop(columns=["released_dt"])
            except Exception as e:
                logger.warning(
                    f"Could not sort by release date due to error: {e}. Displaying unsorted."
                )
                sorted_df = new_releases_df  # Fallback to unsorted
        else:
            logger.info(
                "No 'released' column found in new_releases_df. Displaying unsorted."
            )
        ...
        for game in sorted_df.itertuples():
            with st.expander(f"{getattr(game, 'name', 'Nombre no disponible')}"):
                ...
                if isinstance(platforms_list, list):
                    ...
                    platforms_list, str
                ):  # If it's already a string (e.g. from bad parsing upstream)
                    platforms_str = (
                        platforms_list if platforms_list else "No especificadas"
                    )
                else:  # Other types
                    platforms_str = (
                        str(platforms_list)
                ...
                st.markdown(
                    f"**Puntuación Metacritic:** {metacritic_score if pd.notna(metacritic_score) else 'N/A'}"
                )

                rawg_link = getattr(game, "rawg_link", None)
                if rawg_link and pd.notna(rawg_link):
                    st.markdown(
                        f"**Enlace RAWG:** [Ver en RAWG]({rawg_link})",
