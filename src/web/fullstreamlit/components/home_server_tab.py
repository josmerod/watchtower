...
def render(logger, data_service):
    ...
    st.markdown(
        "Discover popular and interesting self-hosted applications and trends relevant to home server enthusiasts, primarily sourced from the [awesome-selfhosted](https://github.com/awesome-selfhosted/awesome-selfhosted) list."
    )
    ...
    try:
        ...
        if not trends_data:
            st.info(
                "No home server trend data is currently available. Please run the ETL process."
            )
            return
        ...
        if selected_category != "All":
            filtered_data = [
                item for item in trends_data if item["category"] == selected_category
            ]
        ...
        for item in filtered_data:
            with st.expander(f"{item['name']} ({item['category']})"):
                ...
                st.markdown(
                    f"<small>Source: {item['source']} | Added: {item['added_date']}</small>",
                    unsafe_allow_html=True,
                )

        logger.info(
            f"Rendered home_server_tab with {len(filtered_data)} items for category '{selected_category}'."
        )
