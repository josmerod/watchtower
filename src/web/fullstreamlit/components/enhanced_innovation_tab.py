...
def create_technology_radar_chart(radar_data: dict[str, Any]) -> go.Figure:
    ...
    for category in set(categories):
        ...
        category_techs = [tech for tech, mask in zip(technologies, category_mask, strict=False) if mask]
        category_popularity = [score for score, mask in zip(popularity_scores, category_mask, strict=False) if mask]
        category_growth = [rate for rate, mask in zip(growth_rates, category_mask, strict=False) if mask]
        category_job = [job for job, mask in zip(job_demand, category_mask, strict=False) if mask]
        category_community = [comm for comm, mask in zip(community_health, category_mask, strict=False) if mask]
...
def create_adoption_trends_chart(predictions: dict[str, Any]) -> go.Figure:
    ...
    for rec_type in set(recommendations):
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[score for score, m in zip(predicted_scores, mask, strict=False) if m],
            mode='markers+text',
            name=rec_type,
            text=[tech for tech, m in zip(technologies, mask, strict=False) if m],
            textposition='top center',
            marker={
                'size': [conf/5 for conf, m in zip(confidence_levels, mask, strict=False) if m],  # Size based on confidence
