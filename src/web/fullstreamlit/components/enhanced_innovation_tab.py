...
def create_technology_radar_chart(radar_data: dict[str, Any]) -> go.Figure:
    ...
    for category, battle_data in battles.items():
        for framework in battle_data.get("frameworks", []):
            ...
            growth_rates.append(
                max(0, framework["growth_rate"] * 100)
    ...
    for category in set(categories):
        ...
        category_techs = [
            tech
            for tech, mask in zip(technologies, category_mask, strict=False)
            if mask
        ]
        category_popularity = [
            score
            for score, mask in zip(popularity_scores, category_mask, strict=False)
            if mask
        ]
        category_growth = [
            rate
            for rate, mask in zip(growth_rates, category_mask, strict=False)
            if mask
        ]
        category_job = [
            job for job, mask in zip(job_demand, category_mask, strict=False) if mask
        ]
        category_community = [
            comm
            for comm, mask in zip(community_health, category_mask, strict=False)
            if mask
        ]
        ...
        for i, tech in enumerate(category_techs):
            fig.add_trace(
                go.Scatterpolar(
                    r=[
                        category_popularity[i],
                        category_growth[i],
                        category_job[i],
                        category_community[i],
                    ],
                    theta=[
                        "Popularity",
                        "Growth Rate",
...
def create_framework_battle_chart(battles: dict[str, Any]) -> go.Figure:
    ...
    # Update y-axes
    fig.update_yaxes(
        title_text="Battle Status", secondary_y=False, showticklabels=False
    )
...
def create_adoption_trends_chart(predictions: dict[str, Any]) -> go.Figure:
    ...
    for rec_type in set(recommendations):
        ...
        fig.add_trace(
            go.Scatter(
                x=[score for score, m in zip(current_scores, mask, strict=False) if m],
                y=[
                    score for score, m in zip(predicted_scores, mask, strict=False) if m
                ],
                mode="markers+text",
                name=rec_type,
                text=[tech for tech, m in zip(technologies, mask, strict=False) if m],
                textposition="top center",
                marker={
                    "size": [
                        conf / 5
                        for conf, m in zip(confidence_levels, mask, strict=False)
                        if m
...
def display_enhanced_metrics(radar_data: dict[str, Any]):
    ...
    with col2:
        ...
        avg_confidence = (
            sum(pred.get("confidence", 0) for pred in predictions.values())
            / len(predictions)
            if predictions
            else 0
        )
    ...
    with col3:
        strong_buys = len(
            recommendations.get("investment_grades", {}).get("strong_buy", [])
        )
        st.markdown(
            f"""
...
async def load_technology_radar_data(
    data_service: UltraOptimizedDataService,
) -> dict[str, Any]:
...
def render(logger, data_service=None):
    ...
    with main_tabs[0]:  # Technology Radar
        ...
        if battles:
            ...
            if selected_category != "All":
                ...
                st.markdown(
                    f"**{selected_category.title()} Category Leader: {category_data.get('winner', 'Unknown')}**"
                )
    ...
    with main_tabs[2]:  # Adoption Trends
        ...
        st.markdown(
            "AI-powered predictions for technology adoption over the next 12 months:"
        )
        ...
        if predictions:
            ...
            for idx, (tech_name, prediction) in enumerate(
                list(filtered_predictions.items())[:6]
            ):
