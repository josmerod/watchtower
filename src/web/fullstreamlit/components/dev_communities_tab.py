...
DEV_DATA_SOURCES = {
    "dev_community": os.path.join(
        DATA_DIR, "dev_community", "dev_community_latest.json"
    ),
    "indie_hackers": os.path.join(
        DATA_DIR, "indie_hackers", "indiehackers_posts_latest.json"
    ),
    "lobsters": os.path.join(DATA_DIR, "lobsters", "lobsters_stories_latest.json"),
    "discord_trending": os.path.join(
        DATA_DIR, "discord_trending", "discord_communities_latest.json"
    ),
    "hackernews_ask": os.path.join(
        DATA_DIR, "hackernews_ask", "hackernews_ask_latest.json"
    ),
    "stackoverflow_trends": os.path.join(
        DATA_DIR, "stackoverflow_trends", "stackoverflow_trends_latest.json"
    ),
}
            ...
            if os.path.exists(file_path):
                with open(file_path, encoding="utf-8") as f:
                    ...
                    if data:
                        ...
                        st.sidebar.success(
                            f"✅ {source_name.replace('_', ' ').title()}: {len(df)} items"
                        )
                    else:
                        st.sidebar.warning(
                            f"⚠️ {source_name.replace('_', ' ').title()}: No data"
                        )
            else:
                st.sidebar.error(
                    f"❌ {source_name.replace('_', ' ').title()}: File not found"
                )
        except Exception:
            st.sidebar.error(
                f"❌ {source_name.replace('_', ' ').title()}: Error loading data"
            )
    ...
    with col2:
        ...
        for df in community_data.values():
            if "is_trending" in df.columns:
                trending_count += len(df[df["is_trending"]])
...
def display_dev_community_data(df: pd.DataFrame):
    ...
    with col1:
        content_types = (
            ["All", *list(df["content_type"].unique())]
            if "content_type" in df.columns
            else ["All"]
        )
        selected_type = st.selectbox(
            "Content Type", content_types, key="dev_content_type"
        )
    ...
    with col3:
        popularity = (
            ["All", *list(df["popularity_category"].unique())]
            if "popularity_category" in df.columns
            else ["All"]
        )
        selected_popularity = st.selectbox(
            "Popularity", popularity, key="dev_popularity"
        )
    ...
    if selected_tag != "All" and "tag_list" in df.columns:
        filtered_df = filtered_df[
            filtered_df["tag_list"].str.contains(selected_tag, na=False)
        ]
        ...
        with st.container():
            ...
            with col1:
                ...
                if tags:
                    tag_html = " ".join(
                        [
                            f"<span style='background-color: #E3F2FD; color: #1976D2; padding: 2px 6px; border-radius: 12px; font-size: 12px;'>{tag}</span>"
                            for tag in tags[:4]
                        ]
                    )
            ...
            with col2:
                ...
                st.markdown(
                    f'<span style="background-color: #E8F5E8; color: #2E7D32; padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 500;">{content_type.title()}</span>',
...
def display_stackoverflow_trends(df: pd.DataFrame):
    ...
    with col1:
        tech_categories = (
            ["All", *list(df["tech_category"].unique())]
    ...
    with col2:
        ...
        selected_difficulty = st.selectbox(
            "Difficulty", difficulties, key="so_difficulty"
        )

    with col3:
        answer_status = (
            ["All", *list(df["answer_status"].unique())]
    ...
    for _, question in filtered_df.head(10).iterrows():
        with st.container():
            ...
            with col1:
                st.markdown(
                    f"**[{question.get('title', 'No Title')}]({question.get('link', '#')})**"
                )
                ...
                if tags:
                    tag_html = " ".join(
                        [
                            f"<span style='background-color: #FFF3E0; color: #F57C00; padding: 2px 6px; border-radius: 12px; font-size: 12px;'>{tag}</span>"
                            for tag in tags[:5]
                        ]
                    )
                    st.markdown(tag_html, unsafe_allow_html=True)
...
def display_discord_communities(df: pd.DataFrame):
    ...
    with col1:
        community_types = (
            ["All", *list(df["community_type"].unique())]
            if "community_type" in df.columns
            else ["All"]
        )
        selected_type = st.selectbox(
            "Community Type", community_types, key="discord_type"
        )

    with col2:
        size_categories = (
            ["All", *list(df["size_category"].unique())]
            if "size_category" in df.columns
            else ["All"]
        )
        selected_size = st.selectbox(
            "Size Category", size_categories, key="discord_size"
        )

    with col3:
        activity_levels = (
            ["All", *list(df["activity_level"].unique())]
            if "activity_level" in df.columns
            else ["All"]
        )
        selected_activity = st.selectbox(
            "Activity Level", activity_levels, key="discord_activity"
        )
    ...
    for _, community in filtered_df.head(8).iterrows():
        with st.container():
            ...
            with col1:
                ...
                st.write(
                    community.get("description", "No description available")[:150]
                    + "..."
                )
...
def render(logger, community_data: dict[str, pd.DataFrame] | None = None):
    """Render the Developer Communities tab."""
    st.header("👨‍💻 Developer Communities")
    st.markdown(
        "Discover trending discussions, questions, and communities from the developer ecosystem"
    )
                ...
                if not data.empty:
                    ...
                    with col3:
                        if "is_trending" in data.columns:
                            trending_count = len(data[data["is_trending"]])
    ...
    if community_data:
        st.divider()
        st.markdown(
            "*Data refreshed automatically from multiple developer community sources*"
        )
