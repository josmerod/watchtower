    ...
    try:
        ...
        card_body_content = [
            ...
            dbc.Button(
                ...
                size="sm",
                disabled=not url
            ) if url else html.Span("No hay enlace disponible", className="text-muted")
        ]
    ...
    try:
        ...
        if 'venue' in df.columns:
            ...
            df['venue_name'] = df['venue'].apply(
                lambda x: x.get('name', 'N/A') if isinstance(x, dict) else str(x) if x else 'N/A'
            )
    ...
    try:
        ...
        if not events_data:
            return html.Div([
                dbc.Alert(
                    [
                        html.H4("No hay eventos de Valencia disponibles", className="alert-heading"),
                        html.P("No se encontraron datos de eventos. Ejecuta el ETL de Valencia para poblar los datos."),
                        html.Hr(),
                        html.P(f"Ubicación esperada: {VALENCIA_EVENTS_FILE} o {TECH_EVENTS_FILE}", className="mb-0")
                    ],
                    color="info"
                )
            ], className="p-4")
        ...
        view_toggle = dbc.ButtonGroup([
            dbc.Button("Vista de Tarjetas", id="cards-view-btn", color="primary", outline=True),
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.P("Eventos locales y tecnológicos en Valencia y alrededores", className="text-muted")
                ])
            ], className="mb-4"),
            ...
            html.Div([
                # Cards view (default)
                html.Div(card_columns, id="cards-container", style={'display': 'block'}),
                
                # Table view (hidden by default)
                html.Div(events_table, id="table-container", style={'display': 'none'})
            ], id="events-content")
        ...
        return html.Div([
            dbc.Alert(
                f"Error cargando eventos de Valencia: {str(e)}",
                color="danger"
            )
        ], className="p-4")
