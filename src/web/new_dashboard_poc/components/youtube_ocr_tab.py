...
try:
    ...
    print("Could not import YouTube OCR converter. Please ensure the utility is properly installed.")
    converter = None
else:
    # Load data on module import
    try:
        converter = load_youtube_ocr_data("data/youtube_shorts_ocr/youtube_shorts_ocr_results.json")
...
def render_youtube_ocr_tab():
    ...
    if not converter or not converter.data:
        return html.Div([
            ...
            html.Code("Expected file: data/youtube_shorts_ocr/youtube_shorts_ocr_results.json")
        ])
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(summary.get('total_videos_analyzed', 0), className="card-title"),
                        html.P("Total Videos Analyzed", className="card-text")
                    ])
                ])
            ], width=3),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{summary.get('successful_processing_rate', 0)}%", className="card-title"),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(summary.get('total_urls_extracted', 0), className="card-title"),
                        html.P("URLs Extracted", className="card-text")
                    ])
                ])
            ], width=3),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(summary.get('unique_domains', 0), className="card-title"),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{summary.get('avg_confidence_score', 0)}%", className="card-title"),
                        html.P("Avg Confidence Score", className="card-text")
                    ])
                ])
            ], width=4),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{summary.get('avg_video_duration', 0)}s", className="card-title"),
                        html.P("Avg Video Duration", className="card-text")
                    ])
                ])
            ], width=4),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(summary.get('videos_with_urls', 0), className="card-title"),
                        html.P("Videos with URLs", className="card-text")
                    ])
                ])
            ], width=4),
        ], className="mb-4"),
        ...
        html.Div([
            ...
            html.P(f"Start: {summary.get('processing_date_range', {}).get('start', 'N/A')}"),
            html.P(f"End: {summary.get('processing_date_range', {}).get('end', 'N/A')}")
        ]) if summary.get('processing_date_range', {}).get('start') else html.Div()
    ])
...
def register_youtube_ocr_callbacks(app):
    ...
    def export_data(n_clicks):
        ...
        try:
            ...
            if file_paths:
                return dbc.Alert([
                    html.H5("Data exported successfully!", className="alert-heading"),
                    html.Hr(),
                    html.Div([
                ...
                return dbc.Alert("Export failed. Please check the logs.", color="danger")
        except Exception as e:
        ...
        dbc.Col([
            ...
            dbc.Select(
                ...
                options=[{"label": "All", "value": "all"}] + 
                        [{"label": status, "value": status} for status in basic_df['processing_status'].unique()],
                value="all"
            )
        ], width=6),
        dbc.Col([
...
def render_basic_video_content():
    ...
    fig_confidence = px.histogram(
        ...
        labels={'avg_confidence': 'Average Confidence (%)', 'count': 'Number of Videos'}
    )
    ...
    return html.Div([
        ...
        # Data table
        html.Div([
            html.H5("Video Data Table"),
            html.Div(id="basic-video-table")
        ])
    ])
    ...
    return dbc.Row([
        dbc.Col([
            ...
            dbc.Select(
                ...
                options=[{"label": "All", "value": "all"}] + 
                        [{"label": domain, "value": domain} for domain in sorted(urls_df['domain'].unique())],
                value="all"
            )
        ], width=6),
        dbc.Col([
...
def render_urls_content():
    ...
    fig_confidence = px.box(
        ...
        x='domain',
        y='confidence',
        title='URL Confidence Scores by Domain'
    )
    ...
    return html.Div([
        ...
        html.Div([
            html.H5("URLs Data Table"),
            html.Div(id="urls-table")
        ])
    ])
    ...
    return dbc.Row([
        dbc.Col([
            ...
            dcc.Slider(
                ...
                step=100,
                value=0,
                marks={i: str(i) for i in range(0, int(text_df['text_length'].max()) + 1, 500)}
            )
        ], width=6),
        dbc.Col([
...
def render_text_content():
    ...
    return html.Div([
        ...
        # Data table
        html.Div([
            html.H5("Text Analysis Table"),
            html.Div(id="text-analysis-table")
        ])
    ])
