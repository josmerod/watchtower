...
async def _load_and_process_items(config_dict: dict, source_filter: str | None = None, limit: int = 50, offset: int = 0) -> list[UnifiedItem]:
    """Load items from config sources, converting to UnifiedItem.

    File reads run concurrently via asyncio.to_thread() to avoid blocking
    the event loop.
    """
    items: list[UnifiedItem] = []

    sources_to_load = list(config_dict.keys())
    if source_filter:
    ...
    # Concurrently read all source files in thread pool
    raw_results = await asyncio.gather(*(asyncio.to_thread(load_data_from_file, config_dict[key]["path"]) for key in sources_to_load))

    for key, raw_data in zip(sources_to_load, raw_results, strict=True):
    ...
    return unique_items[offset : offset + limit] if limit > 0 else unique_items[offset:]


async def _load_benchmarks(source_filter: str | None = None, limit: int = 50, offset: int = 0) -> dict:
    """Load benchmark data as raw dicts (not UnifiedItem) for table display.

    File reads run concurrently via asyncio.to_thread() to avoid blocking
    the event loop.
    """
    sources_to_load = list(BENCHMARKS_SOURCES_CONFIG.keys())
    if source_filter:
    ...
    # Concurrently read all benchmark files in thread pool
    raw_results = await asyncio.gather(*(asyncio.to_thread(load_data_from_file, BENCHMARKS_SOURCES_CONFIG[key]["path"]) for key in sources_to_load))
    ...
    return {
        ...
        "models": all_models[offset : offset + limit] if limit > 0 else all_models[offset:],
    }


@router.get("/news", response_model=list[UnifiedItem])
async def get_news(
    source: str | None = Query(None, description="Filter by source key (e.g. 'techcrunch')"),
    limit: int = Query(50, ge=1, le=10000, description="Max items to return"),
    offset: int = Query(0, ge=0, description="Number of items to skip"),
):
    """Get news items."""
    try:
        return await _load_and_process_items(NEWS_SOURCES_CONFIG, source, limit, offset)
...
async def get_knowledge(
    source: str | None = Query(None, description="Filter by source key (e.g. 'opensource')"),
    limit: int = Query(50, ge=1, le=10000, description="Max items to return"),
    offset: int = Query(0, ge=0, description="Number of items to skip"),
):
    """Get knowledge garden items."""
    try:
        return await _load_and_process_items(KNOWLEDGE_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_ecommerce(
    ...
    try:
        return await _load_and_process_items(ECOMMERCE_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_entertainment(
    ...
    try:
        return await _load_and_process_items(ENTERTAINMENT_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_intelligence(
    ...
    try:
        return await _load_and_process_items(INTEL_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_travel(
    ...
    """Get travel items."""
    try:
        return await _load_and_process_items(TRAVEL_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_research(
    ...
    """Get research items."""
    try:
        return await _load_and_process_items(RESEARCH_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_museums(
    ...
    """Get museums items."""
    try:
        return await _load_and_process_items(MUSEUMS_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_games(
    ...
    """Get games items."""
    try:
        return await _load_and_process_items(GAMES_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_benchmarks(
    ...
    try:
        return await _load_benchmarks(source, limit, offset)
    except Exception as e:
...
async def get_arxiv(
    source: str | None = Query(None, description="Filter by source key (e.g. 'papers', 'machine_learning')"),
    limit: int = Query(50, ge=1, le=10000, description="Max items to return"),
    offset: int = Query(0, ge=0, description="Number of items to skip"),
):
    """Get ArXiv research papers."""
    try:
        return await _load_and_process_items(ARXIV_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_ai_platforms(
    source: str | None = Query(None, description="Filter by source key (e.g. 'replicate')"),
    limit: int = Query(50, ge=1, le=10000, description="Max items to return"),
    offset: int = Query(0, ge=0, description="Number of items to skip"),
):
    """Get AI platform model data."""
    try:
        return await _load_and_process_items(AI_PLATFORMS_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_expanded(
    source: str | None = Query(None, description="Filter by source key (e.g. 'github_analytics', 'stackexchange')"),
    limit: int = Query(50, ge=1, le=10000, description="Max items to return"),
    offset: int = Query(0, ge=0, description="Number of items to skip"),
):
    """Get expanded intelligence data (GitHub, StackExchange, OpenAlex, packages, Kaggle)."""
    try:
        return await _load_and_process_items(EXPANDED_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_spanish_aid(
    ...
    try:
        return await _load_and_process_items(SPANISH_AID_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_cloud_updates(
    ...
    try:
        return await _load_and_process_items(CLOUD_UPDATES_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
...
async def get_valencia_local(
    ...
    try:
        return await _load_and_process_items(VALENCIA_LOCAL_SOURCES_CONFIG, source, limit, offset)
    except Exception as e:
