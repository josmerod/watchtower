...
def force_restart():
    ...
    try:
        ...
        _stdin, stdout, stderr = ssh.exec_command(cmd_check)
        ...
        if ts:
            ...
            _stdin, stdout, stderr = ssh.exec_command("ls -l /mnt/user/appdata/watchtower/data/news/")
            print("Directory listing:")
            print(stdout.read().decode())

        # 2. Restart Container
        print("\n--- Restarting Watchtower Container ---")
        _stdin, stdout, stderr = ssh.exec_command("docker restart watchtower")
        ...
        _stdin, stdout, stderr = ssh.exec_command("docker ps | grep watchtower")
        print(stdout.read().decode())
